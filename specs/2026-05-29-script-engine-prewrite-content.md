...
IronLint's `script` engine evaluates the **on-disk file**, never the **proposed content** a caller supplies via `ironlint check --file <path> --content -`. In *post-write* gating (the edit already landed on disk) that's correct and invisible. In *pre-write* gating (the gate runs **before** the edit lands) it is silently wrong: the linter inspects stale or absent content, so violations in the proposed edit pass straight through.
...
## 2. What IronLint is (just enough to investigate)

IronLint is a policy-enforcement pipeline for AI coding agents (Rust rewrite of `dynamik-dev/bully`). A coding harness (Claude Code, Reasonix, OpenCode, …) calls `ironlint check` on each edit; IronLint returns a verdict and an exit code; the harness's hook turns that into "allow" or "block."

**Engines** (`crates/ironlint-core/src/engine/`), selected per rule by `engine:`:
...
**Adapter model:** each harness ships a `hooks/hook.sh` under `adapters/<harness>/`. The hook parses that harness's event payload, extracts the changed file (and, for pre-write, the proposed content), invokes `ironlint check`, and maps the exit code to the harness's allow/block convention. **Lifecycle differences between harnesses live in the adapter; core is supposed to be harness-agnostic.**

---

## 3. The root cause, in code

`crates/ironlint-core/src/engine/script.rs:31-51`:
...
pub fn run_script_rule(
    ...
    // `{file}` expands to the shell parameter `"$IRONLINT_FILE"`. ...
    let substituted = script.replace("{file}", "\"$IRONLINT_FILE\"");
    let caps = rule.capabilities.clone().unwrap_or_default();
    let file_str = file.display().to_string();
    let outcome =
        run_with_capabilities_env(&substituted, cwd, &caps, &[("IRONLINT_FILE", &file_str)])?;
    ...
}
```

`IRONLINT_FILE` is set to `file` — the real on-disk path — and the proposed content is **never referenced**. The engine's `RuleContext` *does* carry the content (`engine/mod.rs:17-25`, field `content: Option<&'a str>`), and the runner *does* populate it (`runner.rs:719-735`). The `script` engine simply ignores `ctx.content`:
...
- `crates/ironlint-cli/src/cli.rs:35-38` (the `--content` help text);
...
```bash
HX="$HOME/.cargo/bin/ironlint"          # ironlint 0.1.0
REPRO="$(mktemp -d)"; cd "$REPRO"
cat > .ironlint.yml <<'YAML'
schema_version: 2
rules:
  no-forbidden:
...
ironlint trust >/dev/null

# CASE 1 — proposed content (stdin) CONTAINS FORBIDDEN -> should BLOCK
printf 'FORBIDDEN\n' | "$HX" check --file foo.txt --content - --config .ironlint.yml --format json
...
**Actual output (verbatim, ironlint 0.1.0):**
...
`rbac-node`'s entire `.ironlint.yml` is one `script` rule:
...
Direct confirmation from the transcript that biome itself *can* see proposed content via stdin (it just never gets the chance through IronLint):
...
$ printf 'var x = 1;\n' | ironlint check --file packages/core/src/foo.ts \
      --content - --config .ironlint.yml --format json
  { "status": "pass", "passed_checks": ["biome-check"] }   # ironlint misses it
```
...
Author opts in by writing their tool's normal stdin invocation in `.ironlint.yml` (`biome check --stdin-file-path={file}`, `ruff check --stdin-filename {file} -`, …). `{file}` stays a path *hint*.
...
- **Cons:** opt-in, so a naive `{file}` rule silently under-gates pre-write (discoverability burden — mitigate at the edges: scaffold stdin form in `ironlint init`; warn from `--explain`/a config lint). Only works for stdin-capable tools (but see §7 — the rest shouldn't be pre-write gates). Stdin bypassing the tool's own ignore means IronLint's `scope:` becomes the sole filter (usually desired; occasionally surprising). Touches `capability.rs`, including the Linux namespaced-clone path (see §10).
...
Keep the limitation; make pre-write adapters detect a `script`-only `.ironlint.yml` and warn that script rules won't gate new-file writes; steer authors to `ast`/`semantic`/`session`.
...
- **Lifecycle variability → the adapter.** Each harness's `hook.sh` is the only place that knows pre-vs-post, payload shape, and exit-code meaning. Its one job: produce `(path, content)` for the proposed/landed state and call `ironlint check --file <path> --content -`.
- **Tool variability → `.ironlint.yml`.** The script string holds the tool's stdin incantation. Author-owned; not core surface.
...
**Verified.** Every load-bearing claim holds: root cause is `script.rs:13-20` discarding `ctx.content` (content is intact at `runner.rs:727` and handed identically to all engines at `737-744`); AST/semantic honor content via `context.rs:44-50`; biome's `--stdin-file-path` behavior (path need not exist, extension decides language, **stdin bypasses ignore**) reconfirmed against current biome docs. Empirical anchor (ironlint 0.1.0): the §4 repro reproduces both directions, and a stdin-form rule (`! grep -q FORBIDDEN`, no `{file}`) **passes** even with `FORBIDDEN` piped via `--content -` — because the child inherits ironlint's fd 0, which is at EOF after `--content` is consumed. The bare predicate `printf FORBIDDEN | sh -c '! grep -q FORBIDDEN'` exits 1. The gap between those two is exactly the content Option A delivers — proving A both necessary and a sufficient mechanism.
...
2. **Opt-in vs. discoverability.** Is the silent under-gating of naive `{file}` rules acceptable if mitigated by `ironlint init` scaffolding + an `--explain`/config-lint warning? Or is implicit "do the right thing" worth more core complexity? Could a lint *detect* "script rule reads `{file}` but not stdin, and a pre-write context is in play" reliably?
...
- Root cause: `crates/ironlint-core/src/engine/script.rs:31-51`
- Engine context / why AST/semantic differ: `crates/ironlint-core/src/engine/context.rs:16-50`; `engine/mod.rs:17-25`
- Runner content plumbing: `crates/ironlint-core/src/runner.rs:719-744` (dispatch), `945-1000` (`resolve_check_input`, `content_authoritative`)
- Subprocess spawn (where stdin would be piped): `crates/ironlint-core/src/engine/capability.rs:445-499`
- `--content` CLI flag + its help text documenting the limitation: `crates/ironlint-cli/src/cli.rs:17-47`; `commands/check.rs:89-120,446-464`
...
- Existing tests to extend: `crates/ironlint-cli/tests/adapter_reasonix.rs`, `cli_check_content.rs`
...
- The incident project: `/Users/chrisarter/Documents/projects/rbac-node` (`.ironlint.yml`, `ironlint-install-session.md`)
