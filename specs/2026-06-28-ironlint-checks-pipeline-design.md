# IronLint 0.4 — checks, lifecycles, and the local-CI repositioning

**Status:** design, approved direction (2026-06-28)
**Builds on:** the 0.3 gates substrate (`specs/2026-06-15-ironlint-gates-redesign-design.md`)
...
| `$IRONLINT_EVENT ∈ {edit, write, pre-commit, manual}` | now just `{write, pre-commit}` (drop `edit` and `manual`); add `$IRONLINT_FILES` | §4 |
...
Everything else from 0.3 — `extends` (cycle-checked DFS, local wins), the out-of-repo trust store, `execution.timeout_secs` + `IRONLINT_TIMEOUT`, scope semantics (`config/scope.rs`, bare-pattern-matches-any-depth), inline disable, `ironlint verify`/`doctor` direction, the **outer** exit-code contract — is unchanged.

## 3. The config language (the whole thing)

```yaml
# .ironlint.yml — checks that run on your agent's edits and at commit, locally.
...
checks:
  ...
  rustfmt:
    files: '**/*.rs'
    run: rustfmt --check "$IRONLINT_FILE"
  ...
  typescript:
    ...
    steps:
      ...
      - name: No focused tests
        run: '! grep -n "\.only(" $IRONLINT_FILES'

  # project analysis: runs once at the root, walks the tree itself. ironlint is
  # pure plumbing — it triggers, sets cwd + $IRONLINT_FILES, reads the exit code.
  no-cycles:
    files: 'src/**/*.ts'
    on: [pre-commit]
...
- `name` — optional human label for output (`✗ typescript › Compiles`). The **map key remains the stable id** for `--check`, `ironlint-disable`, and telemetry; `name` is cosmetic.
...
IronLint runs each step and reads **only its exit code**. The block threshold moves from "exactly `2`" to "any ordinary failure":
...
**Inline disable** is unchanged in mechanism, renamed in vocabulary: `ironlint-disable: <check-id>` in a checked file suppresses that check for that file (file-wide; directive ends at whitespace/`*`/`/`).
...
Near line-for-line. Mapping: `pre-commit:`→`on: [pre-commit]`, `commands.<id>`→`checks.<id>`, `glob:`→`files:`, `{staged_files}`→`$IRONLINT_FILES`, `run:`→`run:`. **Absorbed:** the gate role. **Declined:** `parallel`, `stage_fixed`, `exclude`/`root`/`tags` — the fixer/task-runner half. **Added:** the `write` lifecycle — lefthook's earliest reach is `pre-commit`, after the agent already wrote the files; ironlint fires on the write itself. Structural inversion: lefthook keys by hook (duplicate a command to run it at two hooks); ironlint keys by check (`on: [write, pre-commit]`, no duplication).
...
- **`runner.rs`** — dispatch keys on lifecycle: `write` = per matching file with stdin + `$IRONLINT_FILE`/`$IRONLINT_FILES`; `pre-commit` = run-once over the matched set with `$IRONLINT_FILES`, no `$IRONLINT_FILE`, empty stdin. Honor the `on:` filter (skip checks whose `on` excludes the event) — it always applies; the event (`write`/`pre-commit`) maps directly to a `Lifecycle`.
- **`verdict.rs`** — schema 5, `check`/`step` keys, nullable `file`/`step`.
- **`telemetry.rs`** — retarget keys/vocabulary; per-invocation record.
- **adapters** — emit `$IRONLINT_EVENT=write` (folding the old `edit`) and `$IRONLINT_FILES`; the pre-commit adapter passes the staged set. (The full adapter `--event`/ABI side remains Plan 4 territory; 0.4 locks the contract they target.)
...
- **docs / README / `ironlint-config` skill / `ironlint schema`** — reposition to "local CI for agents," the checks vocabulary, nonzero-blocks, and the `write`/`pre-commit` ABI split.
...
- **Unit (ABI):** `$IRONLINT_FILES` set in both lifecycles; `$IRONLINT_FILE`/stdin only on `write`; `pre-commit` runs once over a multi-file set.
- **E2E (`assert_cmd`):** `write` per-file stdin gating; `pre-commit` run-once with `$IRONLINT_FILES` (a project-shape check that walks a tree, and a file-arg check that consumes the set); lefthook-parity `prettier`; nonzero-blocks for a real tool (`rustfmt --check`) with no remap.
...
Reusable actions (`uses:`/`with:`), a `scope:` field, parallel dispatch, `stage_fixed`-style auto-staging, output parsing, per-tool modeling. Deferred but compatible (no shape change): **report-all** steps (run every step, collect all failures) as an opt-in alternative to fail-fast; a NUL-delimited `$IRONLINT_FILES`; per-step `continue-on-error`. Batch de-duplication remains out (the `pre-commit` run-once model removes the original motivation).
...
3. **`on:` default** — `[write]`. `write` covers editing and new files (folds 0.3's `edit`). `ironlint check` likewise defaults to `--event write`.
4. **`pre-commit` shape** — run **once** over the changed set via `$IRONLINT_FILES`; not per-file. Resolves the per-set question deferred in 0.3 §5.
5. **ABI** — add `$IRONLINT_FILES` (both lifecycles); `$IRONLINT_FILE`/stdin are `write`-side; `$IRONLINT_EVENT` is `write` or `pre-commit` only (drop both `edit` and `manual`).
