# IronLint — Plan & Phase 0.1 Design
...
This document locks the open decisions from `overview.md`, sequences the work from 0.1 to 1.0, and details the 0.1 implementation. Read `overview.md` first for context (what IronLint is and why).

`overview.md` describes IronLint at 1.0. This document describes how we get there, and what ships at 0.1 specifically.
...
|-------|-------|--------------|
| **0.1** | Bully parity, in Rust | `ironlint-core` crate + `ironlint` binary. Engines: `script`, `ast`, `semantic`, `session`. Trust + capabilities. Schema v2 with `context: diff \| file \| repo`. `.bully.yml` reads with deprecation warning. Anthropic provider only. Claude Code adapter at parity (PostToolUse + Stop, skills, no subagent). Commands: `check`, `validate`, `init`, `migrate`, `trust`, `baseline`. JSON verdict locked-but-unstable. |
| **0.2** | Provider + tool-agnostic | OpenAI provider. Aider adapter (config-only). pre-commit adapter. `ironlint doctor`. Telemetry/baseline parity with bully; review tooling. |
| **0.3** | Universal adapter + watch | MCP server (`ironlint serve --mcp`). `ironlint watch` daemon. JSON verdict frozen. CI/credentials story (overview.md open Q #1). |
...
| **1.0** | Stable + documented | Verdict contract guaranteed stable across minor versions. Docs site at ironlint.dev. Marketplace rule-packs design (impl deferred). |
...
│   │   │   ├── baseline.rs
│   │   │   ├── telemetry.rs
│   │   │   ├── disable.rs     ← `ironlint-disable:` comment scanner
│   │   │   └── llm/           ← LlmClient trait + anthropic impl
│   │   └── tests/
│   ├── ironlint-cli/            ← binary; thin
│   │   └── src/main.rs
│   └── ironlint-ast-grep/       ← thin wrapper (linked-crate, fallback to shell)
├── adapters/
│   └── claude-code/
│       ├── plugin.json
│       ├── hooks/
...
└── examples/                  ← .ironlint.yml samples per stack
```
...
## 5. Public API surface — `ironlint-core`
...
# Generated by `ironlint trust`. Mismatch blocks all runs.
trust:
...
**Backward compat:** `.bully.yml` files at `schema_version: 1` parse unchanged. A deprecation warning prints once per run suggesting `ironlint migrate`. Schema-v1 rules at engine `script`, `ast`, `semantic`, `session` all map directly. `capabilities` block is absent on v1 rules — defaults apply (deny).

## 7. Trust + capability gate

**Trust file format.** `trust:` is embedded in `.ironlint.yml`, not a sidecar. The fingerprint covers the canonicalized YAML body minus the `trust:` block itself.
...
`ironlint trust` writes the fingerprint back into `.ironlint.yml`. Every other command re-computes and compares; mismatch → exit 1 with message: `"config changed since last trust — review changes and run 'ironlint trust' to acknowledge"`.

A missing or empty `trust:` block is treated as a mismatch (i.e. untrusted). `ironlint init` does **not** auto-trust — the user reviews the scaffolded config and runs `ironlint trust` explicitly. First `ironlint check` after `init` will fail with the trust-mismatch message; this is intentional friction.
...
`ironlint check` failing the trust check exits 1 (internal error), not 2 (block). Distinct from rule violations.
...
Fallback if `ast_grep_core` API proves unstable: shell out to `ast-grep --pattern` and parse JSON output. The `ironlint-ast-grep` wrapper crate isolates this swap.
...
Changeset persistence: `.ironlint/session.json`, written per PostToolUse invocation. Schema:
...
`ironlint check --session` reads `.ironlint/session.json`, evaluates session rules over the aggregated diff, clears state. Session rules default to `context: repo` (a single edit rarely tells the full story).

## 10. CLI commands at 0.1

| Command | Purpose |
|---------|---------|
| `ironlint check` | Run pipeline. Flags: `--file`, `--diff <path>`, `--staged`, `--session`, `--rule <id>`, `--print-prompt`, `--format human\|json`. |
| `ironlint lint <path>` | Sugar for `check --file <path>`. |
| `ironlint init` | Detect stack, scaffold `.ironlint.yml`. |
| `ironlint validate` | Parse + enum-check config; resolve `extends:`. |
| `ironlint trust` | Compute fingerprint; write into `.ironlint.yml`. |
| `ironlint baseline` | Record current violations to `.ironlint/baseline.json`. |
| `ironlint migrate` | Rewrite `.bully.yml` → `.ironlint.yml`, bump schema, prompt for `llm:` block, move `.bully/` → `.ironlint/`. |
...
Deferred past 0.1: `ironlint watch` (0.3), `ironlint serve --mcp` (0.3), `ironlint doctor` (0.2), `ironlint adapter <name> install` (0.2).
...
- `adapters/claude-code/hooks/post_tool_use.sh` — wraps `ironlint check --diff <stdin> --format json`. Exit 2 on block, 0 otherwise. Stderr → user-visible message.
- `adapters/claude-code/hooks/stop.sh` — wraps `ironlint check --session --format json`. Same exit-code mapping.
- `adapters/claude-code/skills/ironlint-init/` — ported from `bully-init`.
- `adapters/claude-code/skills/ironlint-author/` — ported from `bully-author`.
- `adapters/claude-code/skills/ironlint-review/` — ported from `bully-review`.

**Subagent removed.** Bully's `bully-evaluator` subagent provided context isolation (no `Read`/`Grep`/`Glob`, only sees the diff). At IronLint, semantic evaluation goes directly through the configured `LlmClient`. The trade-off (per overview.md open Q #5):
...
1. `ironlint adapter claude-code install` — defer to 0.2 (the command). At 0.1, ship install instructions in README + `plugin.json` ready for `/plugin install`.
2. `ironlint validate` accepts `.bully.yml` with deprecation log line on first parse.
3. `ironlint migrate`:
    - Reads `.bully.yml`, rewrites as `.ironlint.yml` (rename only, schema_version 1 → 2).
    ...
    - Moves `.bully/log.jsonl` → `.ironlint/log.jsonl`, `.bully/baseline.json` → `.ironlint/baseline.json`, `.bully/session.json` → `.ironlint/session.json` if present.
    - Leaves `.bully.yml` in place by default, optionally deletes with `--clean`.
4. Skill name redirects: `/bully-init` → `/ironlint-init` etc. — best-effort within Claude Code adapter, one-line deprecation log. Drop after 0.4.
...
- **Unit (`ironlint-core`)** — config parser (every YAML edge case from bully's corpus), scope matcher, diff parser, capability gate, trust fingerprinting, disable-comment scanner.
- **Engine fixtures** — table-driven: `(rule, file, diff) → expected_verdict`. Port bully's fixtures verbatim so parity is observable.
- **Golden JSON** — snapshot each `ironlint check` against a small fixture repo; PRs fail on unintended verdict drift.
- **Anthropic LlmClient** — `wiremock-rs` for HTTP stubbing in CI. One nightly integration test against real API gated on `IRONLINT_NIGHTLY=1`.
- **Claude Code adapter** — shell-based: stub `ironlint` with canned JSON, drive hook scripts, assert exit code + stderr.
- **Migration** — every `.bully.yml` fixture parses; round-trip through `ironlint migrate` produces semantically-equivalent `.ironlint.yml`.

**Bench parity (gates 0.2 graduation):**

- Port `bully/bench` harness.
- Run 10–20 rule fixture set on bully vs. IronLint.
...
| D | Trust system — fingerprint, gate, `ironlint trust` |
...
| L | Migration UX polish — `ironlint migrate`, redirects, deprecation log |
...
| Risk | Mitigation |
|------|-----------|
| `ast_grep_core` Rust API is unstable / private | `ironlint-ast-grep` wrapper isolates dep. Fall back to shelling out to `ast-grep` CLI if blocked at step F. Spike step F first. |
...
- **Mojo / MAX-backed `LlmClient`** for offline inference. `LlmClient` trait already accommodates; a third-party `ironlint-llm-mojo` crate is a weekend project if Modular ships stable Rust FFI.
- **Rule packs / registry** (`ironlint pack add react-strict`). Design namespacing in `extends:` semantics now so it's painless to add later.
