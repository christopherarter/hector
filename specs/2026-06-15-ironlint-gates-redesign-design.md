# IronLint 0.3 — the "gates" redesign

**Status:** design, approved direction (2026-06-15)
**Supersedes:** the `engine`/`script`/`ast` rule model from `specs/2026-05-11-ironlint-plan-and-0.1-design.md`
**Breaking:** total. No migration path — ironlint is not yet distributed, so the old config shape is simply unsupported.

## 1. Thesis

A bash script plus a `case` statement is enough enforcement for one harness, one
repo, one person. IronLint earns its place only when the *same* enforcement pattern
...
So ironlint owns the boring, portable substrate and **knows nothing about any tool**:
...
- **A hard-block contract** — stable exit codes, stable output, no "warning"
  ambiguity.
- **Proof** — `ironlint verify` proves an intentional violation actually blocks
  through the *real* harness wiring. `ironlint doctor` reports static wiring
...
What ironlint explicitly does **not** own: knowledge that biome is stdin-capable,
that depcruise needs a truthful tree, a taxonomy of check kinds, or a model of any
CLI. The *scripts* own tool behavior. IronLint owns the guarantees that make those
...
gates:
  ...
  depcruise:
    files: ["src/**/*.{ts,tsx}"]
    run: ".ironlint/gates/depcruise.sh"
  no-console:
    files: ["**/*.ts"]
    run: "! grep -nH 'console.log' \"$IRONLINT_FILE\" || exit 2"
```
...
- `run` — a shell command string, handed to the shell **verbatim**. IronLint does
  **no** string templating — no `{file}`, no `{path}`. Everything a gate needs
  ...
  script under `.ironlint/gates/`; the shell makes no distinction.
...
IronLint runs `run` and reads **only its exit code**:

| gate exit code               | meaning                          | contributes to ironlint verdict |
| ---------------------------- | -------------------------------- | ----------------------------- |
| `2`                          | **Block**                        | Block                         |
| `0`, `1`, `3`–`125`          | **Pass**                         | —                             |
...
2 or 0 on its own judgement, with no special support from ironlint.
...
### IronLint's own (outer) exit codes — unchanged, adapters depend on them
...
- `3` — InternalError (≥1 gate crashed: 127 / timeout / signal). Adapters fail-open
  by default; `IRONLINT_FAIL_CLOSED_ON_INTERNAL=1` flips to fail-closed.
...
| `$IRONLINT_ROOT` | project root (directory containing `.ironlint.yml`)           |
| `$IRONLINT_EVENT`| trigger: `edit` \| `write` \| `pre-commit` \| `manual`      |
...
(`biome check --stdin-file-path "$IRONLINT_FILE"`). A gate that needs a truthful
on-disk tree ignores stdin and reads `$IRONLINT_FILE` / scans `$IRONLINT_ROOT`
...
run (`ironlint check` over a tree, CI over a PR diff) a whole-project gate like
...
- **Trust store:** `~/.config/ironlint/trust.json` (XDG-respecting), keyed by the
  canonical absolute path of `.ironlint.yml`.
- **Trust hash:** sha256 over the normalized `.ironlint.yml` bytes **plus the bytes
  of every file under `.ironlint/gates/`** (sorted by relative path). Hashing the
  ...
  config and `.ironlint/gates/`; a `run` that sources scripts elsewhere is the
...
- **On check:** recompute the hash; compare to the blessed entry. Match → run.
  ...
  message `config/gates not trusted — review and run \`ironlint trust\``. No
  interactive auto-prompt (the hook has no human); blessing is always the explicit
  `ironlint trust` action.
- **`ironlint trust`** recomputes and writes the blessed hash to the store.
  **`ironlint init`** auto-blesses the config it just wrote.
- **Property for free:** a fork PR (or an agent) that edits a gate script
  ...
  re-blesses. This is the security story ironlint did not actually have before.

## 7. Proof — `ironlint verify` (the flagship) + `ironlint doctor`
...
- **`ironlint verify`** — *dynamic* proof. For each gate, synthesize a known-bad
  ...
  `.ironlint/gates/<gate>.bad` (or a `# ironlint:bad-fixture <path>` hint); gates
  without a fixture report `unproven` rather than passing.
- **`ironlint doctor`** — *static* diagnostics (extends today's command): is each
  adapter's hook installed and pointing at this ironlint? Is `.ironlint.yml` parseable
...
| `init`             | keep — scaffold `.ironlint.yml` + `.ironlint/gates/` stubs; auto-bless.  |
...
- **Telemetry** (`.ironlint/log.jsonl`, append-only) survives, keyed to the gate
  model: one record per gate×file with `gate`, `file`, `exit_code`, `verdict`,
  `duration_ms`, `event`.
- **Inline disable** survives, renamed to the gate vocabulary:
  `ironlint-disable: <gate-id>` in a checked file suppresses that gate for that file
...
- E2E (`assert_cmd` against the compiled binary): the three example gates above,
  block vs pass via exit code, `--content`/stdin pre-write gating, `ironlint verify`
  green/red, `ironlint doctor`.
...
1. **Timeout** — `execution.timeout_secs` defaults to **30**, overridable per-run by
   the `IRONLINT_TIMEOUT` env var (env wins over config).
2. **`ironlint verify` fixtures** — convention is an executable
   `.ironlint/gates/<gate>.bad` per gate; a gate with no fixture reports `unproven`
