# hector-config authoring skill — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Teach every supported harness how to author hector gates by installing one canonical `hector-config` Agent-Skills `SKILL.md` during `hector init`, sourced from a single embedded guide, plus a `hector schema` command that prints it.

**Architecture:** A single `adapters/shared/hector-config/SKILL.md` is the only authoring source. It is `include_str!`-embedded into the binary, printed by a new `hector schema` command, and materialized as a real `SKILL.md` into each detected harness's skills directory by `hector init` (reusing the existing `adapter::materialize` primitives). The hand-maintained `hector-author` skill is retired.

**Tech Stack:** Rust (Cargo workspace: `hector-core` lib, `hector-cli` bin), clap, serde, `assert_cmd` for CLI tests, `tempfile` for fs tests.

## Global Constraints

- A gate is exactly `{ files, run }`; no engines, no severity, no `{file}` templating. The guide content must use the 0.3 gates vocabulary only (the drift guard forbids `engine:`, `severity`, `rule_id`, `passed_checks`, `violations`, `{file}`, `capabilities:`, `hector migrate`).
- Skill name and install dir are `hector-config`; skills install at `<skills_dir>/hector-config/SKILL.md`.
- CLI command name is `hector schema`.
...
- `Cargo.lock` is gitignored — never commit it. Binary is `hector`.
...
### Task 1: Canonical guide + retire `hector-author` + drift guard

**Files:**
- Create: `adapters/shared/hector-config/SKILL.md`
- Delete: `adapters/claude-code/skills/hector-author/SKILL.md` (and its now-empty `hector-author/` dir)
- Modify: `crates/hector-cli/tests/skills_gates_model.rs`

**Interfaces:**
- Produces: the file `adapters/shared/hector-config/SKILL.md` (later `include_str!`-ed by Tasks 2 and 3). Frontmatter `name: hector-config`. Body teaches the `{files, run}` gates model.

- [ ] **Step 1: Write the failing test**

Replace the body of `crates/hector-cli/tests/skills_gates_model.rs` with this (drops `hector-author` from the scanned set, adds the shared guide):
...
Run: `cargo test -p hector-cli --test skills_gates_model`
Expected: FAIL — `shared_guide_teaches_the_two_field_gate` panics reading the not-yet-created `adapters/shared/hector-config/SKILL.md`, and `hector_author_skill_is_retired` fails because the file still exists.

- [ ] **Step 3: Create the canonical guide**

Create `adapters/shared/hector-config/SKILL.md` with exactly this content:
...
Run: `git rm -r adapters/claude-code/skills/hector-author`
Expected: removes `adapters/claude-code/skills/hector-author/SKILL.md`.
...
```bash
git add adapters/shared/hector-config/SKILL.md crates/hector-cli/tests/skills_gates_model.rs
git rm -r adapters/claude-code/skills/hector-author
git commit -m "$(cat <<'EOF'
feat(skills): add canonical hector-config authoring guide; retire hector-author

One harness-agnostic gate-authoring guide becomes the single source. The
hand-maintained claude-code hector-author skill is consolidated into it. The
...
### Task 2: `hector schema` command

**Files:**
- Create: `crates/hector-cli/src/commands/schema.rs`
- Modify: `crates/hector-cli/src/commands/mod.rs`
- Modify: `crates/hector-cli/src/cli.rs:16-131` (add the `Schema` variant)
- Modify: `crates/hector-cli/src/main.rs:12-66` (dispatch)
- Test: `crates/hector-cli/tests/cli_schema.rs` (new)

**Interfaces:**
- Consumes: `adapters/shared/hector-config/SKILL.md` (from Task 1) via `include_str!`.
- Produces: `commands::schema::run() -> anyhow::Result<i32>`; `Command::Schema`.

- [ ] **Step 1: Write the failing unit test**

Create `crates/hector-cli/src/commands/schema.rs` with ONLY the test module first (so the test compiles against a stub):
...
Add to `crates/hector-cli/src/commands/mod.rs`:

```rust
pub mod schema;
```

Run: `cargo test -p hector-cli commands::schema`
Expected: the four `strip_frontmatter` unit tests PASS. (`run()` is unused for now — that's fine; it's `pub`.)

- [ ] **Step 3: Add the CLI variant**

In `crates/hector-cli/src/cli.rs`, add a new variant to `enum Command` (after `ShowResolvedConfig { … }`, before the closing `}` at line 131):
...
In `crates/hector-cli/src/main.rs`, add an arm to the `match cli.command` (after the `ShowResolvedConfig` arm, before the closing `}` at line 66):
...
Create `crates/hector-cli/tests/cli_schema.rs` (uses only `assert_cmd` + stdout
...
fn schema_prints_the_authoring_guide() {
    let s = schema_stdout();
    assert!(s.contains("$HECTOR_FILE"), "guide must mention $HECTOR_FILE:\n{s}");
...
Run: `cargo test -p hector-cli --test cli_schema`
Expected: PASS (both tests). `assert_cmd` is already a dev-dependency used across the CLI tests; no new dependency is required.

- [ ] **Step 7: Commit**

```bash
git add crates/hector-cli/src/commands/schema.rs crates/hector-cli/src/commands/mod.rs \
        crates/hector-cli/src/cli.rs crates/hector-cli/src/main.rs crates/hector-cli/tests/cli_schema.rs
git commit -m "$(cat <<'EOF'
feat(cli): `hector schema` prints the gate-authoring guide

Embeds the canonical hector-config guide and prints its body (frontmatter
...
**Files:**
- Modify: `crates/hector-core/src/adapter/registry.rs`
- Modify: `crates/hector-core/src/adapter/mod.rs:7-13,67-76` (re-export `SkillSpec`, `SKILL_NAME`; add `skill` field to `Harness`)

**Interfaces:**
- Consumes: `AdapterEnv` (fields `home`, `config_home`, `project_root`); `adapters/shared/hector-config/SKILL.md`.
- Produces:
...
Add to the `#[cfg(test)] mod tests` block in `crates/hector-core/src/adapter/registry.rs`:
...
```rust
    ...
    fn every_harness_ships_the_same_skill_source() {
        for h in all_harnesses() {
            assert!(h.skill.source.contains("name: hector-config"), "{} skill source wrong", h.name);
        }
    }
```
...
Run: `cargo test -p hector-core adapter::registry`
...
In `crates/hector-core/src/adapter/registry.rs`, after the existing embedded-artifact consts (around line 11), add:
...
In `crates/hector-core/src/adapter/registry.rs`, add four consts (next to `CLAUDE`, `REASONIX`, `PI`, `OPENCODE`):
...
In `crates/hector-core/src/adapter/mod.rs`, add to `pub struct Harness` (lines 72-76):
...
In `crates/hector-core/src/adapter/registry.rs`, add the name const near the top:
...
```bash
git add crates/hector-core/src/adapter/registry.rs crates/hector-core/src/adapter/mod.rs
git commit -m "$(cat <<'EOF'
feat(adapter): per-harness SkillSpec + embedded hector-config skill
...
**Files:**
- Modify: `crates/hector-core/src/adapter/ops.rs`
- Modify: `crates/hector-core/src/adapter/mod.rs:12` (re-export `install_skill`, `uninstall_skill`)
...
Add to the `#[cfg(test)] mod tests` block in `crates/hector-core/src/adapter/ops.rs` (the helpers `harness(name)` and `env(tmp)` already exist there):
...
```rust
    ...
    fn install_skill_dry_run_writes_nothing() {
        ...
        assert!(!e.project_root.join(".pi/skills/hector-config/SKILL.md").exists());
    }
    ...
    fn install_skill_global_uses_home_dir() {
        ...
        assert!(e.home.join(".pi/agent/skills/hector-config/SKILL.md").exists());
    }
```

- [ ] **Step 2: Run the test to verify it fails**

Run: `cargo test -p hector-core adapter::ops`
Expected: FAIL to compile — `install_skill` / `uninstall_skill` undefined.

- [ ] **Step 3: Implement the install/uninstall functions**

In `crates/hector-core/src/adapter/ops.rs`, add to the imports at the top:
...
In `crates/hector-core/src/adapter/mod.rs`, update the `ops` re-export (line 12):
...
```bash
git add crates/hector-core/src/adapter/ops.rs crates/hector-core/src/adapter/mod.rs
git commit -m "$(cat <<'EOF'
feat(adapter): install/uninstall the hector-config skill
...
**Files:**
- Modify: `crates/hector-cli/src/commands/init/onboard.rs`
- Test: `crates/hector-cli/tests/cli_init.rs` (add integration assertions)
...
Add to the `#[cfg(test)] mod tests` in `crates/hector-cli/src/commands/init/onboard.rs`:
...
Run: `cargo test -p hector-cli commands::init::onboard`
...
In `crates/hector-cli/src/commands/init/onboard.rs`, add:
...
In `crates/hector-cli/src/commands/init/onboard.rs`, replace the per-harness loop body in `run_hook_phase` so each harness installs its hook AND (subject to dedup) its skill, folding both into `any_ok`/`any_fail`:
...
fn init_dry_run_plans_skill_installs_for_explicit_harnesses() {
    ...
    assert!(s.contains("skills/hector-config/SKILL.md"),
...
(`--dry-run` still scaffolds `.hector.yml`; that's expected and harmless in a tempdir.)
...
```bash
git add crates/hector-cli/src/commands/init/onboard.rs crates/hector-cli/tests/cli_init.rs
git commit -m "$(cat <<'EOF'
feat(init): install the hector-config skill per harness

`hector init` now materializes the authoring skill into each wired harness's
...
**Files:**
- Modify: `docs/reference/cli.md` (add `hector schema`)
- Modify: `docs/adapters/README.md` (retire `hector-author` → `hector-config`; note init installs it for every agent)
- Modify: `docs/adapters/claude-code.md` (drop the `hector-author` reference; point authoring at the init-installed skill + `hector schema`)
- Modify: `README.md` ("Connect your agent" — mention init installs the authoring skill)

**Interfaces:**
- Consumes: the shipped behavior from Tasks 1–5 (command name `hector schema`; skill name `hector-config`; install per harness).

- [ ] **Step 1: Add `hector schema` to the CLI reference**

In `docs/reference/cli.md`, add a section (after `hector show-resolved-config`, before "Read-only commands"):
...
In `docs/adapters/README.md`, in "Managing policy from inside the agent", replace the `hector-author` bullet and the trailing "Claude Code ships all three…" paragraph with copy that reflects the new model: `hector init` installs a **`hector-config`** authoring skill into *every* detected agent (claude-code, pi, opencode, reasonix — all support Agent Skills), and `hector schema` prints the same guide on demand. `hector-init` and `hector-review` remain Claude Code plugin skills.
...
In `docs/adapters/claude-code.md`, in "Author and review gates from inside Claude", replace the `hector-author` mention so it reads: authoring is the `hector-config` skill that `hector init --harness claude-code` installs into `.claude/skills/hector-config/` (and `hector schema` prints the same guide); `/hector-init` and `/hector-review` come with the plugin.

- [ ] **Step 4: Update the root README**

In `README.md`, in the "Connect your agent" section, add one sentence after the `hector init` description: that init also installs a `hector-config` authoring skill so the agent knows how to write and fix gates (or run `hector schema` to read the format yourself).

- [ ] **Step 5: Verify the docs build/read cleanly**

Run: `grep -rn "hector-author" docs/ README.md`
Expected: no matches (every reference migrated to `hector-config`). Then re-read each edited section for accuracy against the shipped behavior.
...
git commit -m "$(cat <<'EOF'
docs: hector-config authoring skill + `hector schema`

`hector init` installs the hector-config authoring skill into every agent;
`hector schema` prints the guide. Retire the hector-author references.
...
**Interfaces:**
- Consumes: the shipped install behavior. The existing `drive.sh` already seeds `~/.reasonix`, `~/.pi`, `~/.config/opencode` and runs `hector init --yes` in `/work`; skills install project-local under `/work` (no `--global`), and claude-code is excluded (no `~/.claude` seeded), so opencode's skill is **not** deduped and lands in `.opencode/skills/`.
...
Expected: PASS — all prior assertions plus the new skill assertions hold. (Requires Docker; the first build compiles a Linux `hector`.)
...
git commit -m "$(cat <<'EOF'
test(e2e): assert hector init installs the hector-config skill
...
- [ ] `grep -rn "hector-author" docs/ README.md adapters/` returns nothing.
```
