# `$IRONLINT_TMPFILE`, `--force`, stack-agnostic `init` — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a write-event `$IRONLINT_TMPFILE` (an ironlint-managed temp file holding proposed content for file-oriented tools), a scope-only `--force` test bypass on `ironlint check`, and make `ironlint init` stack-agnostic (drop Biome/ESLint/ruff templates).

**Architecture:** `$IRONLINT_TMPFILE` is materialized in `runner.rs::run_one_check` (lazily, only when a check references the token; write-event only) and surfaced through a new `GateEnv.tmpfile` field that `engine/gate.rs::run_gate` exports; an RAII guard removes the file on every exit path. `--force` adds a `force: bool` to `CheckOptions`, read in `skip_reason` to suppress only the `out_of_scope` branch for named checks. `init` replaces its per-stack template assemblers with one constant universal baseline and deletes the now-dead `detect.rs`.

**Tech Stack:** Rust (workspace: `ironlint-core` lib + `ironlint-cli` bin `ironlint`), `clap` (CLI), `tempfile` (test dirs), `assert_cmd` (CLI e2e). No new dependencies.

**Design spec:** `specs/2026-06-29-ironlint-tmpfile-force-init-design.md`

## Global Constraints

- **No schema change.** `$IRONLINT_TMPFILE` is additive to the ABI. Verdict stays `SCHEMA_VERSION = 5`; telemetry stays `5`. Do not change any verdict/telemetry shape.
- **ABI is a stability surface.** `$IRONLINT_TMPFILE` is additive (new env var, only ever set, never required). Existing checks that never mention it are byte-for-byte unaffected.
...
- **Binary name is `ironlint`**, not `ironlint-cli`.
...
**Modified (ironlint-core):**
- `crates/ironlint-core/src/engine/gate.rs` — `GateEnv` gains `tmpfile: Option<&Path>`; `run_gate` exports `IRONLINT_TMPFILE`. (Task 1)
- `crates/ironlint-core/src/runner.rs` — `CheckOptions.force`; `skip_reason` honors force; `run_one_check` materializes the temp file via new private helpers + `TmpFileGuard`. (Tasks 2, 3)

**Modified (ironlint-cli):**
- `crates/ironlint-cli/src/cli.rs` — `Check` subcommand gains `--force`. (Task 3)
- `crates/ironlint-cli/src/commands/check.rs` — plumb `force`; validate `--force` requires `--check`. (Task 3)
- `crates/ironlint-cli/src/commands/init/mod.rs` — replace per-stack assemblers with one universal baseline; drop `Stack`/`detect_stack`/`build_config` args/`emit_*`. (Task 4)

**Deleted:**
- `crates/ironlint-cli/src/commands/init/detect.rs` — consumed only by config scaffolding (verified: no other consumer). (Task 4)

**Docs/skill:**
- `AGENTS.md` + `CLAUDE.md` — ABI paragraph (line 60) gains `$IRONLINT_TMPFILE`. (Task 2)
- `adapters/shared/ironlint-config/SKILL.md` — ABI bullets gain `$IRONLINT_TMPFILE` + a temp-file linter example; feeds `ironlint schema`. (Task 2)

**Tests:**
- `crates/ironlint-core/src/engine/gate.rs` (inline `tests`) — gate exports the var. (Task 1)
- `crates/ironlint-core/src/runner.rs` (inline `gate_dispatch_tests`) — materialization/cleanup/lazy/pre-commit + force. (Tasks 2, 3)
- `crates/ironlint-cli/tests/cli_e2e_gates.rs` — `--content` tmpfile path + `--force`. (Tasks 2, 3)
- `crates/ironlint-cli/tests/cli_init.rs` + `crates/ironlint-cli/src/commands/init/mod.rs` inline tests — universal baseline, no tool names. (Task 4)
- `crates/ironlint-cli/src/commands/schema.rs` + `crates/ironlint-cli/tests/cli_schema.rs` — guide mentions `$IRONLINT_TMPFILE`. (Task 2)

---

## Task 1: `GateEnv.tmpfile` + `run_gate` exports `$IRONLINT_TMPFILE`

**Files:**
- Modify: `crates/ironlint-core/src/engine/gate.rs` (struct `GateEnv` ~14-26; `run_gate` env block ~73-85; test helpers ~168-184)
- Modify: `crates/ironlint-core/src/runner.rs` (every `GateEnv { … }` construction: `run_one_check` ~496, `check_set` ~550, any test helper) — add `tmpfile: None`

**Interfaces:**
- Produces: `GateEnv { file: Option<&Path>, files: &[PathBuf], root: &Path, event: &str, tmpfile: Option<&'a Path> }`. When `tmpfile` is `Some(p)`, `run_gate` sets env `IRONLINT_TMPFILE=<p>`; when `None`, the var is unset.
...
fn tmpfile_env_is_unset_when_none() {
    ...
    assert!(matches!(run_gate("test -n \"$IRONLINT_TMPFILE\"", &env, None, t()), GateOutcome::Block { .. }));
}
```

- [ ] **Step 2: Run test to verify it fails (compile error — field missing)**

Run: `cargo test -p ironlint-core tmpfile_env`
...
In `run_gate`, after the `if let Some(f) = env.file { cmd.env("IRONLINT_FILE", f); }` block, add:
...
Run: `cargo test -p ironlint-core tmpfile_env && cargo build`
...
git add crates/ironlint-core/src/engine/gate.rs crates/ironlint-core/src/runner.rs
git commit -m "feat(core): add GateEnv.tmpfile; run_gate exports \$IRONLINT_TMPFILE"
```

---

## Task 2: Materialize `$IRONLINT_TMPFILE` in the runner (lazy, write-only, RAII cleanup) + docs

**Files:**
- Modify: `crates/ironlint-core/src/runner.rs` — new `TmpFileGuard`, `check_references_tmpfile`, `unique_tmp_name`, `maybe_materialize_tmpfile`; rewire `run_one_check` (~484-503)
- Modify: `AGENTS.md`, `CLAUDE.md` (ABI paragraph, line 60), `adapters/shared/ironlint-config/SKILL.md` (ABI bullets ~29-33, examples ~49-54)
- Test: `crates/ironlint-core/src/runner.rs` (`gate_dispatch_tests`), `crates/ironlint-cli/tests/cli_e2e_gates.rs`, `crates/ironlint-cli/src/commands/schema.rs`, `crates/ironlint-cli/tests/cli_schema.rs`
...
- Behavior: on `write`, if the check references `IRONLINT_TMPFILE`, write `content` to `<dir of abs>/ironlint-tmp-<unique>[.<ext>]` (`<ext>` mirrors `abs`'s final extension), export its absolute path, remove it when the guard drops. Materialize failure → `CheckStatus::Error`.
...
fn tmpfile_unset_on_pre_commit() {
    ...
    assert_eq!(verdict.status, Status::Pass, "IRONLINT_TMPFILE must be unset on pre-commit");
}
```

- [ ] **Step 2: Run tests to verify they fail**

Run: `cargo test -p ironlint-core tmpfile_`
...
Add the method to `impl IronLintEngine` (near `run_one_check`):
...
Run: `cargo test -p ironlint-core tmpfile_write_failure_is_internal_error`
Expected: PASS.
...
fn content_flag_materializes_tmpfile() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::write(dir.path().join(".ironlint.yml"),
        "checks:\n  cap:\n    files: \"**/*.rs\"\n    run: \"case \\\"$IRONLINT_TMPFILE\\\" in *.rs) cat \\\"$IRONLINT_TMPFILE\\\" > \\\"$IRONLINT_ROOT/cap.txt\\\"; exit 0;; *) exit 2;; esac\"\n").unwrap();
    let target = dir.path().join("a.rs");
    std::fs::write(&target, "OLD").unwrap();
    // bless trust (check enforces it)
    Command::cargo_bin("ironlint").unwrap().current_dir(dir.path())
        .args(["trust", "--config", ".ironlint.yml"]).assert().success();
    Command::cargo_bin("ironlint").unwrap().current_dir(dir.path())
        .args(["check", "--file", "a.rs", "--content", "-", "--config", ".ironlint.yml"])
...
Run: `cargo test -p ironlint-cli --test cli_e2e_gates content_flag_materializes_tmpfile`
Expected: PASS.

- [ ] **Step 8: Document the new ABI var**

In **`AGENTS.md`** and **`CLAUDE.md`** (identical paragraph at line 60), append to the ABI sentence after the `$IRONLINT_EVENT` clause:

```
, `$IRONLINT_TMPFILE` (write-only, and only when the check's `run`/`steps` reference it: absolute path to an ironlint-materialized temp file — sibling of `$IRONLINT_FILE`, same extension — holding the proposed content; auto-removed after the check)
```

In **`adapters/shared/ironlint-config/SKILL.md`**, add a bullet after the `$IRONLINT_EVENT` bullet (~line 32):
...
In `crates/ironlint-cli/src/commands/schema.rs` (the `GUIDE` test ~line 52-53), add:

```rust
    assert!(strip_frontmatter(GUIDE).contains("$IRONLINT_TMPFILE"));
```

In `crates/ironlint-cli/tests/cli_schema.rs` (after the `$IRONLINT_FILE` assertion ~line 19), add:

```rust
    assert!(s.contains("$IRONLINT_TMPFILE"), "guide must mention $IRONLINT_TMPFILE:\n{s}");
```

Run: `cargo test -p ironlint-cli schema && cargo test -p ironlint-cli --test cli_schema`
Expected: PASS.
...
git add crates/ironlint-core/src/runner.rs crates/ironlint-cli/tests/cli_e2e_gates.rs \
        crates/ironlint-cli/src/commands/schema.rs crates/ironlint-cli/tests/cli_schema.rs \
        AGENTS.md CLAUDE.md adapters/shared/ironlint-config/SKILL.md
git commit -m "feat(core): materialize \$IRONLINT_TMPFILE on write events (lazy, auto-cleaned)"
```
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs` — `CheckOptions.force` (~14-35), `skip_reason` (~432-452)
- Modify: `crates/ironlint-cli/src/cli.rs` — `Check` subcommand (~18-59)
- Modify: `crates/ironlint-cli/src/commands/check.rs` — `run` signature + options + validation (~10-54)
- Test: `crates/ironlint-core/src/runner.rs` (`gate_dispatch_tests`), `crates/ironlint-cli/tests/cli_e2e_gates.rs`

**Interfaces:**
- Consumes: existing `CheckOptions`, `skip_reason`, `commands/check::run`.
- Produces: `CheckOptions { …, force: bool }` (Default `false`); `ironlint check --force` (requires `--check`); `run(…, force: bool)`.
...
Run: `cargo test -p ironlint-core force_runs_out_of_scope_named_check`
...
Run: `cargo test -p ironlint-core force_does_not_bypass_disable_directive`
Expected: PASS.
...
fn force_without_check_is_usage_error() {
    ...
    std::fs::write(dir.path().join(".ironlint.yml"),
        "checks:\n  g:\n    files: \"src/**/*.rs\"\n    run: \"true\"\n").unwrap();
    Command::cargo_bin("ironlint").unwrap().current_dir(dir.path())
        .args(["check", "--file", "x.rs", "--force", "--config", ".ironlint.yml"])
        .assert().failure().code(1);
}
```

Run: `cargo test -p ironlint-cli --test cli_e2e_gates force_without_check_is_usage_error`
Expected: PASS.
...
```bash
cargo test && cargo fmt && cargo clippy --all-targets -- -D warnings
git add crates/ironlint-core/src/runner.rs crates/ironlint-cli/src/cli.rs \
        crates/ironlint-cli/src/commands/check.rs crates/ironlint-cli/tests/cli_e2e_gates.rs
...
**Files:**
- Modify: `crates/ironlint-cli/src/commands/init/mod.rs` — `scaffold_config` (~51-71), remove `Stack`/`detect_stack`/`build_config`/`emit_*`/`scope_list_with_default`, remove `detect::*` import + `mod detect;`, replace with `baseline_config()`; update inline tests (~350-375)
- Delete: `crates/ironlint-cli/src/commands/init/detect.rs`
- Test: `crates/ironlint-cli/tests/cli_init.rs` (~101-124, 203)

**Interfaces:**
- Consumes: nothing new.
- Produces: `fn baseline_config() -> String` (or a `const BASELINE: &str`) — the universal `.ironlint.yml` body, identical regardless of manifest.
...
fn init_scaffolds_universal_baseline_regardless_of_stack() {
    for manifest in ["Cargo.toml", "package.json", "pyproject.toml", "none"] {
        ...
        assert!(cfg.contains("$IRONLINT_TMPFILE"), "{manifest}: missing tmpfile example:\n{cfg}");
...
Run: `cargo test -p ironlint-cli --test cli_init init_scaffolds_universal_baseline_regardless_of_stack`
Expected: FAIL — current output contains `biome`/`no-unwrap`/etc. for detected stacks and lacks `no-merge-markers`/`$IRONLINT_TMPFILE`.
...
# $IRONLINT_TMPFILE holds the proposed content with the right extension, beside
...
Run: `cargo test -p ironlint-cli --test cli_init && cargo build`
...
fn scaffolded_baseline_validates() {
    ...
    Command::cargo_bin("ironlint").unwrap().current_dir(dir.path())
        .args(["init", "--no-hook"]).assert().success();
    Command::cargo_bin("ironlint").unwrap().current_dir(dir.path())
        .args(["validate", "--config", ".ironlint.yml"]).assert().success();
}
```

Run: `cargo test -p ironlint-cli --test cli_init scaffolded_baseline_validates`
Expected: PASS.
...
```bash
cargo test && cargo fmt && cargo clippy --all-targets -- -D warnings
git add crates/ironlint-cli/src/commands/init/mod.rs crates/ironlint-cli/tests/cli_init.rs
git rm crates/ironlint-cli/src/commands/init/detect.rs
...
Run: `rg -l "IRONLINT_TMPFILE" AGENTS.md CLAUDE.md adapters/shared/ironlint-config/SKILL.md`
...
**Spec coverage:**
- §2 `$IRONLINT_TMPFILE` (placement, lazy, write-only, RAII, CLI testing, failure-as-internal, docs) → Tasks 1, 2. ✓
