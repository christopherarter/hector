# `ironlint watch` — Live TUI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a read-only `ironlint watch` TUI that tails `.ironlint/log.jsonl` and shows a live **Stream** (newest-first run feed) and an **Explorer** (per-check health, ranked by blocks), built entirely on today's telemetry — no schema change.

**Architecture:** Pure aggregation + formatting live in a new `ironlint-core::watch` module (fully unit-tested). The CLI command `commands/watch.rs` holds pure view logic (`ViewState`, `handle_key`, `stream_lines`, `explorer_lines`) tested directly, a `ui()` draw function verified with ratatui's `TestBackend`, and a thin impure event loop. Each tick re-reads the log via the existing `telemetry::read_all`, recomputes the summary, and redraws.

**Tech Stack:** Rust (workspace: `ironlint-core` lib + `ironlint-cli` bin `ironlint`), `ratatui` 0.29 (TUI + re-exported `crossterm`), `chrono` (already a dep), `assert_cmd` for CLI e2e.

**Design spec:** `specs/2026-06-28-ironlint-watch-tui-design.md`
...
- **Read-only.** Watch never runs checks, never writes telemetry, and never calls `trust::ensure_trusted`. It loads config via `IronLintEngine::load` (best-effort) and reads the log.
...
**Created (ironlint-core):**
- `crates/ironlint-core/src/watch.rs` — pure view-model + aggregation + formatting. Types: `ArmedCheck`, `CheckRollup`, `LogSummary`. Fns: `summarize`, `fmt_elapsed`, `short_time`, `lifecycle_badge`, `status_glyph`. Carries its own ≥80% via unit tests.

**Modified (ironlint-core):**
- `crates/ironlint-core/src/lib.rs` — add `pub mod watch;`.

**Created (ironlint-cli):**
- `crates/ironlint-cli/src/commands/watch.rs` — `run(dir)`, pure `ViewState`/`View`/`Loop`/`handle_key`/`stream_lines`/`explorer_lines`, draw `ui`/`render_stream`/`render_explorer`, impure `run_tui`/`event_loop`. Plus unit tests + `TestBackend` tests.
- `crates/ironlint-cli/tests/cli_e2e_watch.rs` — e2e: `ironlint watch` in a non-TTY exits 1 with the expected message.

**Modified (ironlint-cli):**
- `crates/ironlint-cli/Cargo.toml` — add `ratatui = "0.29"`.
- `crates/ironlint-cli/src/cli.rs` — add `Watch { dir }` subcommand.
- `crates/ironlint-cli/src/main.rs` — dispatch `Command::Watch`.
- `crates/ironlint-cli/src/commands/mod.rs` — `pub mod watch;`.

**Modified (docs):**
- `docs/reference/cli.md` — document `ironlint watch`.
...
# Phase 1 — Core aggregation (`ironlint-core::watch`)
...
**Files:**
- Create: `crates/ironlint-core/src/watch.rs`
- Modify: `crates/ironlint-core/src/lib.rs` (add `pub mod watch;`)
...
In `crates/ironlint-core/src/lib.rs`, add `pub mod watch;` in alphabetical position (after `pub mod verdict;` is fine, or alongside the others — keep the list sorted: insert after `pub mod verdict;`).

- [ ] **Step 2: Write failing tests for `summarize`.**

Create `crates/ironlint-core/src/watch.rs` with only the test module first (it won't compile until Step 3 adds the types — that's the failing state):
...
Run: `cargo test -p ironlint-core watch::`
...
Insert above the `#[cfg(test)]` block in `crates/ironlint-core/src/watch.rs`:
...
Run: `cargo test -p ironlint-core watch::`
Expected: PASS (all 7 tests). Then `cargo clippy -p ironlint-core --all-targets -- -D warnings` and `cargo fmt` clean.

- [ ] **Step 6: Commit.**

```bash
git add crates/ironlint-core/src/watch.rs crates/ironlint-core/src/lib.rs
...
**Files:**
- Modify: `crates/ironlint-core/src/watch.rs`
...
Add to the `tests` module in `crates/ironlint-core/src/watch.rs`:
...
Run: `cargo test -p ironlint-core watch::tests::fmt_elapsed_boundaries`
Expected: FAIL — `cannot find function fmt_elapsed`.

- [ ] **Step 3: Implement the helpers.**

Add above the `#[cfg(test)]` block in `crates/ironlint-core/src/watch.rs`:
...
Note: `chrono` is already a workspace dependency used by `ironlint-core`'s runner; no `Cargo.toml` change needed. If `cargo build -p ironlint-core` reports `chrono` is not a direct dep of the core crate, add `chrono.workspace = true` to `crates/ironlint-core/Cargo.toml` `[dependencies]` and commit that with this task.

- [ ] **Step 4: Run to verify pass.**

Run: `cargo test -p ironlint-core watch::`
Expected: PASS (all watch tests). `cargo clippy -p ironlint-core --all-targets -- -D warnings` + `cargo fmt` clean.

- [ ] **Step 5: Commit.**

```bash
git add crates/ironlint-core/src/watch.rs crates/ironlint-core/Cargo.toml
...
- Create: `crates/ironlint-cli/src/commands/watch.rs`
...
In `crates/ironlint-cli/Cargo.toml`, under `[dependencies]` (after the `axoupdater` block), add:

```toml
# Live TUI (`ironlint watch`): renders the telemetry stream + per-check explorer.
...
Run: `cargo test -p ironlint-cli --test cli_e2e_watch`
...
In `crates/ironlint-cli/src/commands/mod.rs`, add `pub mod watch;` (keep sorted — after `pub mod validate;` or alphabetically before it; match existing order which is alphabetical, so insert `pub mod watch;` after `pub mod validate;`).

- [ ] **Step 6: Add the clap subcommand.**

In `crates/ironlint-cli/src/cli.rs`, add to `enum Command` (after `Update,`):
...
In `crates/ironlint-cli/src/main.rs`, add to the `match cli.command` (after the `Command::Update` arm):
...
Run: `cargo test -p ironlint-cli --test cli_e2e_watch`
Expected: PASS. Then `cargo clippy -p ironlint-cli --all-targets -- -D warnings` + `cargo fmt` clean.

- [ ] **Step 9: Commit.**

```bash
git add crates/ironlint-cli/Cargo.toml crates/ironlint-cli/src/commands/watch.rs \
  crates/ironlint-cli/src/commands/mod.rs crates/ironlint-cli/src/cli.rs \
  crates/ironlint-cli/src/main.rs crates/ironlint-cli/tests/cli_e2e_watch.rs
git commit -m "feat(cli): add ironlint watch subcommand with no-TTY exit"
```

### Task 2.2: View state + key handling

**Files:**
- Modify: `crates/ironlint-cli/src/commands/watch.rs`

**Interfaces:**
- Consumes: `ironlint_core::watch::LogSummary`.
- Produces:
...
Add a test module to `crates/ironlint-cli/src/commands/watch.rs`:
...
mod tests {
    ...
    use ironlint_core::watch::{CheckRollup, LogSummary};
...
Run: `cargo test -p ironlint-cli watch::tests::tab_toggles_view`
Expected: FAIL — `View`/`ViewState`/`handle_key` undefined.

- [ ] **Step 3: Implement the state + key handler.**

Add to `crates/ironlint-cli/src/commands/watch.rs` (above the `#[cfg(test)]` block). Add the import at the top of the file alongside the others:
...
Run: `cargo test -p ironlint-cli watch::`
Expected: PASS (all 5 state tests). `cargo clippy -p ironlint-cli --all-targets -- -D warnings` + `cargo fmt` clean.
...
**Interfaces:**
- Consumes: `ironlint_core::telemetry::LogEntry`, `ironlint_core::watch::{short_time, fmt_elapsed, status_glyph}`, `ironlint_core::verdict::Status`.
...
Add to the `tests` module in `crates/ironlint-cli/src/commands/watch.rs`:
...
Run: `cargo test -p ironlint-cli watch::tests::stream_renders_pass_row_with_time_file_elapsed_event`
Expected: FAIL — `stream_lines` undefined.

- [ ] **Step 3: Implement `stream_lines` + color constants.**

Add to `crates/ironlint-cli/src/commands/watch.rs`. Add imports at the top:

```rust
use ironlint_core::telemetry::LogEntry;
use ironlint_core::verdict::Status;
use ironlint_core::watch::{fmt_elapsed, short_time, status_glyph};
...
Run: `cargo test -p ironlint-cli watch::`
Expected: PASS. `cargo clippy -p ironlint-cli --all-targets -- -D warnings` + `cargo fmt` clean. (`Modifier` import may be unused until Task 3.3 — if clippy flags it, move that import into Task 3.3's step instead.)
...
**Interfaces:**
- Consumes: `ironlint_core::watch::{LogSummary, lifecycle_badge, fmt_elapsed}`.
...
```rust
use ironlint_core::watch::{CheckRollup as Roll, LogSummary as Sum};

fn roll(name: &str, runs: usize, blocks: usize, internal: usize, p50: Option<u64>) -> Roll {
    Roll {
        name: name.into(),
        on: vec![ironlint_core::config::Lifecycle::Write],
        runs,
        blocks,
        internal,
        p50_ms: p50,
...
Run: `cargo test -p ironlint-cli watch::tests::explorer_summary_line_has_totals_and_pass_pct`
Expected: FAIL — `explorer_lines` undefined.

- [ ] **Step 3: Implement `explorer_lines`.**

Add to `crates/ironlint-cli/src/commands/watch.rs`. Update the `ironlint_core::watch` import to include the new helpers:

```rust
use ironlint_core::watch::{fmt_elapsed, lifecycle_badge, short_time, status_glyph, CheckRollup, LogSummary};
```
...
Run: `cargo test -p ironlint-cli watch::`
Expected: PASS. `cargo clippy -p ironlint-cli --all-targets -- -D warnings` + `cargo fmt` clean. (`short_time`/`status_glyph` are used by `stream_lines`; the shared import line keeps them in scope.)
...
Run: `cargo test -p ironlint-cli watch::tests::ui_stream_view_renders_feed_and_footer`
Expected: FAIL — `ui` undefined.

- [ ] **Step 3: Implement `ui`.**

Add to `crates/ironlint-cli/src/commands/watch.rs`. Add imports:
...
Run: `cargo test -p ironlint-cli watch::`
Expected: PASS (all watch tests, including the two TestBackend ones). `cargo clippy -p ironlint-cli --all-targets -- -D warnings` + `cargo fmt` clean.
...
**Interfaces:**
- Consumes: `ironlint_core::runner::IronLintEngine`, `ironlint_core::watch::{ArmedCheck, summarize}`, `ironlint_core::telemetry::read_all`.
...
fn load_armed_reads_check_names_and_lifecycles() {
    ...
    assert_eq!(armed[0].on, vec![ironlint_core::config::Lifecycle::Write, ironlint_core::config::Lifecycle::PreCommit]);
}
...
Add `tempfile` to `crates/ironlint-cli`'s `[dev-dependencies]` if not already present (it is — used by other e2e tests).

- [ ] **Step 2: Run to verify failure.**

Run: `cargo test -p ironlint-cli watch::tests::load_armed_reads_check_names_and_lifecycles`
...
use ironlint_core::watch::{summarize, ArmedCheck};
...
Run: `cargo test -p ironlint-cli watch::tests::load_armed_reads_check_names_and_lifecycles watch::tests::load_armed_is_empty_when_config_missing`
Expected: PASS.

- [ ] **Step 5: Implement the loop + replace the stub (no new test — verified manually).**

In `crates/ironlint-cli/src/commands/watch.rs`, replace the `// Phase 4 replaces this stub` line and `Ok(0)` in `run()` with:
...
fn event_loop<B: Backend>(terminal: &mut Terminal<B>, dir: &Path, armed: &[ArmedCheck]) -> Result<()> {
    ...
    loop {
        let entries = ironlint_core::telemetry::read_all(&log).unwrap_or_default();
...
# In a real terminal, from a repo that has a .ironlint.yml:
./target/debug/ironlint watch --dir .
```
Expected: the TUI renders (header tabs, body, footer); `Tab`/`→` switches Stream↔Explorer; `↑`/`↓` move the explorer selection; `q` exits and the terminal is restored (no leftover raw mode). If a `.ironlint/log.jsonl` exists, recent runs appear newest-first within ~250ms.
...
cargo fmt --check
cargo clean -p ironlint-cli   # drop the debug build artifact from manual verification
```
Expected: all green.
...
```bash
git add crates/ironlint-cli/src/commands/watch.rs
git commit -m "feat(cli): wire ironlint watch live event loop"
```
...
In `docs/reference/cli.md`, add a `ironlint watch` section after the existing command entries, in the established house format. Content to include:
...
git commit -m "docs: document ironlint watch"
```
