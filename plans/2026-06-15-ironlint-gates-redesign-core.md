# IronLint Gates Redesign — Plan 1: Core Engine Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace ironlint's `engine`/`script`/`ast` rule model with the minimal `gates:` format so that `ironlint check` runs a gate command per matching file and proxies exit code 2 as a Block.

**Architecture:** A gate is `files` (globs) + `run` (a shell string). IronLint matches touched files to gates, runs each `run` once per matching file with the ABI materialized as environment (`IRONLINT_FILE`, `IRONLINT_ROOT`, `IRONLINT_EVENT`) plus proposed content on stdin, and reads only the exit code: `2` → Block, `126/127/≥128/timeout` → InternalError, everything else → Pass. On Block the gate's stdout+stderr is the message. No string templating, no per-rule engines, no severity.

**Tech Stack:** Rust (Cargo workspace, crates `ironlint-core` + `ironlint-cli`), `serde`/`serde_yaml`/`serde_json`, `wait-timeout` (new) for process timeouts, `insta` for snapshots, `assert_cmd` for CLI e2e.

**Spec:** `specs/2026-06-15-ironlint-gates-redesign-design.md`. This plan implements §2–§5, §9–§11 (core engine + `check`). Trust (§6), verify/doctor (§7), and adapters (§4 adapter side) are Plans 2–4.
...
- `ironlint verify` / `doctor` upgrades (§7) — Plan 3.
...
**ironlint-core (`crates/ironlint-core/src/`):**
...
- `runner.rs` — rewritten core: `IronLintEngine { config, root, options }`, `check`, `gate_matches_path`, `check_with_explain`. Deletes engine-trait dispatch.
...
**ironlint-cli (`crates/ironlint-cli/src/`):**
...
**Tests:** `crates/ironlint-core/src/**` unit tests inline; `crates/ironlint-cli/tests/cli_e2e_gates.rs` new; fixtures under `tests/fixtures/gates/`.
...
**Files:**
- Modify: `crates/ironlint-core/src/config/types.rs` (full rewrite)

- [ ] **Step 1: Write the failing tests**

Replace the entire contents of `crates/ironlint-core/src/config/types.rs` test module with these (keep at top of file for now; implementation follows in Step 3):
...
Run: `cargo test -p ironlint-core config::types`
Expected: FAIL — `Config`/`Gate` no longer have the old shape (compile errors referencing removed fields).

- [ ] **Step 3: Write the implementation**

Replace everything *above* the test module in `crates/ironlint-core/src/config/types.rs` with:
...
/// `IRONLINT_TIMEOUT` env var overrides this at run time. `max_workers` tunes
...
/// The path under check arrives as `$IRONLINT_FILE`; proposed content arrives
...
```bash
git add crates/ironlint-core/src/config/types.rs
...
**Files:**
- Modify: `crates/ironlint-core/src/config/parser.rs` (full rewrite)
- Modify: `crates/ironlint-core/src/config/mod.rs` (re-exports)

- [ ] **Step 1: Write the failing tests**

Replace the test module in `crates/ironlint-core/src/config/parser.rs` with:
...
Run: `cargo build -p ironlint-core 2>&1 | head` — expect compile errors referencing removed `SUPPORTED_SCHEMAS`/`EngineKind`.

- [ ] **Step 3: Write the implementation**

Replace the non-test contents of `crates/ironlint-core/src/config/parser.rs` with:

```rust
use super::types::Config;
use anyhow::{anyhow, Context, Result};

/// Parse a `.ironlint.yml` (gates format).
///
    ...
    if let Some(key) = legacy_marker(input) {
        return Err(anyhow!(
            "this looks like a pre-0.3 config (found `{key}:`). The 0.3 format uses a \
             ...
             See specs/2026-06-15-ironlint-gates-redesign-design.md"
        ));
    }
...
Then update `crates/ironlint-core/src/config/mod.rs` re-exports:
...
```bash
git add crates/ironlint-core/src/config/parser.rs crates/ironlint-core/src/config/mod.rs
...
**Files:**
- Modify: `crates/ironlint-core/Cargo.toml` (add `wait-timeout`)
- Create: `crates/ironlint-core/src/engine/gate.rs`

- [ ] **Step 1: Add the dependency**

In `crates/ironlint-core/Cargo.toml` under `[dependencies]`, add:
...
Create `crates/ironlint-core/src/engine/gate.rs` with the test module first (implementation in Step 4):
...
Add `tempfile` to `[dev-dependencies]` in `crates/ironlint-core/Cargo.toml` if not already present (`tempfile = "3"`).

- [ ] **Step 3: Run tests to verify they fail**

Run: `cargo test -p ironlint-core engine::gate 2>&1 | head`
Expected: FAIL — `run_gate`/`GateOutcome` undefined.

- [ ] **Step 4: Write the implementation**

Prepend to `crates/ironlint-core/src/engine/gate.rs` (above the test module):
...
pub struct GateEnv<'a> {
    /// Absolute path to the file under check (`$IRONLINT_FILE`).
    pub file: &'a Path,
    /// Project root; also the gate's cwd (`$IRONLINT_ROOT`).
    pub root: &'a Path,
    /// Trigger: `edit` | `write` | `pre-commit` | `manual` (`$IRONLINT_EVENT`).
    pub event: &'a str,
}
...
Run: `cargo test -p ironlint-core engine::gate`
Expected: PASS (8 tests).

- [ ] **Step 6: Commit**

```bash
git add crates/ironlint-core/Cargo.toml crates/ironlint-core/src/engine/gate.rs
...
**Files:**
- Modify: `crates/ironlint-core/src/engine/mod.rs`
- Delete: `crates/ironlint-core/src/engine/{ast,output,capability,script}.rs`
- Modify: `crates/ironlint-core/Cargo.toml` (drop `ast-grep-core` / related)
...
```bash
git rm crates/ironlint-core/src/engine/ast.rs \
       crates/ironlint-core/src/engine/output.rs \
       crates/ironlint-core/src/engine/capability.rs \
       crates/ironlint-core/src/engine/script.rs
```

- [ ] **Step 3: Drop unused dependencies**

In `crates/ironlint-core/Cargo.toml`, remove any dependency only used by the deleted engines (`ast-grep-core`, `ast-grep-language`, and any AST-only transitive crates). Leave `sha2`/`serde*`/`globset`/`rayon`/`fs4` — still used.

Verify nothing else references them:

```bash
grep -rn "ast_grep\|ast-grep\|capability\|engine::output\|engine::script\|engine::ast" crates/ironlint-core/src crates/ironlint-cli/src
```
...
```bash
git add -A crates/ironlint-core/src/engine crates/ironlint-core/Cargo.toml
...
**Files:**
- Modify: `crates/ironlint-core/src/verdict.rs` (full rewrite)

- [ ] **Step 1: Write the failing tests**

Replace the test module (or add one) in `crates/ironlint-core/src/verdict.rs`:
...
Run: `cargo build -p ironlint-core 2>&1 | head` — expect errors (old `Violation`/`Severity`/`Engine` referenced elsewhere; `from_outcomes` undefined).

- [ ] **Step 3: Write the implementation**

Replace the non-test contents of `crates/ironlint-core/src/verdict.rs` with:
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs` (core rewrite)
...
- [ ] **Step 2: Run** `cargo build -p ironlint-core 2>&1 | head` — expect errors from the old dispatch path.
...
2. Change the `IronLintEngine` struct to hold the parsed config + root + options (keep whatever fields the kept path-canonicalization code needs):

```rust
pub struct IronLintEngine {
    config: crate::config::Config,
    /// Directory containing the config file; gate cwd + IRONLINT_ROOT.
...
3. In `load` (and the builder's `load`), after `let config = crate::config::parse_file_with_extends(config_path)?;`:
   ...
   - Compute `timeout` from `IRONLINT_TIMEOUT` env (parse u64 secs) else `config.execution.timeout_secs`, as `Duration::from_secs`.
...
pub fn check(&self, input: CheckInput) -> Result<Verdict> {
    ...
    for (id, gate) in &self.config.gates {
        ...
        // ironlint-disable: <gate-id> directive in the proposed content.
...
Run: `cargo test -p ironlint-core gate_dispatch_tests` (after Tasks 7–8 land the rest of the references, the whole crate compiles; if compile still blocked by telemetry/disable, finish Task 8 then run).
...
- [ ] **Step 2: Run** `cargo test -p ironlint-core explain_reports_per_gate_outcome` → FAIL (field renamed `rule_id`→`gate_id`, struct shapes).
...
- [ ] **Step 4: Run** `cargo test -p ironlint-core explain_reports_per_gate_outcome` → PASS.
...
- Modify: `crates/ironlint-core/src/disable.rs`
- Modify: `crates/ironlint-core/src/runner.rs` (emit telemetry from `check`)
...
fn disables_named_gate() {
    assert!(is_disabled("code // ironlint-disable: no-todo\n", "no-todo"));
    assert!(!is_disabled("code // ironlint-disable: other\n", "no-todo"));
}
```

- [ ] **Step 2: Run** `cargo build -p ironlint-core 2>&1 | head` → errors (PerRuleRecord/Engine references; LogEntry.rules field).
...
`runner.rs`:
- After building the verdict in `check`, append a `LogEntry::Check` to `<root>/.ironlint/log.jsonl` with one `PerGateRecord` per gate that ran. Timestamp: use `chrono::Utc::now().to_rfc3339()` if `chrono` is already a dep; otherwise format from `std::time::SystemTime`. (Check existing deps — the old runner already wrote telemetry, so the timestamp source already exists; reuse it.)

- [ ] **Step 4: Run** `cargo test -p ironlint-core telemetry disable` → PASS.

- [ ] **Step 5: Commit**

```bash
git add crates/ironlint-core/src/telemetry.rs crates/ironlint-core/src/disable.rs crates/ironlint-core/src/runner.rs
...
**Files:**
- Delete: `crates/ironlint-core/src/baseline.rs`
- Modify: `crates/ironlint-core/src/lib.rs`
- Modify: `crates/ironlint-core/src/runner.rs` (remove baseline-filter step if present)

- [ ] **Step 1: Remove baseline**

```bash
git rm crates/ironlint-core/src/baseline.rs
```
...
```bash
git add -A crates/ironlint-core
...
- Modify: `crates/ironlint-cli/src/commands/check.rs` (rewrite)
- Delete: `crates/ironlint-cli/src/commands/{migrate,baseline}.rs`
- Modify: `crates/ironlint-cli/src/commands/mod.rs`
- Modify: `crates/ironlint-cli/src/commands/{validate,explain,show_resolved_config,guide}.rs` (compile fixes)
...
```rust
/// What triggered this check, surfaced to gates as $IRONLINT_EVENT.
...
use ironlint_core::runner::{CheckInput, CheckOptions, ExplainOutcome, GateExplain, IronLintEngine};
...
fn run_diff(engine: &IronLintEngine, diff: &Path, format: OutputFormat, explain: bool) -> Result<i32> {
...
fn validate_gate_filter(engine: &IronLintEngine, gates: &[String]) -> Option<i32> {
    if gates.is_empty() {
        return None;
    }
...
```bash
git rm crates/ironlint-cli/src/commands/migrate.rs crates/ironlint-cli/src/commands/baseline.rs
```
...
**Files:**
- Create: `crates/ironlint-cli/tests/cli_e2e_gates.rs`
- Create: `tests/fixtures/gates/.ironlint.yml` and sample files
...
```bash
ls crates/ironlint-cli/tests
git rm crates/ironlint-cli/tests/cli_e2e_script_rules.rs   # and any ast/semantic/baseline/migrate e2e files
```
...
fn cfg(dir: &std::path::Path, body: &str) {
    std::fs::write(dir.join(".ironlint.yml"), body).unwrap();
}
...
fn exit_2_gate_blocks_and_exits_2() {
    ...
    Command::cargo_bin("ironlint")
        ...
        .args(["check", "--file", file.to_str().unwrap(), "--config", ".ironlint.yml"])
        .assert()
        .code(2);
}
...
Add `assert_cmd` and `tempfile` to `crates/ironlint-cli/Cargo.toml` `[dev-dependencies]` if not present.
...
```bash
git add -A crates/ironlint-cli/tests tests/fixtures
