# IronLint H1 — `ironlint check --emit-semantic-payload`
...
**Goal:** Teach the `ironlint check` CLI to run deterministic engines (script + ast), collect every `engine: semantic` and `engine: session` rule that would have dispatched, and emit a single `DeferredVerdict` JSON envelope wrapping the bully-shaped payload Claude Code's `additionalContext` consumes. Activated either by a new `llm.provider: claude-code-subagent` value in `.ironlint.yml` or by the long-only `--emit-semantic-payload` flag. The flag is the user-invisible internal switch the adapter uses in tests and scripted invocations; the config value is what end-users set. Direct-API users (anthropic / openrouter / ollama / API-key paths) are untouched.

**Architecture:** Add a `DeferredVerdict` type next to `Verdict` (new module `crates/ironlint-core/src/verdict_deferred.rs`, additive, never replaces the locked `Verdict` shape). Add a `emit_semantic_payload: bool` field on `CheckOptions` and route it through `IronLintEngine::check_with_explain`: when active, the runner short-circuits `engine: semantic` and `engine: session` rules into a `Vec<DeferredRule>` instead of dispatching them, then `CheckReport` gains a `Option<DeferredVerdict>` field that the CLI inspects. The runner change is local to two dispatch arms in `runner.rs`; the rest of the pipeline (script, ast, baseline, telemetry of deterministic outcomes, exit-code computation for deterministic blocks) is untouched. The CLI computes the final exit code by inspecting both the deterministic verdict and whether a deferred envelope is present: deterministic block → exit 2, deterministic pass + deferred → emit `DeferredVerdict` and exit 0, deterministic pass + no deferred → emit standard `Verdict` and exit 0. `--emit-semantic-payload` is mutually exclusive with `--session` (clap-enforced); `--print-prompt` and `--explain` continue to short-circuit before the deferred path runs.
...
|---|---|---|
| Mode selection | New value `provider: claude-code-subagent` in `.ironlint.yml`'s `llm:` block. The new `--emit-semantic-payload` long flag forces the deferred path regardless of provider (used by tests and explicit adapter invocations). | Spec §4 — explicit, visible in config diffs, covered by the trust fingerprint. |
...
| `ironlint_version` field | Stamped via `env!("CARGO_PKG_VERSION")` to match `Verdict` and `LogEntry::SessionInit`. | Existing convention. |
...
| Telemetry on deferred path | Deterministic engines still emit their usual telemetry. **No** `SemanticVerdict` records are written by the runner in deferred mode — those are emitted later by the skill via `ironlint record-verdict` (H2). | Spec §H1. Avoids double-recording. |
...
**Files:**
- Create: `crates/ironlint-core/tests/llm_provider_subagent.rs`
...
use ironlint_core::config::LlmConfig;
use ironlint_core::llm::build_from_config;
...
Run: `cargo test -p ironlint-core --test llm_provider_subagent`
Expected: FAIL — `unknown LLM provider 'claude-code-subagent'` from the existing `bail!` arm.

### Task 1.2: Implement the arm

**Files:**
- Modify: `crates/ironlint-core/src/llm/mod.rs` (the `match cfg.provider.as_str()` block in `build_from_config`; the closing `other =>` bail arm needs its supported-providers list updated)
...
```bash
git add crates/ironlint-core/src/llm/mod.rs crates/ironlint-core/tests/llm_provider_subagent.rs
...
The deferred payload's `_evaluator_input` field is a single string the subagent reads to know what rules to evaluate and which file/diff to evaluate them against. Bully assembles this as a system + user concatenation; IronLint already has `build_prompt_split` producing the same `(system, user)` tuple for Anthropic's split-role API. This phase adds a thin wrapper that concatenates the tuple into one evaluator-input string — same content the model would have received in the direct-API path, written into the payload instead.

### Task 2.1: Failing test — byte-identical to `build_prompt_split` concatenated

**Files:**
- Modify: `crates/ironlint-core/src/llm/prompt.rs` (extend the existing `#[cfg(test)] mod tests` block at the bottom of the file)
...
Run: `cargo test -p ironlint-core --lib llm::prompt::tests::build_evaluator_input`
Expected: FAIL — `cannot find function build_evaluator_input in this scope`.

### Task 2.2: Implement `build_evaluator_input`

**Files:**
- Modify: `crates/ironlint-core/src/llm/prompt.rs` (add a new `pub fn` next to `build_prompt_split`)
...
```bash
git add crates/ironlint-core/src/llm/prompt.rs
...
**Files:**
- Create: `crates/ironlint-core/tests/deferred_verdict_shape.rs`
...
use ironlint_core::verdict_deferred::{
...
fn empty_deferred_verdict_serializes_to_canonical_shape() {
    let v = DeferredVerdict {
        ...
        deferred: true,
        ironlint_version: "0.2.x".to_string(),
        passed_checks: vec![],
...
Run: `cargo test -p ironlint-core --test deferred_verdict_shape`
Expected: FAIL — `unresolved import ironlint_core::verdict_deferred`.

### Task 3.2: Create the module

**Files:**
- Create: `crates/ironlint-core/src/verdict_deferred.rs`
- Modify: `crates/ironlint-core/src/lib.rs` (add `pub mod verdict_deferred;`)

- [ ] **Step 3.2.1: Add the module declaration**

In `crates/ironlint-core/src/lib.rs`, alongside the existing `pub mod verdict;`:
...
```bash
git add crates/ironlint-core/src/lib.rs crates/ironlint-core/src/verdict_deferred.rs crates/ironlint-core/tests/deferred_verdict_shape.rs
...
This phase teaches `IronLintEngine` to short-circuit `Semantic` and `Session` dispatches when deferred mode is active, returning the would-have-been-evaluated rules in a new `CheckReport::deferred` field that the CLI inspects.

### Task 4.1: Failing test — deferred mode collects rules

**Files:**
- Create: `crates/ironlint-core/tests/runner_deferred_mode.rs`
...
fn write_trusted_config(dir: &std::path::Path) -> std::path::PathBuf {
    ...
    // Compute and rewrite the trust fingerprint so IronLintEngine::load accepts it.
    let yaml = fs::read_to_string(&path).unwrap();
    let new = ironlint_core::trust::write_trust_block(&yaml).unwrap();
...
fn deferred_mode_collects_semantic_rule() {
    ...
    assert!(
        report
            ...
            .violations
            .iter()
            .all(|v| v.engine != ironlint_core::verdict::Engine::Semantic),
...
Run: `cargo test -p ironlint-core --test runner_deferred_mode`
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs` (the `CheckOptions` struct around line 28, the `CheckReport` struct around line 61)

- [ ] **Step 4.2.1: Extend `CheckOptions`**

Locate the struct (`grep -n 'pub struct CheckOptions' crates/ironlint-core/src/runner.rs`) and add the field with serde defaults so older library callers continue compiling:
...
Update every internal `CheckReport { … }` constructor in the file to include `deferred: None`. There are 2-3 in `runner.rs`; find them with `grep -n 'CheckReport {' crates/ironlint-core/src/runner.rs`.

### Task 4.3: Implement the deferred dispatch

**Files:**
- Modify: `crates/ironlint-core/src/runner.rs` (the two dispatch arms in `dispatch_one_rule` that handle `EngineKind::Semantic` and `EngineKind::Session`, around lines 568-580 per the earlier grep)
...
Locate `check_with_explain` (`grep -n 'pub fn check_with_explain' crates/ironlint-core/src/runner.rs`). Before the rule-iteration loop, declare:
...
You will need a `config_rule(&str) -> Option<&Rule>` accessor on `IronLintEngine`. If one doesn't exist, add it:
...
(Adjust the field path if `config.rules` is structured differently — verify with `grep -n 'pub struct Config' crates/ironlint-core/src/config/types.rs`.)
...
```bash
git add crates/ironlint-core/src/runner.rs crates/ironlint-core/tests/runner_deferred_mode.rs
...
**Files:**
- Create: `crates/ironlint-cli/tests/cli_e2e_emit_semantic_payload.rs`

- [ ] **Step 5.1.1: Write the failing test**

```rust
//! H1 — end-to-end coverage that `ironlint check --emit-semantic-payload`
...
fn flag_emits_deferred_verdict_envelope() {
    ...
    let out = Command::cargo_bin("ironlint")
        .unwrap()
        .arg("check")
        .arg("--config").arg(tmp.path().join(".ironlint.yml"))
        .arg("--file").arg(&src)
...
fn flag_omitted_means_no_envelope() {
    ...
    let path = tmp.path().join(".ironlint.yml");
...
Run: `cargo test -p ironlint-cli --test cli_e2e_emit_semantic_payload`
Expected: FAIL — clap rejects the unknown `--emit-semantic-payload` flag.

### Task 5.2: Add the clap flag

**Files:**
- Modify: `crates/ironlint-cli/src/cli.rs` (the `Command::Check { … }` variant)
...
Modify `crates/ironlint-cli/src/main.rs` to pass the new arg into `commands::check::run`. Add the parameter to `check::run`'s signature and to its `CheckOptions` construction.

In `crates/ironlint-cli/src/commands/check.rs`:
...
**Files:**
- Modify: `crates/ironlint-cli/src/commands/check.rs` (the `(Some(f), None)` and `(None, Some(d))` arms of the input match)
...
```rust
fn emit_deferred(d: &ironlint_core::verdict_deferred::DeferredVerdict, format: OutputFormat) -> Result<()> {
    match format {
...
```bash
git add crates/ironlint-cli/src/cli.rs crates/ironlint-cli/src/main.rs crates/ironlint-cli/src/commands/check.rs crates/ironlint-cli/tests/cli_e2e_emit_semantic_payload.rs
git commit -m "feat(cli): ironlint check --emit-semantic-payload (H1 phase 5)
...
**Files:**
- Modify: `crates/ironlint-cli/tests/cli_e2e_emit_semantic_payload.rs` (append a new test)
...
Run: `cargo test -p ironlint-cli --test cli_e2e_emit_semantic_payload deterministic_block_suppresses_deferred_envelope`
Expected: PASS.

- [ ] **Step 6.1.3: Commit**

```bash
git add crates/ironlint-cli/tests/cli_e2e_emit_semantic_payload.rs
...
```markdown
# `ironlint check --emit-semantic-payload`
...
Activated by either:
- `llm.provider: claude-code-subagent` in `.ironlint.yml` (end-user-facing),
...
**Files:**
- Move: `plans/2026-05-14-ironlint-h1-emit-semantic-payload.md` → `plans/archive/`
- Modify: `plans/README.md` (move H1 from the Future section to Archive)

- [ ] **Step 7.3.1: Move the plan**

```bash
git mv plans/2026-05-14-ironlint-h1-emit-semantic-payload.md plans/archive/
```
