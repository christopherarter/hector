# IronLint B1 — Parallel Rule Execution
...
|---|---|---|
| Where parallelism lives | **`IronLintEngine::check`** per-file path only. `check_session` stays serial. | Session has one LLM call total — no parallelism payoff. Keeps the scope tight and the diff small. |
...
| Pool sizing precedence | `IRONLINT_MAX_WORKERS` env → `config.execution.max_workers` → `min(8, num_cpus::get())` | Mirrors bully. Env beats config so operators can override without editing YAML. |
...
### Task 1: Add `rayon` and `num_cpus` to `ironlint-core`

**Files:**
- Modify: `crates/ironlint-core/Cargo.toml:8-23`

- [ ] **Step 1: Add the deps**

Edit `crates/ironlint-core/Cargo.toml`. Under `[dependencies]`, after the existing entries, add:
...
```bash
git add crates/ironlint-core/Cargo.toml
...
**Files:**
- Create: `crates/ironlint-core/tests/runner_parallel.rs`
- Create: `crates/ironlint-core/tests/runner_parallel_order.rs`
...
use ironlint_core::llm::{LlmClient, RuleVerdict};
    ...
    fn evaluate(
        ...
        Ok(rules
            ...
            .map(|(id, _)| RuleVerdict {
                ...
                status: ironlint_core::llm::RuleStatus::Pass,
            })
            .collect())
    }
}
...
impl LlmClient for OrderingLlm {
    fn evaluate(
        ...
        Ok(vec![RuleVerdict {
            rule_id: id.to_string(),
            status: ironlint_core::llm::RuleStatus::Violation {
...
fn ordering_engine(cfg_dir: &std::path::Path) -> (IronLintEngine, std::path::PathBuf) {
...
#[test]
fn ironlint_max_workers_env_clamps_pool_size() {
    ...
    // assert behavioral parity: a `IRONLINT_MAX_WORKERS=1` run still returns
    ...
    // read IRONLINT_MAX_WORKERS.
    let prev = std::env::var("IRONLINT_MAX_WORKERS").ok();
    // SAFETY: tests in this file are scoped so no other test reads
    // IRONLINT_MAX_WORKERS concurrently.
    unsafe { std::env::set_var("IRONLINT_MAX_WORKERS", "1"); }
    ...
    match prev {
        Some(v) => unsafe { std::env::set_var("IRONLINT_MAX_WORKERS", v) },
        None => unsafe { std::env::remove_var("IRONLINT_MAX_WORKERS") },
    }
...
#[test]
fn ironlint_max_workers_zero_value_clamps_to_one_not_deadlock() {
    let dir = tempdir().unwrap();
    let (engine, file) = ordering_engine(dir.path());

    let prev = std::env::var("IRONLINT_MAX_WORKERS").ok();
    unsafe { std::env::set_var("IRONLINT_MAX_WORKERS", "0"); }
...
```bash
git add crates/ironlint-core/tests/runner_parallel.rs \
        crates/ironlint-core/tests/runner_parallel_order.rs
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs:198-277`

- [ ] **Step 1: Define `RuleOutcome` and helper inside `impl IronLintEngine`**

Open `crates/ironlint-core/src/runner.rs`. Just before `pub fn check(&self, …)`, add:
...
Inside `impl IronLintEngine`, somewhere before `check()`:

```rust
    /// Build a rayon thread pool sized by the precedence:
    /// `IRONLINT_MAX_WORKERS` env → `config.execution.max_workers` → `min(8, num_cpus::get())`.
    ...
    fn execution_pool(&self) -> rayon::ThreadPool {
        let env = std::env::var("IRONLINT_MAX_WORKERS")
            .ok()
...
**Files:**
- Modify: `crates/ironlint-core/src/config/types.rs:4-16`
- Modify: `crates/ironlint-core/tests/parse_v2.rs` (one new test)

- [ ] **Step 1: Add the struct**

Edit `crates/ironlint-core/src/config/types.rs`. Add to the `Config` struct:
...
/// rules in parallel during `IronLintEngine::check`. Absence = use the default
/// of `min(8, num_cpus::get())`. The `IRONLINT_MAX_WORKERS` env var overrides
/// any value set here.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
Append to `crates/ironlint-core/tests/parse_v2.rs`:
...
rules:
  r1:
    ...
    language: rust
"#;
    let cfg = ironlint_core::config::parse_str(yaml).expect("parse");
...
Run: `cargo test -p ironlint-core --test parse_v2`
...
git add crates/ironlint-core/src/config/types.rs \
        crates/ironlint-core/tests/parse_v2.rs
git commit -m "feat(config): execution.max_workers + IRONLINT_MAX_WORKERS override (B1 phase 4)"
```

(The `IRONLINT_MAX_WORKERS` plumbing lands in Phase 3's runner edit, but the public surface — the config key — is what this commit ratifies.)
...
- [ ] `execution.max_workers` parses and clamps to ≥1 → covered by `parse_v2::parses_execution_max_workers` + `runner_parallel_order::ironlint_max_workers_zero_value_clamps_to_one_not_deadlock`.
...
| `tests/runner_parallel_order.rs` | Determinism (output order = input BTreeMap order, not completion order); `IRONLINT_MAX_WORKERS=1` doesn't break behavior; `IRONLINT_MAX_WORKERS=0` clamps to 1 instead of deadlocking. |
...
| Env-var test races (`std::env::set_var` is process-global) | low — these are the only tests touching `IRONLINT_MAX_WORKERS` | high (cascading flakes) | Save/restore around the test body. If flake observed, add `serial_test` dep. |
...
- [ ] Env-override tests save and restore `IRONLINT_MAX_WORKERS`.
...
- C1 `ironlint doctor` will surface effective pool size via the diagnostic output.
