# IronLint E1 — Baseline Line-Content Checksum Implementation Plan
...
A new CLI subcommand `ironlint baseline refresh` re-reads every baselined `(rule_id, file, line)` and updates the stored checksum to the current line content. Idempotent.

**Tech Stack:** Rust, workspace-stable. `sha2` already a direct dep on `ironlint-core` (used by `trust.rs`). No new deps.
...
| CLI shape | `ironlint baseline refresh` as a subcommand of `Baseline`. | A bare flag (`--refresh`) muddles the existing record-mode behavior. A subcommand is idiomatic clap. Existing `ironlint baseline` retains its current record semantics. |
...
### Task 1: Append failing tests to `crates/ironlint-core/tests/baseline.rs`

**Files:**
- Modify: `crates/ironlint-core/tests/baseline.rs`
...
fn legacy_baseline_without_checksum_loads_with_warning() {
    ...
    let path = dir.path().join(".ironlint").join("baseline.json");
...
```bash
git add crates/ironlint-core/tests/baseline.rs
...
**Files:**
- Modify: `crates/ironlint-core/src/baseline.rs`
...
/// treated as "always match" (matches v1 behavior). Run `ironlint baseline
...
impl Baseline {
    ...
    fn emit_legacy_warning(path: &Path) {
        if LEGACY_WARNING_EMITTED.set(()).is_ok() {
            eprintln!(
                "ironlint: warning — baseline at {} uses the legacy v1 format \
                 (no line_sha256). Run `ironlint baseline refresh` to upgrade. \
            ...
            match Self::file_and_line_from_fingerprint(key) {
                Some((file_rel, Some(line))) => {
                    ...
                    match content.as_deref().and_then(|c| Self::line_at(c, line)) {
                        ...
                        None => {
                            eprintln!(
                                "ironlint: refresh — dropping baseline entry {key}: \
...
- `LEGACY_WARNING_EMITTED` is process-global. The CLI process boundary is the unit; `ironlint check` followed by `ironlint baseline refresh` would each warn once if both encountered a v1 file.
...
- [x] **Step 3: Run the full ironlint-core suite**
...
```bash
git add crates/ironlint-core/src/baseline.rs crates/ironlint-core/tests/baseline.rs
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs` (post-loop baseline-filter block only)

The current code (around line 285–301):

```rust
let baseline_path = self.config_dir.join(".ironlint/baseline.json");
...
Edit `crates/ironlint-core/src/runner.rs:301`:
...
In `crates/ironlint-cli/src/commands/baseline.rs`, where the loop calls `bl.add(&v)`, change to `bl.add_with_content(&v, Some(&content))` — the same `content` already read from disk on the line above. This is what gives newly-recorded entries a real checksum.
...
```bash
git add crates/ironlint-core/src/runner.rs crates/ironlint-cli/src/commands/baseline.rs
...
**Files:**
- Create: `crates/ironlint-core/tests/fixtures/baseline_v1.json`
- Create: `crates/ironlint-core/tests/baseline_legacy.rs`
...
```bash
git add crates/ironlint-core/tests/fixtures/baseline_v1.json crates/ironlint-core/tests/baseline_legacy.rs
git commit -m "feat(baseline): backwards-compat read for old format (E1 phase 3)"
```

---

## Phase 5 — `ironlint baseline refresh` subcommand
...
- Modify: `crates/ironlint-cli/src/commands/baseline.rs`
- Create: `crates/ironlint-cli/tests/cli_baseline_refresh.rs`
...
Baseline {
    ...
    #[arg(long, default_value = ".ironlint.yml", global = true)]
    config: PathBuf,
...
pub enum BaselineAction {
    /// Record current violations to .ironlint/baseline.json.
    Record,
...
`crates/ironlint-cli/tests/cli_baseline_refresh.rs`:
...
fn refresh_updates_checksums_to_current_content() {
    ...
    let baseline_path = root.join(".ironlint/baseline.json");
...
Run: `cargo test -p ironlint-cli --test cli_baseline_refresh`
...
```bash
git add crates/ironlint-cli/src/cli.rs crates/ironlint-cli/src/main.rs \
        crates/ironlint-cli/src/commands/baseline.rs \
        crates/ironlint-cli/tests/cli_baseline_refresh.rs
git commit -m "feat(cli): ironlint baseline refresh subcommand (E1 phase 4)"
```
...
| `refresh_updates_checksums_to_current_content` | `crates/ironlint-cli/tests/cli_baseline_refresh.rs` | AC4: CLI surface roundtrip |
| `refresh_with_no_baseline_succeeds_silently` | `crates/ironlint-cli/tests/cli_baseline_refresh.rs` | No-op safety |
...
|---|---|---|---|
| Old ironlint reading a new v2 baseline file | medium (downgrade scenario) | high (silent parse failure → empty baseline → noise) | Out of scope for this phase. Older ironlint's `serde_json::from_str` will fail; the runner already warns to stderr on parse failure (P2-6 path). Downgrade is an explicit user action, and the warning surfaces the cause. |
| Legacy warning surfaces under benign first-load conditions | low | low | Warning fires once per process via `OnceLock`. Disappears after first `ironlint baseline refresh`. |
...
**Forward-compat for downgrade:** an older ironlint binary (pre-E1) reading a v2 baseline will fail to parse and fall through the P2-6 warning to an empty baseline. We accept the noise as the price of the schema bump. If we wanted full forward-compat we'd dual-write both shapes — explicitly out of scope.
...
- [x] CLI default behavior (no subcommand) still records, preserving the existing `ironlint baseline` invocation.
