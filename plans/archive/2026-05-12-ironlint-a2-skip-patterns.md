# IronLint A2 — Built-in Skip Patterns + Project `skip:` + `~/.ironlint-ignore`
...
**Sequencing:** Second item in the 0.2.0 cohort (after [A1](2026-05-12-ironlint-a1-prompt-injection.md)).

---

**Goal:** Stop running rules — especially expensive `engine: semantic` LLM rules — against lockfiles, generated code, and build artifacts. Default the matcher to bully's well-known list (lockfiles, minified, dist/build/__pycache__/node_modules, generated markers); let users add to it via top-level `skip:` in `.ironlint.yml` or via `~/.ironlint-ignore`.

**Architecture:** A new `crates/ironlint-core/src/config/skip.rs` module exports `built_in_skip_globs() -> &'static [&'static str]` and a `SkipMatcher` wrapping a `globset::GlobSet`. The matcher follows the same right-anchored convention as `ScopeMatcher` (bare `*.lock` matches at any depth). `Config` gains an additive `skip: Vec<String>` field; `extends:` resolution merges inherited `skip:` lists. `IronLintEngine::load_with` builds the effective skip list (built-ins ∪ ~/.ironlint-ignore ∪ project ∪ extends) once and stashes a `SkipMatcher` on the engine. `IronLintEngine::check` consults the matcher first thing — on match, the function returns `Verdict::pass()` with empty violations/passed, writes a `kind: "skipped"` telemetry record, and never enters the rule loop. Per the spec's open question, no new `Status::Skipped` enum variant is added in 0.2 — fold into `Pass`. Reconsider before the 0.3 verdict freeze.

**Tech Stack:** Rust, `globset` (already a workspace dep, used by `ScopeMatcher`), no new deps. Tests use `tempfile` (already a dev-dep) for `~/.ironlint-ignore` simulation via `HOME` env override.
...
| `~/.ironlint-ignore` toggle | Always honored if present; absent is silent no-op | Spec default. No CLI flag to disable for 0.2 (yagni). |
...
CLI test for the self-check (running `ironlint check --file Cargo.lock` against ironlint itself) goes in `crates/ironlint-cli/tests/cli_check.rs`.
...
**Files:**
- Create: `crates/ironlint-core/src/config/skip.rs`
...
mod tests {
    ...
    fn matcher_skips_cargo_lock_in_subdir() {
        let m = SkipMatcher::with_built_ins(&[]).unwrap();
        assert!(m.matches(Path::new("crates/ironlint-core/Cargo.lock")));
    }
    ...
    fn matcher_does_not_skip_normal_source() {
        ...
        assert!(!m.matches(Path::new("crates/ironlint-core/src/runner.rs")));
...
```rust
// In crates/ironlint-core/src/config/mod.rs, add to the existing module list:
pub mod skip;
```

- [x] **Step 3: Run the failing tests**

Run: `cargo test -p ironlint-core --lib config::skip`
...
**Files:**
- Modify: `crates/ironlint-core/src/config/skip.rs` (prepend implementation above the test module)
...
//! short-circuit `IronLintEngine::check` — no rules evaluated, no LLM dispatched.
...
pub const USER_GLOBAL_IGNORE_FILENAME: &str = ".ironlint-ignore";
...
/// Parse the contents of `~/.ironlint-ignore` into a list of globs.
...
**Files:**
- Modify: `crates/ironlint-core/src/config/types.rs` — add `skip` field
- Modify: `crates/ironlint-core/src/config/extends.rs` — union inherited `skip:` into local

- [x] **Step 1: Add the field to `Config`**

In `crates/ironlint-core/src/config/types.rs`, modify the `Config` struct:
...
In `crates/ironlint-core/src/config/extends.rs`, modify `merge_inherited`:
...
Add a new test to `crates/ironlint-core/tests/extends.rs` (or follow the pattern there). First peek at the file to find the right helper-style:

```rust
// At end of crates/ironlint-core/tests/extends.rs:
...
- [x] **Step 5: Run the full ironlint-core test sweep**
...
## Phase 3 — Wire `~/.ironlint-ignore` into `IronLintEngine::load`

### Task 4: Build the effective skip list at load time

**Files:**
- Modify: `crates/ironlint-core/src/runner.rs` — load `~/.ironlint-ignore`, build `SkipMatcher`, store on `IronLintEngine`

- [x] **Step 1: Add `skip: SkipMatcher` field to `IronLintEngine`**
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs` — `IronLintEngine::check`

- [x] **Step 1: Add the short-circuit at the top of `check`**

In `crates/ironlint-core/src/runner.rs::check`, after the path/content/diff destructure but before constructing `disable_map`:
...
Run: `cargo test -p ironlint-core --test runner`
...
**Files:**
- Create: `crates/ironlint-core/tests/runner_skip.rs`

- [x] **Step 1: Write the test**

```rust
//! A2 — skip-pattern short-circuit at the top of IronLintEngine::check.
...
fn write_trusted(dir: &std::path::Path, body: &str) -> std::path::PathBuf {
    ...
    let trusted =
        ironlint_core::trust::write_trust_block(&fs::read_to_string(&path).unwrap()).unwrap();
...
fn cargo_lock_is_skipped_with_default_config() {
    ...
    let engine = IronLintEngine::load(&cfg).expect("load");
    ...
    let verdict = engine
        ...
        .expect("check");
    assert_eq!(verdict.status, ironlint_core::verdict::Status::Pass);
...
fn user_global_ignore_is_honored() {
    ...
    fs::write(fake_home.join(".ironlint-ignore"), "*.special\n").unwrap();
...
Run: `cargo test -p ironlint-core --test runner_skip`
...
**Files:**
- Modify: `crates/ironlint-cli/tests/cli_check.rs` — add a test pointing at the workspace's actual `Cargo.lock`
...
Run: `cargo test -p ironlint-cli --test cli_check check_skips_cargo_lock_with_default_config`
Expected: pass.
...
- All existing `ironlint-core` tests still pass.
- All existing `ironlint-cli` tests still pass.
- The new `runner_skip` and CLI Cargo.lock tests pass.

- [x] **Step 4: Live self-check**

Run from the ironlint workspace root:
```bash
cargo build --release
./target/release/ironlint init   # if not already initialized
./target/release/ironlint trust
./target/release/ironlint check --file Cargo.lock --format json | jq .status
```
Expected: `"pass"`. (And no LLM call attempted, no rule evaluated.) If `ironlint init` doesn't exist or doesn't write a config, just hand-write a minimal `.ironlint.yml` with one rule that scopes `*.lock`.
...
|---|---|---|
| Unit (skip module) | `crates/ironlint-core/src/config/skip.rs` (`#[cfg(test)] mod tests`) | Built-in lists are complete; matcher honors right-anchored bare globs and recursive `dir/**` patterns; user globs are additive; `parse_user_global_ignore` strips blanks and `#` comments. |
| Unit (extends merge) | `crates/ironlint-core/tests/extends.rs` | Inherited `skip:` lists are unioned with local. |
| Integration (runner) | `crates/ironlint-core/tests/runner_skip.rs` | `Cargo.lock` short-circuits; project `skip:` honored; `~/.ironlint-ignore` honored. Skipped verdict has empty violations + empty passed_checks. Telemetry record written with `kind: "skipped"`. |
| CLI smoke | `crates/ironlint-cli/tests/cli_check.rs` | `ironlint check --file Cargo.lock` exits 0 with `status: "pass"` even when a script rule is scoped to `*.lock` (proves the rule never ran). |
| Manual self-check | (live) | `./target/release/ironlint check --file Cargo.lock` returns `pass` against the ironlint workspace itself. |
...
- **Config-schema impact:** additive — new top-level `skip: Vec<String>` (default empty). Configs without `skip:` are byte-identical to before; their trust fingerprint is unchanged. Configs that add `skip:` will need `ironlint trust` to re-acknowledge.
...
- **Performance impact:** at `IronLintEngine::load`, one filesystem read of `~/.ironlint-ignore` (silently skipped if absent), one `GlobSet::build` over ~20 patterns. Per-`check` call: one `GlobSet::is_match` against the file path before any rule iteration. Net: `check` on skipped files is dramatically *faster* (no rules, no LLM).
- **Behavioral risk:** A user with a project rule scoped to `*.lock` (intentional!) will be silently skipped under the default. Mitigation: document this in the changelog and in the `ironlint doctor` output (C1) once that ships. For 0.2, the right escape valve is configuring the rule scope to a more specific path that doesn't intersect built-ins, or — if there's real demand — add the deferred `skip.replace_builtins: true` flag in 0.3.
...
1. **Spec coverage.** §A2 acceptance criteria:
   - [x] `.ironlint.yml` accepts a top-level `skip:` list → Task 3.
   ...
   - [x] `~/.ironlint-ignore` honored if present; absent is silent no-op → Task 4 + Task 6 third test.
...
After Task 8 is green: A2 is shipped. Next item in the 0.2.0 cohort is **A3 (diff pre-filter — `can_match_diff`)**; write `plans/2026-…-ironlint-a3-diff-prefilter.md` next. A3 only depends on the `engine: semantic` dispatch path and is independent of A2's runner-level short-circuit.
