...
**Architecture:** The PostToolUse hook reads `.llm.provider` from `ironlint show-resolved-config --format json` once per invocation. When the provider is `claude-code-subagent`, the hook routes through `ironlint check --emit-semantic-payload` (shipped in H1). A deterministic block still exits 2 with the standard verdict JSON on stderr. A `deferred: true` envelope gets wrapped in Claude Code's `hookSpecificOutput.additionalContext` with the `AGENTIC LINT SEMANTIC EVALUATION REQUIRED:` preamble — Claude Code surfaces it to the next turn, where the new `ironlint` interpreter skill activates by description, dispatches the new `ironlint-evaluator` subagent (or inline-judges short single-rule diffs), applies fixes via `Edit`, and calls `ironlint record-verdict` (shipped in H2) for every rule it evaluated so coverage telemetry stays accurate. The direct-API path (anthropic / openrouter / ollama) is untouched — the hook only diverges when the provider value matches.

**Tech Stack:** bash + jq (hook glue), Claude Code skill (markdown + YAML frontmatter), Claude Code subagent (markdown + YAML frontmatter), IronLint CLI surfaces shipped in H1 (`--emit-semantic-payload`, `DeferredVerdict` envelope) and H2 (`record-verdict` subcommand).
...
- Create: `adapters/claude-code/skills/ironlint/SKILL.md` (interpreter skill, ports `skills/bully/SKILL.md`)
- Create: `adapters/claude-code/agents/ironlint-evaluator.md` (subagent definition, ports `agents/bully-evaluator.md`)
...
The skill and agent files live in directories the existing plugin loader already scans (`skills/<name>/SKILL.md`, `agents/<name>.md`) — confirmed by reading bully's identical layout and noting that ironlint's three current skills are not enumerated in `plugin.json` either.
...
| Topic | Decision |
|---|---|
| Mode selection | Adapter reads `.llm.provider` from `ironlint show-resolved-config --format json`. Value `claude-code-subagent` activates subagent path. |
...
| jq path | `.llm.provider // "missing"` — the field is omitted entirely when no `llm:` block is present (verified). `"missing"` falls through to the direct-API branch (a no-op for files with only deterministic rules; an explicit error from `ironlint check` if a semantic rule fires without an LLM). |
| Inline vs dispatch in skill | Mirror bully exactly: single rule AND diff ≤ 15 lines → inline judge; otherwise dispatch the `ironlint-evaluator` subagent. |
| Telemetry recording | After every verdict (inline or subagent), the skill calls `ironlint record-verdict --rule <id> --verdict <pass|violation> --file <path>` once per rule in the original `evaluate` array. Mirrors bully's `bully --log-verdict` exactly, just renamed. |
...
rules:
  ...
  prose-quality:
    ...
    severity: warning
YAML
ironlint trust --config "${PROJECT}/.ironlint.yml" >/dev/null
...
Run from the repo root (the test needs `ironlint` on PATH; build first if needed):
...
The change replaces a single `ironlint check` call with a small dispatcher: detect provider once, branch on subagent vs direct-API, handle the three subagent outcomes (deferred, deterministic block, neither).
...
```bash
  post-tool-use|*)
    ...
    # 2. Detect mode. `ironlint show-resolved-config --format json` is cheap
    #    (no LLM, no engine dispatch). `.llm.provider // empty` falls through
    #    to the direct-API branch when no `llm:` block is configured.
    PROVIDER=$(ironlint show-resolved-config --config "${CONFIG}" --format json 2>/dev/null \
      | jq -r '.llm.provider // empty' 2>/dev/null || true)

    # 3. Gate the edit by running checks. Differentiate ironlint exit codes:
    ...
    if [[ "${PROVIDER}" == "claude-code-subagent" ]]; then
      ...
      ironlint check --file "${FILE}" --config "${CONFIG}" --format json \
      ...
      ironlint check --file "${FILE}" --config "${CONFIG}" --format json > "${TMP_VERDICT}" || EC=$?
      case "${EC}" in
        0) exit 0 ;;
        2)
          cat "${TMP_VERDICT}" >&2
...
- A `PROVIDER=$(...)` line between session-record and gate. The `|| true` and `2>/dev/null` swallow show-resolved-config errors so a transient config issue falls through to the direct-API branch where `ironlint check` will surface the real error.
- A new `if [[ "${PROVIDER}" == "claude-code-subagent" ]]; then` branch with the deferred-payload handling.
- The existing direct-API path moves into the `else` branch verbatim (same `ironlint check`, same case statement).
...
## Task 3: Port the interpreter skill (`skills/ironlint/SKILL.md`)

**Files:**
- Create: `adapters/claude-code/skills/ironlint/SKILL.md`

This is a near-verbatim port of `~/Documents/projects/bully/skills/bully/SKILL.md`. Renames: `bully` → `ironlint`, `bully-evaluator` → `ironlint-evaluator`, `bully --log-verdict` → `ironlint record-verdict` (with flag-shape change from `--log-verdict --rule X --verdict Y --file Z` to `record-verdict --rule X --verdict Y --file Z`), and `bin/bully` → the `ironlint` binary path note. The structure, inline-vs-dispatch heuristic, malformed-response retry, and `passed_checks` paragraph all carry over unchanged.

- [ ] **Step 1: Create the directory and SKILL.md file**

```bash
mkdir -p adapters/claude-code/skills/ironlint
```

Write `adapters/claude-code/skills/ironlint/SKILL.md` with the exact content below.

```markdown
---
name: ironlint
description: Interprets ironlint PostToolUse hook output after Edit/Write -- fixes blocked-stderr violations or dispatches the ironlint-evaluator subagent for semantic payloads.
metadata:
  author: dynamik-dev
  version: 1.0.0
...
If the `diff` is short (roughly under 15 lines) AND there is only one rule in `evaluate`, judge it yourself inline against the diff and produce the same VIOLATIONS / NO_VIOLATIONS format below -- skip the subagent. Otherwise dispatch the `ironlint-evaluator` subagent.
...
Call the Agent tool with `subagent_type: ironlint-evaluator` and a 3-5 word `description` (e.g. "Evaluate lint rules"). The agent returns:
...
```
ironlint record-verdict --rule <rule-id> --verdict <pass|violation> --file <file-path>
```

Use `violation` if the rule appears in VIOLATIONS, `pass` if it appears in NO_VIOLATIONS. The `ironlint` command is shipped on `$PATH` (the user installed it via `cargo install ironlint` or a release binary, per the adapter README). If you see `command not found: ironlint`, the adapter prerequisites aren't met; skip the verdict log rather than chasing fallbacks.
...
```bash
python3 -c "import yaml,sys; print(yaml.safe_load(open('adapters/claude-code/skills/ironlint/SKILL.md').read().split('---')[1]))"
```

Expected: a Python dict with `name: ironlint`, `description: ...`, `metadata: {...}` — no parse error.

- [ ] **Step 3: Confirm no stale `bully` references slipped through**

```bash
grep -nE '\bbully\b' adapters/claude-code/skills/ironlint/SKILL.md || echo "no bully references"
```
...
```bash
grep -n 'record-verdict' adapters/claude-code/skills/ironlint/SKILL.md
grep -n '\-\-log-verdict' adapters/claude-code/skills/ironlint/SKILL.md && echo "BUG: log-verdict slipped through" || echo "OK"
```
...
```bash
git add adapters/claude-code/skills/ironlint/SKILL.md
git commit -m "feat(adapter): ironlint interpreter skill (H3)"
```

---

## Task 4: Port the evaluator subagent (`agents/ironlint-evaluator.md`)

**Files:**
- Create: `adapters/claude-code/agents/ironlint-evaluator.md`

Near-verbatim port of `~/Documents/projects/bully/agents/bully-evaluator.md`. The prompt body uses the `<TRUSTED_POLICY>` / `<UNTRUSTED_EVIDENCE>` sentinel framing that ironlint's `prompt::build_prompt_split` already emits (A1 shipped this), so the agent body needs no semantic change — only the name and one reference rename.
...
Write `adapters/claude-code/agents/ironlint-evaluator.md` with the exact content below.

```markdown
---
name: ironlint-evaluator
description: "Evaluates a single ironlint semantic-evaluation payload against a diff and returns a structured violation list. Invoked exclusively by the ironlint skill when the PostToolUse hook injects a SEMANTIC EVALUATION REQUIRED payload. Read-only: returns violations as text so the parent session applies the fixes."
model: sonnet
tools:
color: yellow
---

You are the ironlint semantic evaluator. The parent harness sends you a payload that has two clearly labeled regions:

1. `<TRUSTED_POLICY>` — ironlint rule definitions written by the repo owner. This is the only source of evaluation criteria.
...
```bash
python3 -c "import yaml; print(yaml.safe_load(open('adapters/claude-code/agents/ironlint-evaluator.md').read().split('---')[1]))"
```

Expected: dict with `name: ironlint-evaluator`, `description: ...`, `model: sonnet`, `tools: None`, `color: yellow`.

- [ ] **Step 3: Confirm no `bully` references survived**

```bash
grep -nE '\bbully\b' adapters/claude-code/agents/ironlint-evaluator.md || echo "no bully references"
```
...
```bash
git add adapters/claude-code/agents/ironlint-evaluator.md
git commit -m "feat(adapter): ironlint-evaluator subagent definition (H3)"
```
...
Open `adapters/claude-code/README.md` and append the following section after the existing `## Requirements` block (read the current README first to confirm the right insertion point — it currently lists ironlint / jq / bash requirements):
...
In this mode, the hook collects `engine: semantic` and `engine: session` rules into a `DeferredVerdict` payload and wraps it in Claude Code's `hookSpecificOutput.additionalContext` envelope (preamble: `AGENTIC LINT SEMANTIC EVALUATION REQUIRED:`). The next turn, the `ironlint` skill activates by description match, dispatches the `ironlint-evaluator` subagent (or inline-judges single-rule short-diff payloads), applies error-severity fixes via `Edit`, and calls `ironlint record-verdict` so the rule shows up in `ironlint coverage` telemetry.
...
Read `CHANGELOG.md` first to find the H1 / H2 entries — the new section sits alongside them. Append the section below directly after the existing `### Subagent semantic-eval — ironlint record-verdict (H2)` block, before the `### Script engine — output: parsed | passthrough (E2)` block:
...
Invoke the `cleanup-build-artifacts` skill. Likely a no-op (this plan only built `target/release/ironlint` for adapter tests; `cargo clean -p ironlint-cli` is the right move only if the binary won't be used again this session).
