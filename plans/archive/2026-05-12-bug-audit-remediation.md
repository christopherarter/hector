# IronLint 0.1 Bug Audit Remediation Plan
...
**Tech Stack:** Rust workspace (`ironlint-core` + `ironlint-cli`), `globset`, `ast-grep-core`, `reqwest` blocking, `nix` (Linux-only), `insta` snapshots, `assert_cmd` for CLI integration, `wiremock` for HTTP, bash + TypeScript adapters.
...
Phase 0:
- Modify: `crates/ironlint-core/src/config/scope.rs:5-32`
- Modify: `crates/ironlint-core/src/runner.rs:115` (and the matcher construction at `runner.rs:73-77`)
- Test: `crates/ironlint-core/tests/scope.rs`, new `crates/ironlint-core/tests/runner_absolute_path.rs`

Phase 1:
- Modify: `crates/ironlint-core/src/config/extends.rs:13-32`
- Modify: `crates/ironlint-core/src/runner.rs:58-71`
- Modify: `crates/ironlint-core/src/engine/script.rs:44`, `crates/ironlint-core/src/engine/capability.rs:35-36,75-77`
- Modify: `crates/ironlint-core/src/diff/parser.rs:11-55`
- Test: `crates/ironlint-core/tests/extends.rs`, `crates/ironlint-core/tests/e2e_script_rules.rs`, `crates/ironlint-core/tests/diff_parse.rs`

Phase 2:
- Modify: `crates/ironlint-core/src/runner.rs:120-253` (diff branch refactor)
- Modify: `crates/ironlint-cli/src/commands/check.rs:35-58`
- Test: `crates/ironlint-core/tests/runner_diff.rs`, `crates/ironlint-cli/tests/cli_check.rs`

Phase 3:
- Modify: `crates/ironlint-core/src/engine/capability.rs:25-65`
- Optional: `crates/ironlint-core/src/config/types.rs:98-105` (schema if removing variants)
- Modify: `docs/security.md`, `CLAUDE.md` (truth-in-advertising)
- Test: `crates/ironlint-core/tests/capability.rs`

Phase 4:
- Modify: `crates/ironlint-cli/src/commands/baseline.rs:6-49`
- Modify: `crates/ironlint-cli/Cargo.toml` (add `ignore`, `rayon` crates)
- Test: `crates/ironlint-cli/tests/cli_baseline.rs`

Phase 5:
- Modify: `crates/ironlint-core/src/verdict.rs:3,43-51,65-86`
- Modify: `crates/ironlint-core/src/runner.rs:222,278`
- Modify: every engine's Violation construction (column/context decision)
- Test: `crates/ironlint-core/tests/verdict_snapshot.rs`

Phase 6:
- Modify: `crates/ironlint-core/src/disable.rs:21-26,56-58`
- Modify: `crates/ironlint-core/src/runner.rs:209-214`
- Modify: `crates/ironlint-core/src/llm/mod.rs:128-138`
- Modify: `crates/ironlint-core/src/engine/semantic.rs:19-21`, `crates/ironlint-core/src/engine/session.rs:24-26`
- Modify: `crates/ironlint-core/src/engine/capability.rs:51` (timeout/output cap)
- Modify: `crates/ironlint-core/Cargo.toml` (add `wait-timeout` or `tokio::time`)
- Test: each in its existing test file

Phase 7:
- Modify: `crates/ironlint-core/src/llm/anthropic.rs:25`, `crates/ironlint-core/src/llm/openai_compat.rs:35`
- Modify: `adapters/claude-code/hooks/hook.sh:83-87`, `adapters/opencode/src/index.ts:126-130`
- Modify: `crates/ironlint-core/src/telemetry.rs:17-25`
- Modify: `crates/ironlint-core/src/engine/ast.rs:13-69`, `crates/ironlint-core/src/engine/mod.rs` (signature change)
- Test: `crates/ironlint-core/tests/anthropic.rs`, `tests/openai_compat.rs`, `tests/ast_engine.rs`, `tests/telemetry.rs`, `adapters/{claude-code,opencode}/tests/*`
...
Turns IronLint from "no-op in production" to "rules fire." Single-fix phase. Sequential gate for everything else.

### Task 0.1: Make scope match work for absolute input paths

**Files:**
- Modify: `crates/ironlint-core/src/config/scope.rs:5-32`
- Modify: `crates/ironlint-core/src/runner.rs:155-162` (the loop that calls `matcher.matches(&path)`)
- Test: `crates/ironlint-core/tests/scope.rs` (new test), new file `crates/ironlint-core/tests/runner_absolute_path.rs`

- [ ] **Step 1: Write the failing test** in `crates/ironlint-core/tests/scope.rs`. Append:

```rust
#[test]
fn scope_matcher_matches_relative_when_input_is_absolute() {
    use ironlint_core::config::scope::ScopeMatcher;
    ...
    let abs = std::path::PathBuf::from("/Users/anyone/work/ironlint/crates/ironlint-core/src/runner.rs");
    assert!(m.matches(&abs), "absolute path must match relative glob");
    let rel = std::path::PathBuf::from("crates/ironlint-core/src/runner.rs");
...
Run: `cargo test -p ironlint-core --test scope scope_matcher_matches_relative_when_input_is_absolute -- --nocapture`
Expected: FAIL — assertion fails on the absolute case.

- [ ] **Step 3: Add a runner-level integration test** that proves real `IronLintEngine::check` fires for an absolute path. Create `crates/ironlint-core/tests/runner_absolute_path.rs`:

```rust
use ironlint_core::runner::{CheckInput, IronLintEngine};
use std::fs;
...
rules:
  test-rule:
    ...
    script: "exit 1"
"#;
    let cfg_path = root.join(".ironlint.yml");
    fs::write(&cfg_path, cfg).unwrap();
    let trusted = ironlint_core::trust::write_trust_block(cfg).unwrap();
...
Run: `cargo test -p ironlint-core --test runner_absolute_path -- --nocapture`
...
Run: `cargo test -p ironlint-core --test scope && cargo test -p ironlint-core --test runner_absolute_path`
Expected: PASS.
...
```bash
git add crates/ironlint-core/src/runner.rs crates/ironlint-core/src/config/scope.rs \
        crates/ironlint-core/tests/scope.rs crates/ironlint-core/tests/runner_absolute_path.rs
...
`ironlint check`; relative globs like `crates/*/src/**/*.rs` matched nothing,
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs:58-71`, `crates/ironlint-core/src/config/extends.rs:13-33`
- Test: `crates/ironlint-core/tests/extends.rs`
...
fn extends_chain_rejects_untrusted_parent() {
    ...
    let trusted = ironlint_core::trust::write_trust_block(child_raw).unwrap();
    let child = tmp.path().join("child.yml");
    std::fs::write(&child, trusted).unwrap();

    let err = IronLintEngine::load(&child).expect_err("must reject untrusted parent");
    let msg = format!("{err:#}");
    assert!(
...
Run: `cargo test -p ironlint-core --test extends extends_chain_rejects_untrusted_parent -- --nocapture`
...
Run: `cargo test -p ironlint-core --test extends`
Expected: the new test passes; existing tests pass (they all carry trust blocks already).

- [ ] **Step 5: Commit**

```bash
git add crates/ironlint-core/src/config/extends.rs crates/ironlint-core/src/runner.rs \
...
**Files:**
- Modify: `crates/ironlint-core/src/engine/script.rs:33-74`, `crates/ironlint-core/src/engine/capability.rs:14-87`
- Test: `crates/ironlint-core/tests/e2e_script_rules.rs`, `crates/ironlint-core/tests/script_engine.rs`
...
fn script_engine_quotes_file_path_with_shell_metacharacters() {
    use ironlint_core::config::{EngineKind, Rule, Severity};
    use ironlint_core::engine::script::run_script_rule;
    let tmp = tempfile::tempdir().unwrap();
    let cwd = tmp.path();
...
Run: `cargo test -p ironlint-core --test script_engine script_engine_quotes_file_path_with_shell_metacharacters -- --nocapture`
...
fn run_script_rule_internal(
    ...
    // `{file}` becomes the shell parameter `"$IRONLINT_FILE"`. The actual path is
    ...
    let outcome = crate::engine::capability::run_with_capabilities_env(
        ...
        cwd,
        &caps,
        &[("IRONLINT_FILE", &file.display().to_string())],
    )?;
    // ... rest unchanged
}
```
...
Run: `cargo test -p ironlint-core --test script_engine script_engine_quotes_file_path_with_shell_metacharacters`
Expected: PASS.

- [ ] **Step 5: Run full e2e to verify legit rules still work**

Run: `cargo test -p ironlint-core --test e2e_script_rules`
Expected: PASS.

- [ ] **Step 6: Commit**

```bash
git add crates/ironlint-core/src/engine/script.rs crates/ironlint-core/src/engine/capability.rs \
        crates/ironlint-core/tests/script_engine.rs
git commit -m "$(cat <<'EOF'
...
shell. Replace {file} with "$IRONLINT_FILE" and pass the path via the child
...
Run: `cargo test -p ironlint-core --test diff_parse parse_unified_rejects_path_traversal -- --nocapture`
...
Run: `cargo test -p ironlint-core --test diff_parse`
Expected: PASS.
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs:120-253`
- Modify: `crates/ironlint-cli/src/commands/check.rs:35-58`
- Test: `crates/ironlint-core/tests/runner_diff.rs`, `crates/ironlint-cli/tests/cli_check.rs`
...
fn ast_rule_runs_in_diff_mode_when_file_on_disk() {
    ...
    let engine = ironlint_core::runner::IronLintEngine::load(&cfg_path).unwrap();
    let diff = format!(
        "--- a/src/foo.rs\n+++ b/src/foo.rs\n@@ -1 +1 @@\n-fn main() {{}}\n+fn main() {{ let _ = foo.unwrap(); }}\n"
    );
    let v = engine
        .check(ironlint_core::runner::CheckInput::Diff {
            file: target.clone(),
            unified_diff: diff,
        })
        .unwrap();
...
```rust
#[test]
fn ironlint_disable_directive_applies_in_diff_mode() {
    let tmp = tempfile::tempdir().unwrap();
    let root = tmp.path();
    std::fs::create_dir_all(root.join("src")).unwrap();
    let target = root.join("src/foo.rs");
    std::fs::write(
        &target,
        "fn main() { let _ = foo.unwrap(); } // ironlint-disable: no-unwrap\n",
    )
    .unwrap();
    let cfg = "schema_version: 2\nrules:\n  no-unwrap:\n    description: x\n    engine: ast\n    language: rust\n    scope: [\"src/**/*.rs\"]\n    severity: error\n    pattern: $E.unwrap()\n";
    let trusted = ironlint_core::trust::write_trust_block(cfg).unwrap();
    ...
    assert!(
        !v.violations.iter().any(|x| x.rule_id == "no-unwrap"),
        "ironlint-disable on the same line must silence the rule, got {v:?}"
    );
}
```

- [ ] **Step 3: Write the multi-file failing test** in `crates/ironlint-cli/tests/cli_check.rs`:
...
fn cli_check_diff_processes_every_changed_file() {
    ...
    let out = Command::cargo_bin("ironlint").unwrap()
...
Run: `cargo test -p ironlint-core --test runner_diff ast_rule_runs_in_diff_mode_when_file_on_disk ironlint_disable_directive_applies_in_diff_mode -- --nocapture && cargo test -p ironlint-cli --test cli_check cli_check_diff_processes_every_changed_file -- --nocapture`
...
(None, Some(d)) => {
    ...
    let changed = ironlint_core::diff::parser::parse_unified(&unified_diff)?;
    if changed.is_empty() {
    ...
    let verdict = ironlint_core::verdict::Verdict::from_violations(
...
Run: `cargo test -p ironlint-core --test runner_diff && cargo test -p ironlint-cli --test cli_check`
Expected: PASS.
...
```bash
git add crates/ironlint-core/src/runner.rs crates/ironlint-cli/src/commands/check.rs \
        crates/ironlint-core/tests/runner_diff.rs crates/ironlint-cli/tests/cli_check.rs
git commit -m "$(cat <<'EOF'
...
- DisableMap empty so ironlint-disable directives never applied (P0-7),
...
fn capability_run_succeeds_for_unprivileged_user() {
    use ironlint_core::config::{Capabilities, WritesPolicy};
...
Run (Linux non-root): `cargo test -p ironlint-core --test capability capability_run_succeeds_for_unprivileged_user`
...
Add the `libc` dependency to `ironlint-core/Cargo.toml`. Put it under the top-level `[dependencies]` (not the Linux target block) because telemetry's `flock` in Phase 7 also uses it on macOS:
...
**Files:**
- Modify: `crates/ironlint-core/src/engine/capability.rs:60-65` (delete dead stub)
- Modify: `docs/security.md`, `CLAUDE.md`
- Optional schema change: `crates/ironlint-core/src/config/types.rs:98-105`
...
If you need write isolation today, run ironlint inside an OS-level
...
```bash
git add crates/ironlint-core/src/engine/capability.rs docs/security.md CLAUDE.md
git commit -m "$(cat <<'EOF'
...
`ironlint baseline` walks every file with `walkdir` and reads each into memory. On IronLint's own repo with `target/` populated, this OOMs or takes minutes. Single task.

### Task 4.1: gitignore-aware, parallel baseline walk

**Files:**
- Modify: `crates/ironlint-cli/src/commands/baseline.rs:6-49`
- Modify: `crates/ironlint-cli/Cargo.toml` (add `ignore = "0.4"`, `rayon = "1"`)
- Test: `crates/ironlint-cli/tests/cli_baseline.rs`
...
fn baseline_skips_gitignored_and_target_dirs() {
    ...
    let baseline: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(root.join(".ironlint/baseline.json")).unwrap()).unwrap();
...
Run: `cargo test -p ironlint-cli --test cli_baseline baseline_skips_gitignored_and_target_dirs -- --nocapture`
Expected: FAIL — `target/` entries appear in the baseline.

- [ ] **Step 3: Add deps** to `crates/ironlint-cli/Cargo.toml`:
...
pub fn run(config: &Path, scan_glob: Option<String>) -> Result<i32> {
    let engine = IronLintEngine::load(config)?;
    let dir = config
        .parent()
        .filter(|p| !p.as_os_str().is_empty())
        .unwrap_or(Path::new("."));
    let baseline_path = dir.join(".ironlint/baseline.json");
...
Run: `cargo test -p ironlint-cli --test cli_baseline`
Expected: PASS.
...
```bash
git add crates/ironlint-cli/src/commands/baseline.rs crates/ironlint-cli/Cargo.toml \
        crates/ironlint-cli/tests/cli_baseline.rs Cargo.toml
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `crates/ironlint-core/src/verdict.rs:3,43-51`
- Modify: `crates/ironlint-core/src/runner.rs:222,278`
- Test: `crates/ironlint-core/tests/verdict_snapshot.rs`
...
fn engine_enum_separates_trust_from_internal() {
    use ironlint_core::verdict::Engine;
...
Run: `cargo test -p ironlint-core --test verdict_snapshot engine_enum_separates_trust_from_internal`
...
```bash
git add crates/ironlint-core/src/verdict.rs crates/ironlint-core/src/runner.rs \
        crates/ironlint-core/tests/verdict_snapshot.rs crates/ironlint-core/tests/snapshots
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `crates/ironlint-core/src/engine/ast.rs:60-69` (column has data from `start_pos()`)
- Modify: `crates/ironlint-core/src/engine/{script,semantic,session,ast}.rs` (context for AST is N surrounding lines)
- Test: `crates/ironlint-core/tests/ast_engine.rs`, `crates/ironlint-core/tests/verdict_snapshot.rs`
...
fn ast_violation_populates_column_and_context() {
    let rule = ironlint_core::config::Rule {
        description: "x".into(),
        engine: ironlint_core::config::EngineKind::Ast,
        scope: vec!["**/*.rs".into()],
    ...
    let ctx = ironlint_core::engine::RuleContext {
        rule_id: "no-unwrap",
        rule: &rule,
    ...
    use ironlint_core::engine::RuleEngine;
    let v = ironlint_core::engine::ast::AstEngine.run(&ctx).unwrap().unwrap();
...
Run: `cargo test -p ironlint-core --test ast_engine ast_violation_populates_column_and_context`
...
Run: `cargo test -p ironlint-core --test ast_engine`
Expected: PASS.

- [ ] **Step 5: Commit**

```bash
git add crates/ironlint-core/src/engine/ast.rs crates/ironlint-core/tests/ast_engine.rs
git commit -m "$(cat <<'EOF'
...
### Task 6.1: File-level `ironlint-disable:` honors `v.line == None` (P1-2) [parallel]

**Files:** `crates/ironlint-core/src/disable.rs:21-26`, `crates/ironlint-core/src/runner.rs:209-214`, `crates/ironlint-core/tests/disable.rs`
...
fn file_level_disable_silences_script_violation_without_line() {
    use ironlint_core::disable::DisableMap;
    let src = "// ironlint-disable: noisy-script\nfn main() {}\n";
...
- [ ] **Step 3: Verify & commit** (`cargo test -p ironlint-core --test disable`, then commit with the standard "fix(disable): apply directives to file-level violations (P1-2)" message).

### Task 6.2: LLM unknown status → engine error, not silent violation (P1-5) [parallel]

**Files:** `crates/ironlint-core/src/llm/mod.rs:113-141`, `crates/ironlint-core/tests/llm_factory.rs`
...
fn parse_verdicts_returns_err_on_unknown_status() {
    ...
    let err = ironlint_core::llm::parse_verdicts(body)
...
fn parse_verdicts_lowercases_status() {
    ...
    let v = ironlint_core::llm::parse_verdicts(body).unwrap();
    assert!(matches!(v[0].status, ironlint_core::llm::RuleStatus::Pass));
    assert!(matches!(v[1].status, ironlint_core::llm::RuleStatus::Pass));
}
```
...
**Files:** `crates/ironlint-core/src/engine/semantic.rs:19-21`, `crates/ironlint-core/src/engine/session.rs:24-26`, `crates/ironlint-core/tests/semantic_engine.rs`, `crates/ironlint-core/tests/session_engine.rs`
...
async fn semantic_engine_errors_on_rule_id_mismatch() {
    ...
    Mock::given(method("POST"))
        ...
        .mount(&server)
        .await;
    let client = ironlint_core::llm::AnthropicClient::new("k", "m", Some(server.uri()));
    ...
    use ironlint_core::engine::RuleEngine;
    let err = ironlint_core::engine::semantic::SemanticEngine.run(&ctx).expect_err("must error on mismatch");
...
**Files:** `crates/ironlint-core/src/engine/capability.rs:51`, `crates/ironlint-core/Cargo.toml` (add `wait-timeout = "0.2"`)
...
let status = match child.wait_timeout(TIMEOUT).context("wait_timeout")? {
    ...
    None => { child.kill().ok(); child.wait().ok(); return Ok(ExecOutcome {
        stdout: String::new(),
        stderr: format!("ironlint: script killed after {TIMEOUT:?}"),
        exit_code: 124,
    });}
};
...
**Files:** `crates/ironlint-core/src/llm/anthropic.rs:25`, `crates/ironlint-core/src/llm/openai_compat.rs:35`, `crates/ironlint-core/tests/anthropic.rs`, `crates/ironlint-core/tests/openai_compat.rs`
...
**Files:** `adapters/claude-code/hooks/hook.sh:78-90`, `adapters/opencode/src/index.ts:126-130`, `crates/ironlint-core/src/engine/session.rs:17-22`, `adapters/{claude-code,opencode}/tests/*`
...
**Files:** `crates/ironlint-core/src/telemetry.rs:17-25`, `crates/ironlint-core/tests/telemetry.rs`
...
fn telemetry_append_is_atomic_under_concurrent_writers() {
    ...
    let handles: Vec<_> = (0..16).map(|i| {
        ...
        thread::spawn(move || {
            for j in 0..100 {
                let entry = ironlint_core::telemetry::LogEntry {
                    timestamp: "t".into(),
                    kind: "check".into(),
                ...
                ironlint_core::telemetry::append(&p, &entry).unwrap();
            }
        })
    }).collect();
...
**Files:** `crates/ironlint-core/src/engine/ast.rs:13-69`, `crates/ironlint-core/src/engine/mod.rs` (signature), every `RuleEngine` impl, `crates/ironlint-core/src/runner.rs` (use the vec), `tests/ast_engine.rs`
...
fn ast_returns_every_match_not_just_first() {
    ...
    use ironlint_core::engine::RuleEngine;
    let vs = ironlint_core::engine::ast::AstEngine.run(&ctx).unwrap();
...
- [ ] **P2-1 session.json read-modify-write race** in `crates/ironlint-cli/src/commands/session.rs:5-21`. Test: spawn 32 threads each calling `record` with a distinct `EditRecord`; assert final `session.json` contains all 32. Fix: `fs2::FileExt::lock_exclusive` on a sibling `.lock` file around load → mutate → save.
...
- [ ] **P2-7 `ironlint trust` destroys comments** in `trust.rs:71-81`. Test: assert that `write_trust_block("# leading comment\nschema_version: 2\n…")` preserves the comment. Fix: locate existing `trust:` block via line regex; edit in place; preserve everything else verbatim. If no `trust:` block, append it at EOF.
- [ ] **P2-8 `ironlint migrate` naive string replace** in `commands/migrate.rs:20`. Test: assert that a `.bully.yml` with `# schema_version: 1` in a comment doesn't get touched. Fix: parse YAML, set `schema_version: 2`, re-serialize.
- [ ] **P2-11 v1 schema accepted but unloadable** in `runner.rs:60-73`. Test: load a v1 config; expect a clean "run ironlint migrate" error before trust verify. Fix: detect `schema_version == 1` at parse time *before* `trust::verify`.
...
- [ ] **P2-4 disable parser breaks on `/`** in `disable.rs:56-58,63`. Test: `ironlint-disable: python/no-print` must yield the full id. Fix: only treat `/` as terminator when followed by `/` or `*`.
...
- [ ] **P2-9 init template grep masks exit 2** in `commands/init.rs:54,65,87`. Test: integration test asserts that `ironlint check` on a Rust file with no `.unwrap()` matches passes (exit 0) and a file with `.unwrap()` blocks. Fix: change `grep PATTERN {file} && exit 1 || exit 0` to `! grep -q PATTERN {file}`.
- [ ] **P2-14 opencode `apply_patch` not gated** in `adapters/opencode/src/index.ts:8`. Test: in `tests/`, add a test stub that simulates `apply_patch` and asserts the multi-file patch is broken into per-file invocations of `ironlint check`. Fix: parse multi-file patch in the adapter, loop `ironlint check --file` over each `+++ b/<path>`.
...
- [ ] **P2-21 telemetry write failures silently dropped** in `runner.rs:196,248,279,303`. Test: make `.ironlint/log.jsonl` not writable; assert a stderr warning. Fix: replace `let _ = …` with `if let Err(e) = … { eprintln!("ironlint: telemetry append failed: {e:#}"); }`.
