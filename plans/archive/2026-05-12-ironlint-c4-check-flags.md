# IronLint C4 — `--rule`, `--explain`, `--print-prompt` flags on `ironlint check`
...
**MODIFIED:**
- `crates/ironlint-cli/src/cli.rs` — three new `Check` args (`--rule` repeatable; `--explain` flag; `--print-prompt` flag).
- `crates/ironlint-cli/src/main.rs` — pass the new args through to `commands::check::run`.
- `crates/ironlint-cli/src/commands/check.rs` — accept the new args; validate `--rule` ids against the loaded config; render explain output to stderr; route `--print-prompt` to a new core helper.
- `crates/ironlint-core/src/runner.rs` — new `CheckOptions` struct (rule filter, explain capture, print-prompt mode); new builder method `with_options`; thread options into `check`; emit `RuleExplain` records when explain is on; honor the rule filter; expose a new method `render_semantic_prompts(&self, input: CheckInput) -> Result<Vec<RenderedPrompt>>` that walks scoped semantic rules and returns the rendered (system, user) text without touching the LLM.
- `crates/ironlint-core/src/lib.rs` — re-export the new types so the CLI can use them.

**NEW:**
- `crates/ironlint-cli/tests/cli_check_flags.rs` — assert_cmd tests for all three flags.
- `crates/ironlint-core/tests/runner_check_options.rs` — library-level tests verifying the rule filter, the explain capture, and the prompt-render path (zero LLM dispatches via the counting `LlmClient`).

**Out-of-scope to TOUCH:**
- `crates/ironlint-core/src/verdict.rs` — verdict JSON shape is locked. Explain output goes to stderr, never the verdict.
- `crates/ironlint-core/src/config/types.rs::execution` — B1 territory.
- `crates/ironlint-core/src/baseline.rs`, `crates/ironlint-cli/src/commands/baseline.rs` — E1 territory.
...
**Files:**
- Create: `crates/ironlint-core/tests/runner_check_options.rs`
...
use anyhow::Result;
use ironlint_core::config::Rule;
use ironlint_core::llm::{LlmClient, RuleStatus, RuleVerdict};
...
fn rule_filter_runs_only_listed_ids() {
    ...
    let engine = IronLintEngine::builder().with_options(opts).load(&cfg).unwrap();
    let verdict = engine
...
Run: `cargo test -p ironlint-core --test runner_check_options rule_filter_runs_only_listed_ids`
...
- Modify: `crates/ironlint-core/src/lib.rs` (re-export)

- [ ] **Step 3: Define `CheckOptions` in `runner.rs`**

Add near the top of `runner.rs` (above `IronLintEngine`):

```rust
use std::collections::HashSet;

/// Optional per-run knobs for `IronLintEngine::check`. Plumbed via
/// `IronLintEngine::builder().with_options(...)` so the public `check`
...
impl IronLintEngineBuilder {
    ...
    pub fn load(self, config_path: &Path) -> Result<IronLintEngine> {
        IronLintEngine::load_with(config_path, self.llm, self.options)
    }
}
```
...
Verify the lib re-exports — `runner` should be `pub mod runner;` already. No additional re-export needed since `CheckOptions` is accessed via `ironlint_core::runner::CheckOptions`.
...
```bash
git add crates/ironlint-core/src/runner.rs crates/ironlint-core/tests/runner_check_options.rs
...
IronLintEngineBuilder::with_options so the public check signature is
...
- Modify: `crates/ironlint-cli/src/commands/check.rs`
- Create: `crates/ironlint-cli/tests/cli_check_flags.rs`
...
Run: `cargo test -p ironlint-cli --test cli_check_flags`
Expected: FAIL — `--rule` flag not recognized.

- [ ] **Step 9: Add `--rule` flag to clap**

In `crates/ironlint-cli/src/cli.rs`, extend the `Check` variant:
...
Update `run` signature and body. New imports: `use ironlint_core::runner::CheckOptions;` and `use std::collections::HashSet;`.
...
fn validate_rule_filter(engine: &IronLintEngine, rules: &[String]) -> Option<i32> {
...
Expose `config_rule_ids` on `IronLintEngine` in `runner.rs`:
...
Run: `cargo test -p ironlint-cli --test cli_check_flags rule_flag_restricts_evaluation_to_named_rule`
Expected: PASS.

Run: `cargo test -p ironlint-cli --test cli_check_flags unknown_rule_id_exits_one_with_clear_error`
Expected: PASS.

- [ ] **Step 12: Commit**

```bash
git add crates/ironlint-cli/src/cli.rs crates/ironlint-cli/src/main.rs crates/ironlint-cli/src/commands/check.rs crates/ironlint-cli/tests/cli_check_flags.rs crates/ironlint-core/src/runner.rs
git commit -m "$(cat <<'EOF'
feat(cli): add --rule flag for selective rule execution (C4 phase 1)

`ironlint check --rule <id>` (repeatable via clap Append) restricts the
...
**Files:**
- Modify: `crates/ironlint-core/tests/runner_check_options.rs`
...
fn explain_captures_every_in_scope_rule() {
    ...
    assert!(matches!(fire.outcome, ironlint_core::runner::ExplainOutcome::Fire));
    let pass = report.explain.iter().find(|e| e.rule_id == "pass-rule").unwrap();
    assert!(matches!(pass.outcome, ironlint_core::runner::ExplainOutcome::Pass));
}
```

- [ ] **Step 14: Run test, watch it fail**

Run: `cargo test -p ironlint-core --test runner_check_options explain_captures_every_in_scope_rule`
...
fn try_semantic_skip(&self, rule_id: &str, rule: &Rule, path: &Path, diff: &str) -> Option<String> {
    ...
    let log_path = self.config_dir.join(".ironlint/log.jsonl");
...
In `crates/ironlint-cli/tests/cli_check_flags.rs`, add:
...
if explain {
    ...
    let report = match (file, diff) {
        ...
        (None, Some(d)) => {
            ...
            let mut agg_explain: Vec<ironlint_core::runner::RuleExplain> = Vec::new();
            ...
            ironlint_core::runner::CheckReport {
...
```rust
fn print_explain(rows: &[ironlint_core::runner::RuleExplain]) {
    use ironlint_core::runner::ExplainOutcome;
    for row in rows {
...
Run: `cargo test -p ironlint-cli --test cli_check_flags explain_prints_per_rule_outcome_to_stderr`
Expected: PASS.
...
`ironlint check --explain` prints `<rule-id> <engine> <fire|pass|dispatched|skipped <reason>>`
to stderr after the verdict, leaving stdout JSON intact. Implemented via
a new `IronLintEngine::check_with_explain` that returns the verdict plus a
...
Run: `cargo test -p ironlint-core --test runner_check_options print_prompt_path_does_not_dispatch_llm`
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs`
- Modify: `crates/ironlint-core/src/lib.rs` (if needed)
...
/// `ironlint check --print-prompt` to debug prompt construction without
/// burning API calls.
///
...
In `crates/ironlint-cli/tests/cli_check_flags.rs`, append:
...
Run: `cargo test -p ironlint-cli --test cli_check_flags print_prompt_renders_and_exits_zero` → FAIL.
...
`ironlint check --print-prompt` renders the (system, user) prompt for
...
- `crates/ironlint-cli/src/commands/check.rs` — heavy CLI plumbing; many branches. If under 90%, add CLI tests for: missing `--file` and `--diff`, `--print-prompt` with neither file nor diff, `--print-prompt` with empty diff.
- `crates/ironlint-core/src/runner.rs` — `render_semantic_prompts` has File-vs-Diff branches and scope/engine filters; cover all three branches in the core test.

- [ ] **Step 33: Plan archive**

When the PR is merged, move this plan to `plans/archive/2026-05-12-ironlint-c4-check-flags.md` and update `plans/README.md`.
...
**Zero rules after filter.** `ironlint check --rule unknown` exits 1 before any rule runs. `ironlint check --rule rule-out-of-scope` exits 0 with an empty `passed_checks`/`violations` — the verdict is `Pass` (nothing to evaluate is the same shape as everything passing trivially). This matches bully's behavior.
...
- `IronLintEngine` gains one field (`options: CheckOptions`); `IronLintEngineBuilder` gains `with_options`. Trivial 3-way merge.
