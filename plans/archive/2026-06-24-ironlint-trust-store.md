# IronLint Trust Store (Plan 2) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the dormant in-YAML trust fingerprint with a real, out-of-repo allow-list (the direnv model): `ironlint check` refuses to run any gate until the config **and** its gate scripts have been blessed *on this machine* via `ironlint trust`.

**Architecture:** A new `ironlint-core::trust` module computes a sha256 over the config bytes plus every file under `.ironlint/gates/` (sorted by relative path), and reads/writes a blessed-hash store at `~/.config/ironlint/trust.json` keyed by the config's canonical absolute path. Enforcement lives in the **CLI `check` command** (`commands/check.rs`) — it calls `trust::ensure_trusted(config)` before building the engine and fails closed (exit 1) on mismatch/missing. Core `IronLintEngine::load` stays pure, so core unit tests are unaffected; only CLI tests that invoke `check` bless first. `ironlint trust` writes the blessed hash; `ironlint init` auto-blesses what it scaffolds. Home-dir resolution is hand-rolled XDG (`$XDG_CONFIG_HOME` → `$HOME/.config`) — no new dependency, and `$XDG_CONFIG_HOME` doubles as the test redirect hook.
...
Copied verbatim from `specs/2026-06-15-ironlint-gates-redesign-design.md` §6 and `AGENTS.md`. Every task's requirements implicitly include this section.

- **Trust store path:** `~/.config/ironlint/trust.json`, XDG-respecting (`$XDG_CONFIG_HOME` overrides `$HOME/.config`). Keyed by the **canonical absolute path** of `.ironlint.yml`.
- **Trust hash:** sha256 over the config file bytes **plus the bytes of every file under `.ironlint/gates/`** (sorted by relative path). Any change to any gate script invalidates trust. Limitation (documented, not fixed): trust covers the config and `.ironlint/gates/` only; a `run` that sources scripts elsewhere, or an `extends:` parent config, is the author's responsibility.
- **Fail closed:** on mismatch or missing entry, refuse to run any gate, **outer exit code `1`** (config/load-error tier), message `config/gates not trusted — review and run \`ironlint trust\``.
- **No interactive prompt.** Blessing is always the explicit `ironlint trust` action (the hook has no human). `ironlint init` auto-blesses the config it just wrote.
...
- **Repo rules:** bug fixes start with a failing test (TDD: write failing test → verify red → minimal impl → verify green → commit). Every touched `crates/*/src/` file must reach ≥80% region coverage. Cognitive complexity ≤15 per function (clippy). `Cargo.lock` is gitignored — do not commit. Binary is `ironlint`. `cargo fmt` + `cargo clippy --all-targets -- -D warnings` clean before each commit.

## Out of scope (later plans)

- `ironlint verify` (dynamic proof) and the full `doctor` expansion incl. trust-status reporting — **Plan 3**.
...
- **`crates/ironlint-core/src/trust.rs`** — *replaced wholesale*. Old API (`canonicalize_for_fingerprint`, `fingerprint`, `verify`, `write_trust_block`, the anchor/alias scanner, `yaml_to_json`, `sort_json_keys`) is deleted. New API: `compute_hash`, the `TrustStore`/`TrustEntry` types, `read_store`/`write_store`, `ensure_trusted_in`/`bless_in` (testable cores), and the thin `trust_store_path`/`ensure_trusted`/`bless` wrappers.
- **`crates/ironlint-core/tests/trust_canonical_json.rs`** — *deleted* (pins the removed legacy API; would break compilation).
- **`crates/ironlint-cli/src/commands/trust.rs`** — stub → real bless via `trust::bless`.
- **`crates/ironlint-cli/src/commands/check.rs`** — add the `ensure_trusted` gate at the top of `run()`.
- **`crates/ironlint-cli/src/commands/init/mod.rs`** — auto-bless after writing the config.
- **`crates/ironlint-cli/src/cli.rs`** — fix the stale `Trust` help string.
- **`crates/ironlint-cli/tests/common/mod.rs`** — *new* shared `bless()` helper for e2e tests.
- **`crates/ironlint-cli/tests/cli_e2e_trust.rs`** — *new* enforcement e2e tests.
...
**Files:**
- Modify (replace wholesale): `crates/ironlint-core/src/trust.rs`
- Delete: `crates/ironlint-core/tests/trust_canonical_json.rs`
...
Expected: the only hits are in `crates/ironlint-core/src/trust.rs` itself and `crates/ironlint-core/tests/trust_canonical_json.rs`. If any *other* file references them, stop and report (the plan assumed trust.rs is unwired). The CLI `trust` stub does **not** call these (verify with the same grep).

- [ ] **Step 2: Delete the dormant legacy test**

```bash
git rm crates/ironlint-core/tests/trust_canonical_json.rs
```

- [ ] **Step 3: Write the failing tests for `compute_hash`**

Replace the entire body of `crates/ironlint-core/src/trust.rs` test module with these (and leave the impl empty for now so they fail to compile/assert):
...
Replace the **non-test** portion of `crates/ironlint-core/src/trust.rs` with:
...
/// every file under `<config-dir>/.ironlint/gates/` (sorted by relative path).
/// Returns `"sha256:<hex>"`.
pub fn compute_hash(config_path: &Path) -> Result<String> {
    ...
    let gates_dir = config_dir.join(".ironlint").join("gates");
    if gates_dir.is_dir() {
...
Run: `cargo test -p ironlint-core --lib trust:: && cargo clippy -p ironlint-core --all-targets -- -D warnings && cargo fmt`
...
git rm --cached crates/ironlint-core/tests/trust_canonical_json.rs 2>/dev/null || true
git add -A crates/ironlint-core/tests/
git commit -m "feat(trust)!: hash config + .ironlint/gates/ for the trust store; drop in-YAML fingerprint"
```
...
**Files:**
- Modify: `crates/ironlint-core/src/trust.rs`
...
```rust
    ...
    fn never_blessed_is_not_trusted() {
        ...
        assert!(err.contains("ironlint trust"), "message must point at `ironlint trust`: {err}");
    }
    ...
    fn editing_a_gate_after_bless_revokes_trust() {
        ...
        write(&proj.path().join(".ironlint/gates/g.sh"), "#!/bin/sh\nexit 2\n");
...
Run: `cargo test -p ironlint-core && cargo clippy -p ironlint-core --all-targets -- -D warnings && cargo fmt`
...
### Task 4: `ironlint trust` blesses the config (CLI)

**Files:**
- Modify: `crates/ironlint-cli/src/commands/trust.rs`
- Modify: `crates/ironlint-cli/src/cli.rs:54-58` (the `Trust` doc comment)
- Test: `crates/ironlint-cli/tests/cli_e2e_trust.rs` (new — the `trust`-command portion)

**Interfaces:**
- Consumes: `ironlint_core::trust::bless` (Task 3).

- [ ] **Step 1: Write the failing e2e test**

Create `crates/ironlint-cli/tests/cli_e2e_trust.rs`:
...
In `crates/ironlint-cli/src/cli.rs`, replace the `Trust` doc comment:
...
Run: `cargo test -p ironlint-cli --test cli_e2e_trust && cargo clippy -p ironlint-cli --all-targets -- -D warnings && cargo fmt`
Expected: PASS, clean.

- [ ] **Step 6: Commit**

```bash
git add crates/ironlint-cli/src/commands/trust.rs crates/ironlint-cli/src/cli.rs crates/ironlint-cli/tests/cli_e2e_trust.rs
git commit -m "feat(cli): ironlint trust blesses config + gates in the trust store"
```

---

### Task 5: Enforce trust in `ironlint check` + bless existing check tests

**Files:**
- Modify: `crates/ironlint-cli/src/commands/check.rs` (top of `run()`)
- Create: `crates/ironlint-cli/tests/common/mod.rs` (shared `bless` helper)
- Modify: `crates/ironlint-cli/tests/cli_e2e_trust.rs` (add enforcement cases)
...
**Interfaces:**
- Consumes: `ironlint_core::trust::ensure_trusted` (Task 3).

- [ ] **Step 1: Write the failing enforcement e2e tests**

Append to `crates/ironlint-cli/tests/cli_e2e_trust.rs`:
...
Run: `cargo test -p ironlint-cli --test cli_e2e_trust unblessed_config_check_exits_1`
...
In `crates/ironlint-cli/src/commands/check.rs`, at the very top of `run()` (before building `CheckOptions`):
...
Create `crates/ironlint-cli/tests/common/mod.rs`:
...
```rust
    ...
    cmd.env("IRONLINT_DEBUG_LOAD_COUNT", "1")
        .args(["check", "--config"]).arg(&cfg)
        .arg("--file").arg(&target);
    cmd.assert().success();
```
...
Run: `cargo test -p ironlint-cli && cargo clippy -p ironlint-cli --all-targets -- -D warnings && cargo fmt`
...
```bash
git add crates/ironlint-cli/src/commands/check.rs crates/ironlint-cli/tests/
git commit -m "feat(cli)!: ironlint check fails closed on untrusted config/gates"
```

---

### Task 6: `ironlint init` auto-blesses the scaffolded config

**Files:**
- Modify: `crates/ironlint-cli/src/commands/init/mod.rs:30-36` (`run`)
- Test: `crates/ironlint-cli/tests/cli_init.rs` (add one case)
...
Run: `cargo test -p ironlint-cli --test cli_init init_auto_blesses_so_check_is_trusted`
Expected: FAIL — check exits 1 (config not blessed).

- [ ] **Step 3: Auto-bless in `init::run`**

In `crates/ironlint-cli/src/commands/init/mod.rs`, after `std::fs::write(&cfg_path, body)?;`:
...
```rust
    ...
    println!(
        "review the config, then run: ironlint check --file <path> --config {}",
        cfg_path.display()
    );
    Ok(0)
```
...
Run: `cargo test -p ironlint-cli --test cli_init && cargo clippy -p ironlint-cli --all-targets -- -D warnings && cargo fmt`
Expected: PASS, clean.
...
git commit -m "feat(cli): ironlint init auto-blesses the scaffolded config"
```
...
- `trust` module bullet: drop "**Kept but unwired**"; describe the new store (`~/.config/ironlint/trust.json`, hash over config + `.ironlint/gates/`, keyed by canonical abs path).
- Exit-code contract `1` line: add "or untrusted config/gates".
- Conventions bullet about `trust.rs` and `tests/trust_canonical_json.rs` being dormant: remove it (the test is deleted; trust.rs is live). Note the new enforcement site is the CLI `check` command, not `IronLintEngine::load`.
...
If present, add an `Unreleased` bullet: "Trust: out-of-repo allow-list at `~/.config/ironlint/trust.json`; `ironlint check` fails closed until `ironlint trust` blesses the config + `.ironlint/gates/`; `ironlint init` auto-blesses."
...
- [ ] Manual smoke: in a scratch dir, `ironlint init` → `ironlint check --file x` passes trust; edit a gate script → `ironlint check` exits 1 "not trusted"; `ironlint trust` → passes again.
