# IronLint C1 — `ironlint doctor` diagnostic subcommand
...
| Default config name within `--dir` | `.ironlint.yml` | Mirrors every other subcommand. |
...
| Telemetry writes | **None.** | Doctor is read-only. No `.ironlint/log.jsonl` append. |
...
| Adapter check scope | **Best-effort & advisory.** A missing `~/.claude/settings.json` is a `warn` ("Claude Code adapter not detected"), not a `fail`. IronLint is editor-agnostic; not every user runs Claude Code. | The hook check itself (when the file exists) is structural: parse JSON, look for a `PostToolUse` matcher whose `command` references `ironlint` or the adapter's `hook.sh`. |
...
| `.ironlint/` writable check | Probe by writing then deleting a marker file under `<dir>/.ironlint/`. Failure → `fail` with a remediation hint to `chmod` the directory. | Matches bully behavior; `.ironlint/` is created lazily by other commands but doctor reports the state without creating it. We DO create the directory if it doesn't exist (read-only-on-state means we don't *modify policy state* — creating an empty `.ironlint/` is the same kind of side effect as `init` writing `.ironlint.yml`). |
...
README.md                               ← MODIFIED: add `ironlint doctor` to the commands list
...
**Telemetry-schema impact.** None. Doctor is read-only — no `.ironlint/log.jsonl` appends, no `kind` strings introduced.

**Config-schema impact.** None. Doctor reads but does not modify `.ironlint.yml`.
...
fn nonempty_env_var_is_present() {
    let name = "IRONLINT_DOCTOR_TEST_PRESENT";
...
Run: `cargo test -p ironlint-core --test llm_api_key_env_present`
Expected: FAIL with `unresolved import ironlint_core::llm::api_key_env_present` / `function or associated item not found`.

### Task 1.2: Implement `api_key_env_present`

**Files:**
- Modify: `crates/ironlint-core/src/llm/mod.rs`

- [ ] **Step 1.2.1: Add the helper next to `read_api_key`**

In `crates/ironlint-core/src/llm/mod.rs`, immediately above the existing `fn read_api_key`, add:

```rust
/// C1: side-effect-free probe used by `ironlint doctor` to report whether
...
```bash
git add crates/ironlint-core/src/llm/mod.rs crates/ironlint-core/tests/llm_api_key_env_present.rs
git commit -m "$(cat <<'EOF'
feat(llm): expose api_key_env_present probe (C1 phase 1)

`ironlint doctor` (C1) needs to report whether a configured `api_key_env`
...
**Files:**
- Create: `crates/ironlint-cli/tests/cli_e2e_doctor.rs`

- [ ] **Step 2.1.1: Write the bootstrap CLI test**

```rust
//! C1 — CLI integration tests for `ironlint doctor`.
//!
...
fn doctor_runs_and_reports_binary_check() {
    ...
    assert!(s.contains(env!("CARGO_PKG_VERSION")), "doctor output must include the running ironlint version: {s}");
}
```

- [ ] **Step 2.1.2: Run, expect failure**

Run: `cargo test -p ironlint-cli --test cli_e2e_doctor doctor_runs_and_reports_binary_check`
...
- Create: `crates/ironlint-cli/src/commands/doctor.rs`

- [ ] **Step 2.2.1: Add the `Doctor` variant in `cli.rs`**

In `crates/ironlint-cli/src/cli.rs`, append a variant inside `enum Command`:
...
In `crates/ironlint-cli/src/main.rs`, add the dispatch arm to the `match cli.command` block (before the closing brace):

```rust
        Command::Doctor { dir, format } => commands::doctor::run(&dir, format)?,
```

In `crates/ironlint-cli/src/commands/mod.rs`, add the new module:
...
```rust
//! C1 — `ironlint doctor` diagnostic subcommand.
//!
...
/// so the human checklist surfaces "which ironlint am I talking to".
...
Run: `cargo test -p ironlint-cli --lib commands::doctor::tests`
...
```bash
git add crates/ironlint-cli/src/cli.rs crates/ironlint-cli/src/main.rs crates/ironlint-cli/src/commands/mod.rs crates/ironlint-cli/src/commands/doctor.rs crates/ironlint-cli/tests/cli_e2e_doctor.rs
git commit -m "$(cat <<'EOF'
feat(cli): scaffold `ironlint doctor` subcommand (C1 phase 2)
...
reports the resolved ironlint path and version. Exit code is 0 when
...
fn doctor_fails_when_config_missing() {
    ...
    assert!(s.contains("ironlint init"), "remediation must hint at `ironlint init`: {s}");
}
...
fn doctor_fails_when_trust_fingerprint_broken() {
    ...
    assert!(s.contains("ironlint trust"), "remediation must hint at `ironlint trust`: {s}");
}
...
fn doctor_warns_on_legacy_schema_version_one() {
    ...
    // Doctor must surface that as a `parses` fail with a `ironlint migrate` hint.
    let dir = tempdir().unwrap();
    let home = tempdir().unwrap();
    let cfg = dir.path().join(".ironlint.yml");
    ...
    assert_eq!(out.status.code(), Some(1));
    let s = String::from_utf8_lossy(&out.stdout);
    assert!(s.contains("ironlint migrate"), "v1 remediation must hint at migrate: {s}");
}
...
Run: `cargo test -p ironlint-cli --test cli_e2e_doctor`
...
**Files:**
- Modify: `crates/ironlint-cli/src/commands/doctor.rs`
...
```rust
/// Config file present at `<dir>/.ironlint.yml`. Hard requirement; without
/// a config IronLint has nothing to do.
...
/// Schema-v1 configs fail here with a clear `ironlint migrate` hint
    ...
    match ironlint_core::config::parse_file_with_extends(&ctx.config_path) {
        ...
        Err(e) => {
            ...
            let hint = if msg.contains("schema_version 1") {
                Some("run `ironlint migrate` to upgrade `.bully.yml`/v1 config to v2".into())
            } else {
...
fn check_trust(ctx: &DoctorContext) -> CheckResult {
    ...
    match ironlint_core::trust::verify(&raw) {
        Ok(()) => CheckResult {
            name: "trust",
            status: Status::Pass,
        ...
        Err(e) => CheckResult {
            ...
            detail: format!("{e:#}"),
            remediation: Some(
                "review the diff against the last trusted state, then run `ironlint trust` to acknowledge".into(),
            ),
        },
    }
}
...
/// bully); v2 is `pass`; anything else is `fail` with a "this ironlint
/// is too old/new" hint.
fn check_schema_version(ctx: &DoctorContext) -> CheckResult {
    ...
    match ironlint_core::config::peek_schema_version(&raw) {
        Some(2) => CheckResult {
            name: "schema",
            status: Status::Pass,
        ...
        Some(1) => CheckResult {
            ...
            remediation: Some("run `ironlint migrate` to upgrade to schema_version 2".into()),
        },
        Some(n) => CheckResult {
            name: "schema",
            status: Status::Fail,
            ...
            remediation: Some(format!(
                "this ironlint supports {:?}; upgrade or downgrade ironlint to match",
                ironlint_core::config::SUPPORTED_SCHEMAS
            )),
        },
        None => CheckResult {
            name: "schema",
            ...
            remediation: Some("add `schema_version: 2` at the top of `.ironlint.yml`".into()),
        },
    }
}
```
...
```rust
    ...
    fn config_present_pass_when_file_exists() {
        ...
        fs::write(d.path().join(".ironlint.yml"), "schema_version: 2\nrules: {}\n").unwrap();
    ...
    fn schema_fail_on_v1_with_migrate_hint() {
        let d = tempdir().unwrap();
        fs::write(d.path().join(".ironlint.yml"), "schema_version: 1\nrules: {}\n").unwrap();
        let r = check_schema_version(&ctx_with(d.path()));
        assert_eq!(r.status, Status::Fail);
        assert!(r.remediation.unwrap().contains("ironlint migrate"));
    }

    #[test]
    fn schema_fail_on_unsupported_version() {
        let d = tempdir().unwrap();
        fs::write(d.path().join(".ironlint.yml"), "schema_version: 99\nrules: {}\n").unwrap();
        let r = check_schema_version(&ctx_with(d.path()));
        assert_eq!(r.status, Status::Fail);
    }
    ...
    fn schema_fail_on_missing_version() {
        let d = tempdir().unwrap();
        fs::write(d.path().join(".ironlint.yml"), "rules: {}\n").unwrap();
        let r = check_schema_version(&ctx_with(d.path()));
        assert_eq!(r.status, Status::Fail);
    }
...
parse failures route v1 configs to `ironlint migrate`; trust failures
point at `ironlint trust`. parses uses the non-trust-verifying
...
fn doctor_warns_when_semantic_rule_present_without_api_key() {
    ...
    write_trusted(
        dir.path(),
        "schema_version: 2\nllm:\n  provider: anthropic\n  model: claude\n  api_key_env: IRONLINT_DOCTOR_TEST_NO_SUCH_KEY\nrules:\n  sem:\n    description: \"x\"\n    engine: semantic\n    scope: [\"**/*.rs\"]\n    severity: warning\n    context: file\n",
    );
    let out = Command::cargo_bin("ironlint")
        .unwrap()
        .env("HOME", home.path())
        .env_remove("IRONLINT_DOCTOR_TEST_NO_SUCH_KEY")
        .args(["doctor", "--dir", dir.path().to_str().unwrap()])
        .output()
        .unwrap();
    ...
    assert!(s.contains("IRONLINT_DOCTOR_TEST_NO_SUCH_KEY"), "remediation must name the env var: {s}");
}
...
fn check_scope_globs(ctx: &DoctorContext) -> CheckResult {
    ...
    for (rule_id, rule) in &cfg.rules {
        if let Err(e) = ironlint_core::config::scope::ScopeMatcher::new(&rule.scope) {
    ...
    if bad.is_empty() {
        ...
        CheckResult {
            ...
            remediation: Some("fix the listed glob(s) and re-run `ironlint trust`".into()),
        }
    }
}
...
fn check_engines(ctx: &DoctorContext) -> CheckResult {
    ...
    let needs_llm = cfg.rules.values().any(|r| {
        matches!(
            r.engine,
            ironlint_core::config::EngineKind::Semantic | ironlint_core::config::EngineKind::Session
        )
    });
    if !needs_llm {
        return CheckResult {
...
fn llm_block_status(cfg: Option<&ironlint_core::config::LlmConfig>) -> CheckResult {
    ...
    let env_name = match llm.api_key_env.as_deref() {
        ...
        _ => {
            return CheckResult {
                ...
                remediation: Some(
                    "set `api_key_env: <NAME>` in the `llm:` block of `.ironlint.yml`".into(),
                ),
            };
        }
    };
    if ironlint_core::llm::api_key_env_present(env_name) {
        CheckResult {
            name: "engines",
            status: Status::Pass,
            detail: format!(
...
```rust
    ...
    fn engines_pass_when_no_llm_rules() {
        let d = tempdir().unwrap();
        let trusted = ironlint_core::trust::write_trust_block(
            "schema_version: 2\nrules:\n  r:\n    description: \"x\"\n    engine: script\n    scope: [\"*\"]\n    severity: error\n    script: \"true\"\n",
        ).unwrap();
        fs::write(d.path().join(".ironlint.yml"), trusted).unwrap();
    ...
    fn engines_pass_for_ollama_without_key() {
        let cfg = ironlint_core::config::LlmConfig {
    ...
    fn engines_warn_when_api_key_env_unset() {
        let cfg = ironlint_core::config::LlmConfig {
            ...
            model: "claude".into(),
            api_key_env: Some("IRONLINT_DOCTOR_TEST_DEFINITELY_UNSET_AAA".into()),
            base_url: None,
        };
        std::env::remove_var("IRONLINT_DOCTOR_TEST_DEFINITELY_UNSET_AAA");
        let r = llm_block_status(Some(&cfg));
        assert_eq!(r.status, Status::Warn);
        assert!(r.remediation.unwrap().contains("IRONLINT_DOCTOR_TEST_DEFINITELY_UNSET_AAA"));
    }
    ...
    fn engines_pass_when_api_key_env_set() {
        let cfg = ironlint_core::config::LlmConfig {
            provider: "anthropic".into(),
            model: "claude".into(),
            api_key_env: Some("IRONLINT_DOCTOR_TEST_PRESENT_KEY".into()),
            base_url: None,
        };
        std::env::set_var("IRONLINT_DOCTOR_TEST_PRESENT_KEY", "x");
        let r = llm_block_status(Some(&cfg));
        std::env::remove_var("IRONLINT_DOCTOR_TEST_PRESENT_KEY");
        assert_eq!(r.status, Status::Pass);
    }
...
Bully checks `.claude/settings.json` for a wired PostToolUse hook plus an evaluator agent. IronLint's adapter ships its own `hooks.json` under `${CLAUDE_PLUGIN_ROOT}/hooks/hook.sh`. Doctor reports advisory status: missing `~/.claude/settings.json` is `warn` (not every user runs Claude Code); present-but-no-ironlint-hook is `warn` with a remediation hint; present-and-wired is `pass`.
...
fn doctor_adapter_pass_when_hook_wired() {
    ...
    // Wire a PostToolUse hook whose command references `ironlint` so the
    // detector recognizes it without needing the real adapter installed.
    let settings = r#"{"hooks":{"PostToolUse":[{"matcher":"Edit|Write","hooks":[{"type":"command","command":"ironlint check --diff -"}]}]}}"#;
...
#[test]
fn doctor_adapter_warn_when_settings_present_but_no_ironlint_hook() {
    let dir = tempdir().unwrap();
    let home = tempdir().unwrap();
    let claude = home.path().join(".claude");
    fs::create_dir_all(&claude).unwrap();
    ...
    assert_eq!(out.status.code(), Some(0));
    let s = String::from_utf8_lossy(&out.stdout);
    assert!(s.contains("adapter") && s.contains("warn"), "expected `adapter warn` when no ironlint hook: {s}");
...
/// hook whose `command` references `ironlint` (the binary) or an IronLint
        ...
        matcher_block
            ...
            .map(|hooks| {
                hooks.iter().any(|h| {
                    h.get("command")
                        .and_then(|c| c.as_str())
                        .is_some_and(|cmd| cmd.contains("ironlint") || cmd.contains("hook.sh"))
                })
            })
            .unwrap_or(false)
    })
...
/// is `warn` (not every user runs Claude Code); present-without-ironlint
/// is `warn`; wired is `pass`. Never `fail` — ironlint is editor-agnostic
...
fn check_adapter() -> CheckResult {
    ...
    if claude_hook_wired(&settings) {
        CheckResult {
            name: "adapter",
            status: Status::Pass,
            detail: format!("Claude Code PostToolUse hook references ironlint ({})", path.display()),
            remediation: None,
        }
    } else {
        CheckResult {
            name: "adapter",
            status: Status::Warn,
            detail: format!("{} present but no PostToolUse hook references ironlint", path.display()),
            remediation: Some(
                "install the adapter or add a PostToolUse entry calling ironlint — see docs/adapters/claude-code.md".into(),
            ),
        }
    }
}
...
```rust
    #[test]
    fn hook_wired_finds_ironlint_command() {
        let v: serde_json::Value = serde_json::from_str(
            r#"{"hooks":{"PostToolUse":[{"hooks":[{"type":"command","command":"ironlint check"}]}]}}"#,
        ).unwrap();
...
hook that references `ironlint` or the adapter's `hook.sh`. Missing
file → warn (IronLint is editor-agnostic; not every user runs Claude
...
## Phase 6 — Runtime-state check (`.ironlint/` writable)
...
```rust
#[test]
fn doctor_runtime_state_pass_when_ironlint_dir_writable() {
    let dir = tempdir().unwrap();
    let home = tempdir().unwrap();
    write_trusted(
        dir.path(),
...
Run: `cargo test -p ironlint-cli --test cli_e2e_doctor doctor_runtime_state_pass_when_ironlint_dir_writable`
...
```rust
/// `<dir>/.ironlint/` is writable. Probes by creating the dir if absent,
/// writing a marker file, then deleting it. We DO create `.ironlint/` —
/// that's the same kind of side effect as `init` writing `.ironlint.yml`,
...
fn check_runtime_state(ctx: &DoctorContext) -> CheckResult {
    let ironlint_dir = ctx.dir.join(".ironlint");
    if let Err(e) = std::fs::create_dir_all(&ironlint_dir) {
        return CheckResult {
            name: "runtime_state",
            status: Status::Fail,
            detail: format!("cannot create {}: {e}", ironlint_dir.display()),
            remediation: Some(format!(
    ...
    let probe = ironlint_dir.join(".doctor-write-probe");
    if let Err(e) = std::fs::write(&probe, b"ok") {
        return CheckResult {
            name: "runtime_state",
            status: Status::Fail,
            detail: format!("cannot write to {}: {e}", ironlint_dir.display()),
            remediation: Some(format!("chmod u+w {}", ironlint_dir.display())),
        };
    }
    ...
    for name in ["baseline.json", "session.json", "log.jsonl"] {
        if let Ok(meta) = std::fs::metadata(ironlint_dir.join(name)) {
            sizes.push(format!("{name}={}b", meta.len()));
        }
    }
    let detail = if sizes.is_empty() {
        format!("{} writable (empty)", ironlint_dir.display())
    } else {
        format!("{} writable ({})", ironlint_dir.display(), sizes.join(", "))
    };
    CheckResult {
        name: "runtime_state",
        status: Status::Pass,
...
```rust
    #[test]
    fn runtime_state_pass_creates_ironlint_dir() {
        let d = tempdir().unwrap();
        let r = check_runtime_state(&ctx_with(d.path()));
        assert_eq!(r.status, Status::Pass);
        assert!(d.path().join(".ironlint").is_dir(), "ironlint dir created by probe");
    }

    #[test]
    fn runtime_state_reports_existing_state_files_sizes() {
        let d = tempdir().unwrap();
        let h = d.path().join(".ironlint");
...
### Task 7.1: Add `insta` to `ironlint-cli` dev-deps if not already present

- [ ] **Step 7.1.1: Inspect Cargo.toml**

Run: `grep insta /Users/chrisarter/Documents/projects/ironlint/crates/ironlint-cli/Cargo.toml || echo "not present"`
If not present, edit `crates/ironlint-cli/Cargo.toml` `[dev-dependencies]` to add:
...
fn doctor_json_output_snapshot_for_clean_v2_config() {
    ...
    //   - ironlint_version: changes every release
    //   - per-check `detail`: contains absolute paths and sizes
    if let Some(obj) = value.as_object_mut() {
        obj.insert("ironlint_version".into(), serde_json::Value::String("[REDACTED]".into()));
    }
...
Run: `cargo test -p ironlint-cli --test cli_e2e_doctor doctor_json_output_snapshot_for_clean_v2_config`
...
```bash
git add crates/ironlint-cli/tests/cli_e2e_doctor.rs crates/ironlint-cli/tests/snapshots/cli_e2e_doctor__doctor_json_output_snapshot_for_clean_v2_config.snap
git commit -m "$(cat <<'EOF'
...
review). Redacts ironlint_version, per-check detail, and remediation
...
```markdown
# `ironlint doctor` output schema

`ironlint doctor` is a read-only diagnostic command. It prints a checklist
of every load-time invariant IronLint cares about and exits `0` when every
...
| Field | Type | Meaning |
|---|---|---|
| `ironlint_version` | string | The version of the running `ironlint` binary. |
...
| `schema` | `schema_version` is a supported value (currently `2`). `fail` on `1` (legacy bully — remediation: `ironlint migrate`). |
...
| `adapter` | `~/.claude/settings.json` exists and a PostToolUse hook references `ironlint` or the adapter's `hook.sh`. Missing settings file is `warn` (not every user runs Claude Code). |
| `runtime_state` | `<dir>/.ironlint/` is writable (probed by writing+deleting a marker file). Reports sizes of `baseline.json`, `session.json`, `log.jsonl` if present. |
...
These are *distinct* from `ironlint check`'s `0` / `1` / `2` contract.
...
git commit -m "$(cat <<'EOF'
docs: document ironlint doctor output schema (C1 phase 8)

`docs/doctor.md` is the public contract for `ironlint doctor
...
git add README.md
git commit -m "$(cat <<'EOF'
docs(readme): list `ironlint doctor` in the CLI surface (C1 phase 8)
...
```rust
    ...
    fn runtime_state_fail_when_parent_dir_not_writable() {
        ...
        let d = tempdir().unwrap();
        // Make the parent dir read-only so create_dir_all fails on `.ironlint`.
...
```rust
    ...
    fn emit_human_does_not_crash_on_empty_checks() {
        let r = Report { ironlint_version: "0".into(), checks: vec![] };
...
```bash
git mv plans/2026-05-13-ironlint-c1-doctor.md plans/archive/2026-05-13-ironlint-c1-doctor.md
```
...
```bash
git add plans/README.md plans/archive/2026-05-13-ironlint-c1-doctor.md
git commit -m "$(cat <<'EOF'
docs(plans): archive C1 (ironlint doctor) plan
...
- [ ] **Each check has a remediation hint pointing at the relevant command or doc.**
  → Phases 3–6: `config` → `ironlint init`; `parses` (v1) → `ironlint migrate`; `trust` → `ironlint trust`; `schema` (v1) → `ironlint migrate`; `scope_globs` → fix-and-trust; `engines` → set `api_key_env` or add `llm:`; `adapter` → `docs/adapters/claude-code.md`; `runtime_state` → chmod hint.
...
- [ ] **Doctor never modifies state. Read-only.**
  → All check fns are read-only against *policy* state (configs, baselines, telemetry). `check_runtime_state` does create `<dir>/.ironlint/` if absent — within the documented contract (run state, not policy state). No `.ironlint/log.jsonl` writes anywhere in `commands::doctor`.
- [ ] **Replace bully's "Python version" with `ironlint` binary date/version vs. latest release; defer the "latest release" check to 0.3.**
...
- [x] Telemetry-schema impact: none. Doctor performs zero `.ironlint/log.jsonl` writes. Documented in Risk/rollback.
- [x] Open spec questions resolved inline: §Q4 (no phone-home; defer to 0.3), §C1 Notes (no Python-version equivalent — `binary` reports the running ironlint version instead).
...
- [x] Plan filename matches the repo convention (`plans/YYYY-MM-DD-ironlint-<id>-<slug>.md`), not the writing-plans skill default of `docs/superpowers/plans/...`.
