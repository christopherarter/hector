# IronLint 0.1a — Foundation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship a `ironlint` binary that parses `.ironlint.yml` (and `.bully.yml` with deprecation), enforces trust + per-rule capabilities, runs `script` rules, and emits the verdict JSON. No LLM, no ast-grep, no Claude Code adapter yet — those land in plans B and C.

**Architecture:** Cargo workspace with two crates. `ironlint-core` is a library: config parser, verdict types, engine trait, script engine, trust system, diff/scope/disable utilities. `ironlint-cli` is a thin binary that wires CLI subcommands (`check`, `trust`, `validate`) to the library.
...
- Create: `Cargo.toml`
- Create: `.gitignore`
- Create: `crates/ironlint-core/Cargo.toml`
- Create: `crates/ironlint-core/src/lib.rs`
- Create: `crates/ironlint-cli/Cargo.toml`
- Create: `crates/ironlint-cli/src/main.rs`
...
[workspace]
resolver = "2"
members = ["crates/ironlint-core", "crates/ironlint-cli"]
...
- [ ] **Step 1.3: Write `crates/ironlint-core/Cargo.toml`**
...
- [ ] **Step 1.4: Write `crates/ironlint-core/src/lib.rs` (stub)**
...
- [ ] **Step 1.5: Write `crates/ironlint-cli/Cargo.toml`**
...
[dependencies]
ironlint-core = { path = "../ironlint-core" }
clap.workspace = true
anyhow.workspace = true
serde_json.workspace = true
...
- [ ] **Step 1.6: Write `crates/ironlint-cli/src/main.rs` (stub)**

```rust
fn main() {
    println!("ironlint 0.1.0");
}
```
...
Run: `cargo build`
Expected: clean build of both crates, `target/debug/ironlint` exists.

- [ ] **Step 1.8: Commit**

```bash
git add Cargo.toml .gitignore crates/
git commit -m "feat: cargo workspace skeleton (ironlint-core + ironlint-cli)"
```
...
**Files:**
- Create: `crates/ironlint-core/src/verdict.rs`
- Create: `crates/ironlint-core/tests/verdict_snapshot.rs`
...
fn verdict_block_serializes_to_canonical_json() {
    let v = Verdict {
        schema_version: 1,
        ironlint_version: "0.1.0".to_string(),
        status: Status::Block,
...
Run: `cargo test -p ironlint-core --test verdict_snapshot`
Expected: FAIL with "unresolved import `ironlint_core::verdict::...`".
...
- [ ] **Step 2.4: Update `crates/ironlint-core/src/lib.rs`** (already imports verdict; verify)
...
Run again: `cargo test -p ironlint-core --test verdict_snapshot`
Expected: PASS.

- [ ] **Step 2.6: Commit**

```bash
git add crates/ironlint-core/src/verdict.rs crates/ironlint-core/src/lib.rs crates/ironlint-core/tests/
...
**Files:**
- Create: `crates/ironlint-core/src/config/mod.rs`
- Create: `crates/ironlint-core/src/config/types.rs`
- Create: `crates/ironlint-core/src/config/parser.rs`
- Modify: `crates/ironlint-core/src/lib.rs`
- Create: `tests/fixtures/valid_v2.ironlint.yml`
...
Create `crates/ironlint-core/tests/parse_v2.rs`:

```rust
use ironlint_core::config::{parse_str, EngineKind, Severity, WritesPolicy};

const V2: &str = include_str!("../../../tests/fixtures/valid_v2.ironlint.yml");
...
- [ ] **Step 3.4: Write `crates/ironlint-core/src/config/types.rs`**
...
- [ ] **Step 3.5: Write `crates/ironlint-core/src/config/parser.rs`**
...
pub fn parse_str(input: &str) -> Result<Config> {
    serde_yaml::from_str::<Config>(input).context("parsing ironlint config")
}
...
```bash
git add crates/ironlint-core/src/config/ crates/ironlint-core/src/lib.rs crates/ironlint-core/tests/parse_v2.rs tests/fixtures/valid_v2.ironlint.yml
...
**Files:**
- Modify: `crates/ironlint-core/src/config/parser.rs`
- Create: `tests/fixtures/valid_v1.bully.yml`
- Create: `crates/ironlint-core/tests/parse_v1.rs`
...
```rust
use ironlint_core::config::parse_str;
...
Run: `cargo test -p ironlint-core --test parse_v1`
...
Replace `crates/ironlint-core/src/config/parser.rs`:
...
/// Callers should log a one-time deprecation warning suggesting `ironlint migrate`.
...
```bash
git add crates/ironlint-core/src/config/parser.rs crates/ironlint-core/tests/parse_v1.rs tests/fixtures/valid_v1.bully.yml
...
**Files:**
- Create: `crates/ironlint-core/src/config/extends.rs`
- Modify: `crates/ironlint-core/src/config/mod.rs`
- Modify: `crates/ironlint-core/src/config/parser.rs`
- Create: `tests/fixtures/with_extends/base.ironlint.yml`
- Create: `tests/fixtures/with_extends/child.ironlint.yml`
- Create: `crates/ironlint-core/tests/extends.rs`
...
fn extends_merges_rules() {
    let path = Path::new("tests/fixtures/with_extends/child.ironlint.yml");
...
Expected: FAIL with "unresolved import `ironlint_core::config::parse_file_with_extends`".
...
Update `crates/ironlint-core/src/config/mod.rs`:
...
```bash
git add crates/ironlint-core/src/config/ crates/ironlint-core/tests/extends.rs tests/fixtures/with_extends/
...
**Files:**
- Create: `crates/ironlint-core/src/config/scope.rs`
- Modify: `crates/ironlint-core/src/config/mod.rs`
- Create: `crates/ironlint-core/tests/scope.rs`
...
Run: `cargo test -p ironlint-core --test scope`
...
Add to `crates/ironlint-core/src/config/mod.rs`:
...
```bash
git add crates/ironlint-core/src/config/scope.rs crates/ironlint-core/src/config/mod.rs crates/ironlint-core/tests/scope.rs
...
**Files:**
- Create: `crates/ironlint-core/src/diff/mod.rs`
- Create: `crates/ironlint-core/src/diff/parser.rs`
- Modify: `crates/ironlint-core/src/lib.rs`
- Create: `crates/ironlint-core/tests/diff_parse.rs`
...
```bash
git add crates/ironlint-core/src/diff/ crates/ironlint-core/src/lib.rs crates/ironlint-core/tests/diff_parse.rs
...
`// ironlint-disable: rule-id reason: <text>` on a line silences that rule for that line.

**Files:**
- Create: `crates/ironlint-core/src/disable.rs`
- Modify: `crates/ironlint-core/src/lib.rs`
- Create: `crates/ironlint-core/tests/disable.rs`
...
const SOURCE: &str = "\
let x = 1;
eval(expr); // ironlint-disable: no-eval reason: sandboxed input
console.log('hi'); /* ironlint-disable: no-console-log reason: debug only */
let y = 2;
";
...
Run: `cargo test -p ironlint-core --test disable`
...
fn parse_disable_directives(line: &str) -> Vec<String> {
    // ironlint-disable: <rule-id> [reason: <free text>]
    let mut out = Vec::new();
    for marker in ["ironlint-disable:", "ironlint-disable:".to_string().as_str()] {
        let mut rest = line;
...
```bash
git add crates/ironlint-core/src/disable.rs crates/ironlint-core/src/lib.rs crates/ironlint-core/tests/disable.rs
git commit -m "feat: ironlint-disable comment scanner"
```
...
`ironlint trust` writes a sha256 of the canonicalized config (sorted keys, comments stripped, trust block excluded) into the `trust:` block. Subsequent runs re-compute and compare.

**Files:**
- Create: `crates/ironlint-core/src/trust.rs`
- Modify: `crates/ironlint-core/src/lib.rs`
- Create: `crates/ironlint-core/tests/trust.rs`
...
```rust
use ironlint_core::trust::{canonicalize_for_fingerprint, fingerprint, verify};
...
Run: `cargo test -p ironlint-core --test trust`
Expected: FAIL (unresolved).
...
pub fn verify(input: &str) -> Result<()> {
    ...
    let recorded = value
        ...
        .and_then(|f| f.as_str())
        .ok_or_else(|| anyhow!("trust block missing or empty; run `ironlint trust`"))?
        .to_string();
    ...
    if recorded == expected {
        ...
        Err(anyhow!(
            "config changed since last trust — review changes and run `ironlint trust` to acknowledge"
        ))
    }
}
...
```bash
git add crates/ironlint-core/src/trust.rs crates/ironlint-core/src/lib.rs crates/ironlint-core/tests/trust.rs
...
**Files:**
- Create: `crates/ironlint-core/src/engine/mod.rs`
- Create: `crates/ironlint-core/src/engine/capability.rs`
- Modify: `crates/ironlint-core/src/lib.rs`
- Create: `crates/ironlint-core/tests/capability.rs`
...
use ironlint_core::engine::capability::{run_with_capabilities, ExecOutcome};
use std::path::PathBuf;
...
fn run_best_effort(cmd: &str, cwd: &Path, caps: &Capabilities) -> Result<ExecOutcome> {
    ...
    if !caps.network || !matches!(caps.writes, WritesPolicy::Unrestricted) {
        eprintln!(
            "ironlint: capability enforcement is best-effort on this platform (see docs/security.md); running command unrestricted"
        );
    }
...
```markdown
# IronLint — Capability Enforcement
...
macOS sandbox profile integration is tracked in `specs/2026-05-11-ironlint-plan-and-0.1-design.md` §13 (risks). Re-evaluated at 1.0.
```

- [ ] **Step 10.7: Commit**

```bash
git add crates/ironlint-core/src/engine/ crates/ironlint-core/src/lib.rs crates/ironlint-core/tests/capability.rs docs/security.md
...
**Files:**
- Create: `crates/ironlint-core/src/engine/script.rs`
- Modify: `crates/ironlint-core/src/engine/mod.rs`
- Create: `crates/ironlint-core/tests/script_engine.rs`
...
```rust
use ironlint_core::config::{Capabilities, Rule, EngineKind, Severity, WritesPolicy};
...
fn failing_script_produces_violation() {
    ...
    assert_eq!(v.severity, ironlint_core::verdict::Severity::Error);
    assert_eq!(v.engine, ironlint_core::verdict::Engine::Script);
...
Run: `cargo test -p ironlint-core --test script_engine`
Expected: FAIL.
...
Update `crates/ironlint-core/src/engine/mod.rs`:
...
```bash
git add crates/ironlint-core/src/engine/ crates/ironlint-core/tests/script_engine.rs
...
**Files:**
- Create: `crates/ironlint-core/src/runner.rs`
- Modify: `crates/ironlint-core/src/lib.rs`
- Create: `crates/ironlint-core/tests/runner.rs`
...
fn passing_rule_yields_pass_verdict() {
    ...
    let engine = IronLintEngine::load(&cfg_path).expect("load");
...
fn untrusted_config_fails_to_load() {
    ...
    let result = IronLintEngine::load(&cfg_path);
...
```bash
git add crates/ironlint-core/src/runner.rs crates/ironlint-core/src/lib.rs crates/ironlint-core/tests/runner.rs
git commit -m "feat: IronLintEngine runner with trust + scope + script engine"
```
...
Wire up the `ironlint` binary with `clap` subcommands.

**Files:**
- Create: `crates/ironlint-cli/src/cli.rs`
- Create: `crates/ironlint-cli/src/commands/mod.rs`
- Modify: `crates/ironlint-cli/src/main.rs`
- Create: `crates/ironlint-cli/tests/cli_version.rs`
...
fn version_flag_prints_version() {
    ...
    cmd.arg("--version").assert().success().stdout(predicates::str::contains("ironlint "));
}
...
Run: `cargo test -p ironlint-cli --test cli_version`
Expected: FAIL (binary still prints "ironlint 0.1.0" from stub; --version not supported via clap).
...
#[derive(Debug, Parser)]
#[command(name = "ironlint", version, about = "Policy-enforcement pipeline for AI coding agents")]
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
}
...
Create `crates/ironlint-cli/src/commands/check.rs`:
...
Create `crates/ironlint-cli/src/commands/trust.rs`:
...
Create `crates/ironlint-cli/src/commands/validate.rs`:
...
## Task 14: `ironlint validate`

Loads + parses the config. Reports schema violations. Skips trust check (validate is also used before trust is set).

**Files:**
- Modify: `crates/ironlint-cli/src/commands/validate.rs`
- Create: `crates/ironlint-cli/tests/cli_validate.rs`
...
Run: `cargo test -p ironlint-cli --test cli_validate`
Expected: FAIL (validate is a stub).

- [ ] **Step 14.3: Implement validate**

Replace `crates/ironlint-cli/src/commands/validate.rs`:
...
```bash
git add crates/ironlint-cli/src/commands/validate.rs crates/ironlint-cli/tests/cli_validate.rs
git commit -m "feat: ironlint validate command"
```

---

## Task 15: `ironlint trust`
...
- Create: `crates/ironlint-cli/tests/cli_trust.rs`
...
fn trust_then_verify_round_trip() {
    ...
    Command::cargo_bin("ironlint")
        ...
        .assert()
        .success();
    // After trust, the config should be loadable through IronLintEngine.
    let written = std::fs::read_to_string(&cfg).unwrap();
    ironlint_core::trust::verify(&written).expect("verify after trust");
}
```

- [ ] **Step 15.2: Run test to verify failure**

Run: `cargo test -p ironlint-cli --test cli_trust`
Expected: FAIL.

- [ ] **Step 15.3: Implement trust command**

Replace `crates/ironlint-cli/src/commands/trust.rs`:
...
```bash
git add crates/ironlint-cli/src/commands/trust.rs crates/ironlint-cli/tests/cli_trust.rs
git commit -m "feat: ironlint trust command"
```

---

## Task 16: `ironlint check`

Loads config, runs `IronLintEngine::check`, emits JSON or human output, returns exit code per the contract (0/1/2).

**Files:**
- Modify: `crates/ironlint-cli/src/commands/check.rs`
- Create: `crates/ironlint-cli/tests/cli_check.rs`
...
Run: `cargo test -p ironlint-cli --test cli_check`
Expected: FAIL.

- [ ] **Step 16.3: Implement check**

Replace `crates/ironlint-cli/src/commands/check.rs`:
...
impl SeverityHuman for ironlint_core::verdict::Violation {
    fn severity_human(&self) -> &'static str {
        match self.severity {
            ironlint_core::verdict::Severity::Error => "error",
            ironlint_core::verdict::Severity::Warning => "warn",
        }
    }
}
```
...
```bash
git add crates/ironlint-cli/src/commands/check.rs crates/ironlint-cli/tests/cli_check.rs
git commit -m "feat: ironlint check command with exit-code contract"
```
...
Drive the whole pipeline from outside: create a project dir, scaffold a `.ironlint.yml`, trust it, run check, assert verdict.

**Files:**
- Create: `crates/ironlint-core/tests/e2e_script_rules.rs`
...
```bash
git add crates/ironlint-core/tests/e2e_script_rules.rs
...
- [`specs/overview.md`](specs/overview.md) — IronLint at 1.0
- [`specs/2026-05-11-ironlint-plan-and-0.1-design.md`](specs/2026-05-11-ironlint-plan-and-0.1-design.md) — plan + 0.1 design
```
...
Trust it (review the config first — `ironlint` runs the scripts in it):
...
```bash
ironlint check --file src/foo.rs
```
...
```bash
git tag -a ironlint-0.1a-foundation -m "0.1a foundation: script engine, trust, capability gate, CLI check/trust/validate"
```
...
This plan covers spec items: §4.1 (ironlint-core surface — partial; engines beyond script and the LlmClient trait land in plan B), §4.2 (CLI — partial: check/trust/validate done; init/migrate/baseline in plan B), §5 (schema v2 + v1 compat — done), §7 (trust — done), §6/§8 (verdict JSON — done), §9.1 (script engine — done).
...
- `ironlint init` / `migrate` / `baseline`
- Claude Code adapter
- Telemetry log writing (`.ironlint/log.jsonl`)
