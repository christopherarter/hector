# IronLint A3 — Diff Pre-Filter for Semantic Engine
...
**Sequencing:** Third item in the 0.2.0 cohort (after [A1](2026-05-12-ironlint-a1-prompt-injection.md), [A2](2026-05-12-ironlint-a2-skip-patterns.md)).
...
**Files:**
- Create: `crates/ironlint-core/tests/diff_analysis.rs`
...
```rust
use ironlint_core::diff::analysis::{can_match_diff, CanMatch, SkipReason};
use std::path::Path;
...
Expected: compile error — unresolved import `ironlint_core::diff::analysis`.

### Task 2: Implement the analysis module

**Files:**
- Create: `crates/ironlint-core/src/diff/analysis.rs`
- Modify: `crates/ironlint-core/src/diff/mod.rs:1-3` (add `pub mod analysis;`)
- Possibly modify: `crates/ironlint-core/Cargo.toml` (add `regex` to `[dependencies]` if not already a direct dep)

- [x] **Step 1: Confirm `regex` availability**

Run: `cargo tree -p ironlint-core --depth 1 | grep -i regex`

If absent from direct deps, run: `cargo add -p ironlint-core regex` (no version pin — use whatever the workspace converges on).

- [x] **Step 2: Add module declaration**

Edit `crates/ironlint-core/src/diff/mod.rs`:
...
```bash
git add crates/ironlint-core/src/diff/mod.rs \
        crates/ironlint-core/src/diff/analysis.rs \
        crates/ironlint-core/tests/diff_analysis.rs \
...
**Files:**
- Modify: `crates/ironlint-core/src/telemetry.rs:7-15`
- Modify: `crates/ironlint-core/tests/telemetry.rs` (one new test case)
...
Append to `crates/ironlint-core/tests/telemetry.rs`:
...
Edit `crates/ironlint-core/src/telemetry.rs`:
...
Edit `crates/ironlint-core/src/runner.rs` lines 188, 313, 393 (the three telemetry appends): add `reason: None,` at the end of each struct literal.

Also fix the existing tests in `crates/ironlint-core/tests/telemetry.rs` that construct `LogEntry` (`append_creates_log_and_writes_jsonl` and any others) — add `reason: None,` to those literals.
...
```bash
git add crates/ironlint-core/src/telemetry.rs \
        crates/ironlint-core/src/runner.rs \
        crates/ironlint-core/tests/telemetry.rs
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs` (the per-rule loop, ~lines 209–282)
...
Open `crates/ironlint-core/src/runner.rs` around lines 200–290 to verify the exact shape before editing.
...
**Files:**
- Create: `crates/ironlint-core/tests/runner_semantic_prefilter.rs`
...
The real public surface (verified against `crates/ironlint-core/src/runner.rs:87-89` and `tests/runner_diff.rs`) is `IronLintEngine::check(CheckInput::Diff { file, unified_diff })` and `trust::write_trust_block(raw_yaml)` for trust-stamping a config inline.
...
```bash
git add crates/ironlint-core/tests/runner_semantic_prefilter.rs
...
| `tests/runner_semantic_prefilter.rs` | End-to-end: counting LLM client confirms zero `evaluate` calls for a skipped diff; one call for a real addition; `kind: "semantic_skipped"` record lands in `.ironlint/log.jsonl` with the right reason and rule_id. |
