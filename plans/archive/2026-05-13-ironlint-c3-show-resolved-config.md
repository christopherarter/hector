# IronLint C3 — `ironlint show-resolved-config`
...
**NEW:**
- `crates/ironlint-cli/src/commands/show_resolved_config.rs` — the subcommand implementation: `pub fn run(config: &Path, format: ShowFormat) -> Result<i32>`, three private formatters, one private `ResolvedView` shape, plus unit tests for each formatter.
- `crates/ironlint-cli/tests/cli_e2e_show_resolved_config.rs` — `assert_cmd` + `tempfile` integration tests for all three formats; `insta` snapshots for the canonical outputs; tests for the missing-config and invalid-format error paths; multi-level `extends:` test asserting inheritance + override + origin.
...
**MODIFIED:**
- `crates/ironlint-cli/src/cli.rs` — new `Command::ShowResolvedConfig { config, format }` variant plus a new `ShowFormat { Tsv, Yaml, Json }` `ValueEnum`.
- `crates/ironlint-cli/src/main.rs` — dispatch arm that calls `commands::show_resolved_config::run`.
- `crates/ironlint-cli/src/commands/mod.rs` — `pub mod show_resolved_config;`.
- `crates/ironlint-core/src/config/extends.rs` — add `pub fn resolve_with_origin(root: &Path) -> Result<(Config, BTreeMap<String, PathBuf>)>` and a private `resolve_inner_with_origin` walker that records every rule's source file. The existing `resolve` / `resolve_trusted` / `resolve_inner` are unchanged.
...
- `crates/ironlint-core/src/config/types.rs` — `Rule`, `Config`, `EngineKind`, `Severity` shapes are stable; this plan does **not** add fields to any of them.
- `crates/ironlint-core/src/runner.rs` — `IronLintEngine::load` is the trust-gated path used by `check`. Show-resolved-config bypasses it deliberately (read-only, doesn't run rules).
- `crates/ironlint-core/src/verdict.rs` — verdict shape is locked; this command never emits a verdict.
...
**Files:**
- Create: `crates/ironlint-core/tests/extends_origin.rs`
...
Run: `cargo test -p ironlint-core --test extends_origin`
...
**Files:**
- Modify: `crates/ironlint-core/src/config/extends.rs`
...
fn resolve_inner_with_origin(
    ...
    if matches!(super::parser::peek_schema_version(&content), Some(1)) {
        return Err(anyhow!(
            "{} is schema_version 1 (legacy bully); run `ironlint migrate` to upgrade to schema_version 2",
            canonical.display()
        ));
    }
...
```bash
git add crates/ironlint-core/src/config/extends.rs crates/ironlint-core/tests/extends_origin.rs
...
**Files:**
- Create: `crates/ironlint-cli/tests/cli_e2e_show_resolved_config.rs`

- [ ] **Step 7: Write the failing TSV integration test**

```rust
//! C3 — `ironlint show-resolved-config` end-to-end coverage.
...
fn tsv_default_emits_id_engine_severity_scope_fix_hint_origin() {
    ...
    assert!(
        cols0[5].ends_with(".ironlint.yml"),
    ...
    assert!(cols1[5].ends_with(".ironlint.yml"));
}
...
Run: `cargo test -p ironlint-cli --test cli_e2e_show_resolved_config tsv_default_emits_id_engine_severity_scope_fix_hint_origin`
...
In `crates/ironlint-cli/src/cli.rs`, append a new variant inside `Command` (between `Session` and the closing brace) **and** a new `ShowFormat` enum at the bottom:
...
Create `crates/ironlint-cli/src/commands/show_resolved_config.rs` with the bare-minimum scaffold (so the next task's implementation step is purely additive — no fighting the borrow checker on a half-typed module):

```rust
//! C3 — `ironlint show-resolved-config`. Print the post-extends merged
...
pub fn run(config: &Path, format: ShowFormat) -> Result<i32> {
    match ironlint_core::config::extends::resolve_with_origin(config) {
        Ok((cfg, origins)) => {
...
fn format_tsv(
    _cfg: &ironlint_core::config::Config,
...
**Files:**
- Modify: `crates/ironlint-cli/src/commands/show_resolved_config.rs`
...
```rust
fn format_tsv(
    cfg: &ironlint_core::config::Config,
...
fn sorted_rules(
    cfg: &ironlint_core::config::Config,
) -> Vec<(&String, &ironlint_core::config::Rule)> {
    let mut v: Vec<(&String, &ironlint_core::config::Rule)> = cfg.rules.iter().collect();
    v.sort_by(|a, b| a.0.cmp(b.0));
    v
}

fn engine_kind_str(k: ironlint_core::config::EngineKind) -> &'static str {
    match k {
        ironlint_core::config::EngineKind::Script => "script",
        ironlint_core::config::EngineKind::Ast => "ast",
        ironlint_core::config::EngineKind::Semantic => "semantic",
        ironlint_core::config::EngineKind::Session => "session",
    }
}

fn severity_str(s: ironlint_core::config::Severity) -> &'static str {
    match s {
        ironlint_core::config::Severity::Error => "error",
        ironlint_core::config::Severity::Warning => "warning",
    }
}
```
...
Run: `cargo test -p ironlint-cli --test cli_e2e_show_resolved_config tsv_extends_chain_inherits_and_overrides_with_origin`
Expected: PASS.

- [ ] **Step 16: Commit**

```bash
git add crates/ironlint-cli/src/cli.rs crates/ironlint-cli/src/main.rs crates/ironlint-cli/src/commands/mod.rs crates/ironlint-cli/src/commands/show_resolved_config.rs crates/ironlint-cli/tests/cli_e2e_show_resolved_config.rs
git commit -m "$(cat <<'EOF'
feat(cli): add show-resolved-config TSV default (C3 phase 1)

`ironlint show-resolved-config` prints the post-extends merged rule set
...
**Files:**
- Modify: `crates/ironlint-cli/tests/cli_e2e_show_resolved_config.rs`
...
fn yaml_format_emits_canonical_merged_config_without_trust() {
    ...
    assert!(stdout.contains(".ironlint.yml"));
}
...
Run: `cargo test -p ironlint-cli --test cli_e2e_show_resolved_config yaml_format_emits_canonical_merged_config_without_trust`
...
struct ResolvedView<'a> {
    ...
    llm: Option<&'a ironlint_core::config::LlmConfig>,
    #[serde(skip_serializing_if = "Vec::is_empty")]
    skip: &'a Vec<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    execution: Option<&'a ironlint_core::config::ExecutionConfig>,
...
struct RuleView<'a> {
    #[serde(flatten)]
    rule: &'a ironlint_core::config::Rule,
    origin: String,
}

fn build_view<'a>(
    cfg: &'a ironlint_core::config::Config,
...
Run: `cargo test -p ironlint-cli --test cli_e2e_show_resolved_config yaml_format_origin_comment_precedes_each_rule`
Expected: PASS.

- [ ] **Step 21: Commit**

```bash
git add crates/ironlint-cli/src/commands/show_resolved_config.rs crates/ironlint-cli/tests/cli_e2e_show_resolved_config.rs
git commit -m "$(cat <<'EOF'
feat(cli): show-resolved-config --format yaml (C3 phase 2)

`ironlint show-resolved-config --format yaml` emits the canonical merged
...
fn json_format_emits_sorted_rules_with_origin_field() {
    ...
    assert!(alpha["origin"].as_str().unwrap().ends_with(".ironlint.yml"));
...
Run: `cargo test -p ironlint-cli --test cli_e2e_show_resolved_config json_format_emits_sorted_rules_with_origin_field`
...
### Task 5.1: Missing `.ironlint.yml` exits 1 with a hint on stderr
...
fn missing_config_exits_one_with_hint() {
    let dir = tempdir().unwrap();
    // No `.ironlint.yml` written; the path doesn't exist.
...
Run: `cargo test -p ironlint-cli --test cli_e2e_show_resolved_config missing_config_exits_one_with_hint`
...
Run: `cargo test -p ironlint-cli --test cli_e2e_show_resolved_config invalid_format_value_is_rejected_by_clap`
Expected: PASS — clap's `ValueEnum` handles this for free.

- [ ] **Step 31: Commit**

```bash
git add crates/ironlint-cli/tests/cli_e2e_show_resolved_config.rs
git commit -m "$(cat <<'EOF'
test(cli): cover show-resolved-config error paths (C3 phase 4)

Two new integration tests:
- missing `.ironlint.yml` exits 1 with the file path on stderr,
...
mod tests {
    ...
    fn rule_for(scope: Vec<&str>, fix_hint: Option<&str>) -> ironlint_core::config::Rule {
        ironlint_core::config::Rule {
            description: "x".into(),
    ...
    fn cfg_with(rules: Vec<(&str, ironlint_core::config::Rule)>) -> ironlint_core::config::Config {
        let mut map = std::collections::BTreeMap::new();
        for (id, r) in rules {
            map.insert(id.to_string(), r);
        }
        ironlint_core::config::Config {
            schema_version: 2,
            llm: None,
            extends: vec![],
            trust: None,
    ...
    fn tsv_emits_six_tab_separated_columns_per_row() {
        ...
        assert_eq!(
            lines[0],
            "alpha\tscript\twarning\t*.rs,*.txt\thint\t/path/to/.ironlint.yml"
        );
    ...
    fn yaml_strips_trust_and_extends_and_inserts_origin_comments() {
        ...
        cfg.trust = Some(ironlint_core::config::TrustBlock {
            fingerprint: "deadbeef".into(),
        });
        cfg.extends = vec!["should-not-leak.yml".into()];
        let origins = origins_for(vec![("alpha", "/path/to/.ironlint.yml")]);
        ...
        assert!(out.contains("# origin: /path/to/.ironlint.yml"));
    ...
    fn json_serializes_rules_sorted_by_id_with_origin() {
        ...
        assert_eq!(v["rules"]["alpha"]["origin"], "/a/.ironlint.yml");
    ...
    fn yaml_origin_comment_only_inside_rules_block() {
        ...
        let origins = origins_for(vec![("alpha", "/p/.ironlint.yml")]);
        let out = format_yaml(&cfg, &origins).unwrap();
...
Run: `cargo test -p ironlint-cli --lib show_resolved_config`
Expected: All four unit tests PASS. (They live in the binary crate's lib-style `mod tests`; clap doesn't get involved.)

- [ ] **Step 34: Commit**

```bash
git add crates/ironlint-cli/src/commands/show_resolved_config.rs
git commit -m "$(cat <<'EOF'
...
file it was *defined in* — your local `.ironlint.yml`, an
...
```bash
ironlint show-resolved-config | cut -f1,2,6     # ids + engine + origin
ironlint show-resolved-config | grep semantic   # all semantic rules
```
...
- `ironlint show-resolved-config [--format tsv|yaml|json]` — print the post-`extends:` merged rule set, with each rule annotated by the file that defined it. See [docs/show-resolved-config.md](docs/show-resolved-config.md).
```
...
- `crates/ironlint-core/src/config/extends.rs` — the new walker has three new functions; the existing tests in `extends_origin.rs` cover both the single-file and the multi-level paths. If coverage on `merge_inherited_with_origin` drops below 80%, add an extends-chain test where the inherited config's `llm:` and `skip:` fields are populated and the local doesn't define them, which exercises both non-rules merge branches.
- `crates/ironlint-cli/src/commands/show_resolved_config.rs` — the `#[cfg(test)] mod tests` covers all three formatters and the YAML annotator's rule-key-vs-body discrimination. If coverage gaps remain on `run`'s `Err` arm, add a unit test that constructs a non-existent `Path` and asserts `run` returns `Ok(1)` with the formatter never called.
- `crates/ironlint-cli/src/cli.rs` — clap derive only; no new logic to cover.
- `crates/ironlint-cli/src/main.rs` — dispatch arm only; covered by every integration test that exercises the new subcommand.
- `crates/ironlint-cli/src/commands/mod.rs` — module-list line; trivial.

### Task 8.3: Plan archive

- [ ] **Step 41: When the PR merges**

Move this plan to `plans/archive/2026-05-13-ironlint-c3-show-resolved-config.md` and update `plans/README.md`'s index.
...
- [x] Plan filename matches repo convention `plans/YYYY-MM-DD-ironlint-<id>-<slug>.md` (`plans/2026-05-13-ironlint-c3-show-resolved-config.md`), not the writing-plans skill default.
