# IronLint H2 — `ironlint record-verdict` subcommand
...
**Goal:** Add `ironlint record-verdict --rule <id> --verdict <pass|violation> [--file <path>] [--dir <path>]` — a tiny CLI subcommand that appends one `LogEntry::SemanticVerdict` record to `.ironlint/log.jsonl`. Consumed by the Claude Code adapter's interpreter skill (H3) after it parses subagent output, so coverage reports (`ironlint coverage`, D2) reflect subagent-evaluated rules instead of treating them as dead. No new core surface; reuses the existing `telemetry::append` + `LogEntry::SemanticVerdict` shipped in D1.

**Architecture:** New file `crates/ironlint-cli/src/commands/record_verdict.rs` owns a single `pub fn run(rule: String, verdict: VerdictValue, file: Option<String>, dir: &Path) -> Result<i32>`. The clap-side enum `VerdictValue { Pass, Violation }` enforces the two-value constraint at parse time so the runtime body cannot see an invalid value. The function: resolves `dir` to `.ironlint/log.jsonl`, builds a `LogEntry::SemanticVerdict { ts: now_rfc3339(), rule, verdict, file }`, calls `telemetry::append`, returns `0` on success or `1` on disk failure. Never returns `2` — this command is not a gate.
...
| `--dir` default | `.` (cwd). Same convention as `init` / `doctor` / `explain`. | Locates `.ironlint/log.jsonl` for tests via a tempdir. |
| `.ironlint/` auto-create | Yes, `telemetry::append` already calls `std::fs::create_dir_all(parent)?` so a missing `.ironlint/` directory is created on first append. The first append in a fresh project does not require a prior `ironlint init` or `ironlint session start`. | Matches existing `telemetry::append` behaviour; see `crates/ironlint-core/src/telemetry.rs::append`. |
...
| Auth model | None. No HMAC, no nonce. The trust model is unchanged: anyone who can run `ironlint record-verdict` can also append to `.ironlint/log.jsonl` directly. | Spec §H2 Notes. Auth would be theater since it lives on the same machine as the log. |
| `session_init` lazy stamp | Reuse the same lazy `SessionInit` write `ironlint session record` performs (`crates/ironlint-cli/src/commands/session.rs`). The first `record-verdict` in a session writes a `SessionInit` record before its `SemanticVerdict`. Operators expect the session log to start with `session_init`; no need for a separate `ironlint session start` precondition. | Mirrors `session record`'s D1 behaviour. The `read_all` lifter doesn't synthesize `SessionInit` from `SemanticVerdict` alone, so without the lazy stamp the legacy fallback path would emit a deprecation warning for fresh logs. |
| Trust gate | **Skipped.** This subcommand does not load `.ironlint.yml`. No trust check, no fingerprint verify. | The command writes telemetry; it does not consult rule definitions. Trust is only required for paths that *execute* rule logic. |
...
**Config-schema impact.** None. Does not read `.ironlint.yml`.

**Performance.** One file open + one `write_all` + one fsync (via `telemetry::append`). Under 5ms on any healthy filesystem.

**Rollback.** Pure addition. Removing the `RecordVerdict` variant, the dispatch arm, the command module, the test, and the docs entry restores the prior CLI surface. The on-disk `.ironlint/log.jsonl` records written via this command remain valid `SemanticVerdict` entries and continue to parse via `telemetry::read_all`.
...
**Files:**
- Create: `crates/ironlint-cli/tests/cli_e2e_record_verdict.rs`

- [ ] **Step 1.1.1: Write the failing test**

```rust
//! H2 — end-to-end coverage that `ironlint record-verdict` appends a
//! `SemanticVerdict` line to `.ironlint/log.jsonl`.
...
Run: `cargo test -p ironlint-cli --test cli_e2e_record_verdict record_verdict_subcommand_is_recognised`
Expected: FAIL — `unrecognized subcommand 'record-verdict'`.

### Task 1.2: Add the clap variant and dispatch skeleton

**Files:**
- Modify: `crates/ironlint-cli/src/cli.rs` (add a `RecordVerdict` variant to the `Command` enum)
- Modify: `crates/ironlint-cli/src/commands/mod.rs` (add `pub mod record_verdict;`)
- Create: `crates/ironlint-cli/src/commands/record_verdict.rs` (stub `run`)
- Modify: `crates/ironlint-cli/src/main.rs` (dispatch the new variant)
...
```rust
    /// Append one `semantic_verdict` record to `.ironlint/log.jsonl`.
    ///
    ...
    RecordVerdict {
        ...
        #[arg(long = "file")]
        file: Option<String>,
        /// Directory containing `.ironlint/log.jsonl`. Defaults to cwd.
...
`crates/ironlint-cli/src/commands/record_verdict.rs`:

```rust
//! H2: `ironlint record-verdict` — append a single `SemanticVerdict`
//! record to `.ironlint/log.jsonl`. Consumed by the Claude Code
...
`crates/ironlint-cli/src/commands/mod.rs`:
...
Find the existing `match cli.command` block in `main.rs` (`grep -n 'match cli.command' crates/ironlint-cli/src/main.rs`). Add the arm:
...
Run: `cargo test -p ironlint-cli --test cli_e2e_record_verdict`
Expected: PASS, both tests (the subcommand is recognised, invalid verdict is parse-rejected).

- [ ] **Step 1.2.6: Commit**

```bash
git add crates/ironlint-cli/src/cli.rs crates/ironlint-cli/src/main.rs crates/ironlint-cli/src/commands/mod.rs crates/ironlint-cli/src/commands/record_verdict.rs crates/ironlint-cli/tests/cli_e2e_record_verdict.rs
...
**Files:**
- Modify: `crates/ironlint-cli/tests/cli_e2e_record_verdict.rs` (replace the Phase 1 placeholder test with a real assertion)
...
fn record_verdict_appends_one_semantic_verdict_line() {
    ...
    let log_path = tmp.path().join(".ironlint/log.jsonl");
    assert!(log_path.exists(), ".ironlint/log.jsonl must be created");
...
fn record_verdict_with_no_file_omits_field() {
    ...
    let content = fs::read_to_string(tmp.path().join(".ironlint/log.jsonl")).unwrap();
    let line = content
        .lines()
...
fn record_verdict_writes_session_init_lazily() {
    ...
    // record before its semantic_verdict, so `ironlint coverage` (D2) and
...
**Files:**
- Modify: `crates/ironlint-cli/src/commands/record_verdict.rs` (replace the stub `run` body)
...
pub fn run(
    ...
    let log_path = dir.join(".ironlint/log.jsonl");
    ...
    // Idempotent in the same sense as `ironlint session record` — we check
    ...
    if !log_path.exists() || std::fs::metadata(&log_path).map(|m| m.len() == 0).unwrap_or(true) {
        let init = ironlint_core::telemetry::LogEntry::SessionInit {
            ts: chrono::Utc::now().to_rfc3339(),
            ironlint_version: env!("CARGO_PKG_VERSION").to_string(),
            schema_version: ironlint_core::telemetry::SCHEMA_VERSION,
        };
        if let Err(e) = ironlint_core::telemetry::append(&log_path, &init) {
            eprintln!("ERROR: failed to write session_init: {e:#}");
            return Ok(1);
        }
    }

    let entry = ironlint_core::telemetry::LogEntry::SemanticVerdict {
        ts: chrono::Utc::now().to_rfc3339(),
        rule,
    ...
    if let Err(e) = ironlint_core::telemetry::append(&log_path, &entry) {
...
Verify the exact `SessionInit` variant field names against `crates/ironlint-core/src/telemetry.rs` — the spec uses `ironlint_version` and `schema_version` per D1's wire format. If the variant differs (e.g. takes more fields), match the actual definition; the spec confirmed shape was `session_init` carrying `ts` / `ironlint_version` / `schema_version`. Adjust the `telemetry::SCHEMA_VERSION` constant reference to match the actual public name (could be `SCHEMA_VERSION` or `TELEMETRY_SCHEMA_VERSION` — `grep -n 'pub const' crates/ironlint-core/src/telemetry.rs`).
...
```bash
git add crates/ironlint-cli/src/commands/record_verdict.rs crates/ironlint-cli/tests/cli_e2e_record_verdict.rs
...
.ironlint/log.jsonl, lazily writing session_init on the first call
...
**Files:**
- Modify: `crates/ironlint-cli/tests/cli_e2e_record_verdict.rs` (append a test)
...
Run: `cargo test -p ironlint-cli --test cli_e2e_record_verdict record_verdict_returns_1_on_telemetry_write_failure`
...
```bash
git add crates/ironlint-cli/tests/cli_e2e_record_verdict.rs
...
```markdown
# `ironlint record-verdict`

Adapter-internal subcommand. Appends one `semantic_verdict` record to
`.ironlint/log.jsonl` so subagent-evaluated rules show up in coverage
reports (`ironlint coverage`, D2) instead of looking dead.
...
\`\`\`
ironlint record-verdict --rule <id> --verdict <pass|violation> [--file <path>] [--dir <path>]
\`\`\`
...
| `--dir` | no | `.` | Directory containing `.ironlint/log.jsonl`. Created if it doesn't exist. |
...
The first invocation against a fresh `.ironlint/log.jsonl` stamps a
...
`ironlint record-verdict` can also write to `.ironlint/log.jsonl` directly.
...
git commit -m "docs(h2): document ironlint record-verdict adapter-internal contract"
```
...
git commit -m "docs(changelog): record H2 ironlint record-verdict subcommand"
```

### Task 5.2: Archive the plan

**Files:**
- Move: `plans/2026-05-14-ironlint-h2-record-verdict.md` → `plans/archive/`
- Modify: `plans/README.md` (move H2 from the Future section to Archive)

- [ ] **Step 5.2.1: Move the plan**

```bash
git mv plans/2026-05-14-ironlint-h2-record-verdict.md plans/archive/
```
...
|---|---|
| `ironlint record-verdict --rule r1 --verdict pass --file foo.rs` appends one valid `semantic_verdict` line | `cli_e2e_record_verdict::record_verdict_appends_one_semantic_verdict_line` (Phase 2) |
...
| Running in a directory with no `.ironlint/` initializes it (mirrors `commands/check.rs`) | Covered implicitly by every Phase-2 test using a fresh `tempdir()`. Add an explicit assertion in Phase 2's `record_verdict_appends_one_semantic_verdict_line` test (`assert!(log_path.exists())`) — already present in the test above. |
