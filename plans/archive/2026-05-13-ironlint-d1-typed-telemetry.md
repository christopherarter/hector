# IronLint D1 — Typed Telemetry Records Implementation Plan
...
Replace the single flat `LogEntry` struct in `crates/ironlint-core/src/telemetry.rs` with a `serde(tag = "type", rename_all = "snake_case")` enum carrying four explicit variants — `SessionInit`, `Check`, `SemanticVerdict`, `SemanticSkipped` — each capturing the structure the consumer (D2 `coverage`, D3 `debt`, dashboards, log-greppers) actually needs. The existing flat shape stays readable for one release via a `#[serde(untagged)]` legacy reader so downgrade-and-roll-forward sequences don't lose data, and `ironlint_version` + `schema_version` start being stamped into every session so future analyzers can negotiate.

Bully wrote four record types from day one (`session_init`, `semantic_verdict`, `semantic_skipped`, file-level `check`); IronLint has been overloading a single struct with a stringly-typed `kind` field plus optional `reason`. D2 and D3 cannot be cleanly written against today's shape — per-rule outcomes are missing, the `kind` discriminator is untyped, and there is no `session_init` anchor that ties later records to an ironlint binary version. D1 fixes those three things at once.

## Architecture

`crates/ironlint-core/src/telemetry.rs` grows from one struct to:
...
`SessionInit` is written from a new helper, `commands/session.rs::start`, called from a new `ironlint session start` subcommand and lazily from `commands/session.rs::record` when no `session.json` exists yet (mirrors bully's "first edit triggers session_init"). The version comes from `env!("CARGO_PKG_VERSION")`.

## Tech Stack

Rust, workspace-stable. Already-direct deps on `ironlint-core`: `serde`, `serde_json`, `chrono`, `fs4`, `anyhow`. **No new deps.** `insta` (workspace dev-dep) is reused for the per-variant snapshots.
...
| Backwards-compat reader | `#[serde(untagged)]` over `(LogEntry, LogEntryLegacy)`. One-time stderr deprecation warning per process via `OnceLock<()>`. | Mirrors the v1→v2 dance from `Baseline` in [E1](archive/2026-05-12-ironlint-e1-baseline-checksum.md); same shape, same lifetime (deprecation window = until 0.3 verdict freeze, removed in the same PR). |
...
| `ironlint_version` source | `env!("CARGO_PKG_VERSION")` | Same approach as `Verdict::pass` and `Verdict::from_violations`. Compile-time, zero overhead. |
...
| Test fixture for legacy reader | Verbatim 5-line capture from a current-main `.ironlint/log.jsonl`, checked into `crates/ironlint-core/tests/fixtures/log_legacy.jsonl`. | Pins the wire shape that production ironlints are producing today. |
| `cargo insta` snapshots | One per variant. Kept under `crates/ironlint-core/tests/snapshots/` (default `insta` location). | Doubles as the documentation source for `docs/telemetry.md`. |
...
| Public-API impact | breaking | low | `pub enum LogEntry` replaces `pub struct LogEntry`. Pre-1.0; `ironlint-core` is consumed only by `ironlint-cli` in this workspace, and external crates aren't promised API stability yet. CHANGELOG documents the break. |
| Performance impact | low | low | Per-rule records expand JSONL line count by ~N rules/file. With a typical 5–20-rule config and ~50 files/check, that's ~250–1000 lines per check. `.ironlint/log.jsonl` already grows linearly with check count; this multiplies the constant by ~10. We do not rotate today; that stays a separate operability ticket. The single `flock`+`write_all` per record path is unchanged, so wall-clock impact per record is unchanged. |
...
**Rollback:** `git revert` of the squash-merge. The legacy reader path means existing `.ironlint/log.jsonl` files written under d1 will not parse under pre-d1 ironlint — operators rolling back will see corrupt-line warnings on logs from d1, and either truncate or accept the noise. Acceptable; same posture as E1.
...
└── tests/
    └── cli_typed_telemetry.rs               ← NEW: end-to-end `ironlint check` → assert all four record types in the log
...
|---|---|---|---|---|
| 1 | `crates/ironlint-core/src/runner.rs` | 527 | `LogEntry { kind: "skipped", file, status: "pass", … }` (skip-pattern A2 path) | `LogEntry::Check { ts, file, status: Status::Pass, elapsed_ms, rules: vec![] }` |
| 2 | `crates/ironlint-core/src/runner.rs` | 638 | `LogEntry { kind: "check", file, status: "<status>", … }` (per-file post-loop) | `LogEntry::Check { ts, file, status: verdict.status, elapsed_ms, rules: <PerRuleRecord-per-evaluated-rule> }` |
| 3 | `crates/ironlint-core/src/runner.rs` | 735 | `LogEntry { kind: "semantic_skipped", file, rule_id: Some(…), reason: Some(…), … }` (A3 path) | `LogEntry::SemanticSkipped { ts, file, rule, reason }` |
| 4 | `crates/ironlint-core/src/runner.rs` | 814 | `LogEntry { kind: "check_session", file: "", … }` (session aggregate) | `LogEntry::Check { ts, file: "".into(), status: verdict.status, elapsed_ms, rules: <PerRuleRecord-per-session-rule> }` |
| 5 | `crates/ironlint-cli/src/commands/session.rs` (new) | n/a | (no telemetry written today) | `LogEntry::SessionInit { ts, ironlint_version, schema_version }` from `start` and lazily from `record` |

`crates/ironlint-core/src/engine/semantic.rs` and `crates/ironlint-core/src/engine/session.rs` do **not** write telemetry directly today. The runner is the single writer, and we keep it that way — `SemanticVerdict` is also emitted from the runner inside the `Check` flow (one `PerRuleRecord` per rule, plus a `SemanticVerdict` line for every semantic rule that reached dispatch). Centralizing all writes in the runner keeps the parallel-dispatch ordering predictable and removes the need to thread the log path through the engine trait.
...
**Files:**
- Modify: `crates/ironlint-core/tests/telemetry.rs`
...
use ironlint_core::telemetry::{LogEntry, PerRuleRecord, SCHEMA_VERSION as TELEMETRY_SCHEMA};
use ironlint_core::verdict::{Engine, Status};

#[test]
fn session_init_round_trips() {
    let entry = LogEntry::SessionInit {
        ts: "2026-05-13T12:00:00Z".into(),
        ironlint_version: "0.2.2".into(),
        schema_version: TELEMETRY_SCHEMA,
    };
    let line = serde_json::to_string(&entry).unwrap();
    assert!(line.contains("\"type\":\"session_init\""), "discriminator field present: {line}");
    assert!(line.contains("\"ironlint_version\":\"0.2.2\""));
...
fn snake_case_field_names_match_spec() {
    ...
    assert!(obj.contains_key("ironlint_version"));
    assert!(obj.contains_key("schema_version"));
    assert!(!obj.contains_key("ironlintVersion"), "must be snake_case, not camelCase");
    assert!(!obj.contains_key("ironlint-version"));
}
```

- [ ] **Step 2: Run, confirm failure.**

Run: `cargo test -p ironlint-core --test telemetry 2>&1 | tail -40`

Expected: compile errors. Exact stderr fragment:
- `error[E0432]: unresolved import `ironlint_core::telemetry::PerRuleRecord``
- `error[E0432]: unresolved import `ironlint_core::telemetry::SCHEMA_VERSION``
- and: `error[E0599]: no variant or associated item named `SessionInit` found for struct `LogEntry``

- [ ] **Step 3: Commit failing tests.**

```bash
git add crates/ironlint-core/tests/telemetry.rs
...
**Files:**
- Modify: `crates/ironlint-core/src/telemetry.rs`
...
pub struct PerRuleRecord {
    ...
    /// `"disabled"` for `ironlint-disable:`-suppressed rows. `None` for
    /// vanilla pass/fire.
...
fn emit_legacy_warning(path: &Path) {
    if LEGACY_WARNING_EMITTED.set(()).is_ok() {
        eprintln!(
            "ironlint: warning — telemetry log at {} contains pre-D1 (flat) records; \
...
pub fn read_all(path: &Path) -> Result<Vec<LogEntry>> {
    ...
    for (i, line) in raw.lines().enumerate() {
        ...
        match serde_json::from_str::<LogEntryRead>(line) {
            ...
            Err(e) => {
                eprintln!(
                    "ironlint: warning — telemetry log {}:{} dropped (parse error: {e})",
                    path.display(),
                    i + 1
                );
            }
...
fn append_creates_log_and_writes_jsonl() {
    ...
    let log = dir.path().join(".ironlint/log.jsonl");
    let entry = LogEntry::Check {
...
Run: `cargo test -p ironlint-core --test telemetry`

Expected: all tests in the file pass — both the rewritten pre-D1 tests and the new D1 round-trip tests from Task 1.

- [ ] **Step 4: Run the full ironlint-core suite. It will fail at the call sites — that's fine.**

Run: `cargo build -p ironlint-core 2>&1 | tail -30`
...
```bash
git add crates/ironlint-core/src/telemetry.rs crates/ironlint-core/tests/telemetry.rs
git commit -m "$(cat <<'EOF'
...
Append to `crates/ironlint-core/tests/runner_skip.rs`:
...
"#,
    ...
    let engine = ironlint_core::runner::IronLintEngine::load(&cfg).expect("load");
    let lockfile = dir.path().join("Cargo.lock");
    fs::write(&lockfile, "# generated\n").unwrap();
    engine
        .check(ironlint_core::runner::CheckInput::File {
            path: lockfile.clone(),
            content: fs::read_to_string(&lockfile).unwrap(),
        })
        .expect("check");

    let log = fs::read_to_string(dir.path().join(".ironlint/log.jsonl")).expect("telemetry");
...
And append to `crates/ironlint-core/tests/runner_semantic_prefilter.rs`:
...
";
    ...
    let log = std::fs::read_to_string(dir.path().join(".ironlint/log.jsonl")).unwrap();
...
Run: `cargo test -p ironlint-core --test runner_skip --test runner_semantic_prefilter 2>&1 | tail -30`

Expected: failures with `"\"type\":\"check\""` not found in the log content (because runner still writes the flat shape).

- [ ] **Step 3: Edit `crates/ironlint-core/src/runner.rs`. Add a `record` field to `RuleOutcome` and emit the four new shapes.**
...
fn append_semantic_verdict(&self, rule_id: &str, file: Option<&str>, verdict_str: &str) {
    ...
    if let Err(e) = crate::telemetry::append(&self.config_dir.join(".ironlint/log.jsonl"), &entry) {
        eprintln!("ironlint: telemetry append failed: {e:#}");
    }
}
```
...
- [ ] **Step 4: Run the full ironlint-core suite.**

Run: `cargo test -p ironlint-core 2>&1 | tail -40`
...
```bash
grep -rn '\\"kind\\":' crates/ironlint-core/tests/ crates/ironlint-cli/tests/
```
...
```bash
git add crates/ironlint-core/src/runner.rs crates/ironlint-core/tests/runner_skip.rs crates/ironlint-core/tests/runner_semantic_prefilter.rs
git commit -m "$(cat <<'EOF'
...
fn session_start_writes_session_init_telemetry() {
    ...
    let trusted = ironlint_core::trust::write_trust_block(cfg_body).unwrap();
    ...
    let log = fs::read_to_string(dir.path().join(".ironlint/log.jsonl")).expect("log");
    assert!(log.contains("\"type\":\"session_init\""), "log:\n{log}");
    assert!(log.contains("\"ironlint_version\":"), "version stamp present:\n{log}");
...
Run: `cargo test -p ironlint-cli --test cli_session_start 2>&1 | tail -20`
...
In `crates/ironlint-cli/src/cli.rs`, augment `SessionAction`:
...
In `crates/ironlint-cli/src/main.rs`, add the dispatch arm. The current arm reads `SessionAction::Record { … } => commands::session::record(…)`; add:

```rust
SessionAction::Start { dir } => commands::session::start(&dir)?,
```

In `crates/ironlint-cli/src/commands/session.rs`, add a `start` function and wire `record` to lazily emit `SessionInit` when no prior `session.json` exists:

```rust
use ironlint_core::telemetry::{append as append_telemetry, LogEntry, SCHEMA_VERSION as TELEMETRY_SCHEMA};
...
let mut state = if state_path.exists() {
    ...
    let log = ironlint_dir.join("log.jsonl");
...
Run: `cargo test -p ironlint-cli --test cli_session_start`
...
```bash
git add crates/ironlint-cli/src/cli.rs crates/ironlint-cli/src/main.rs crates/ironlint-cli/src/commands/session.rs crates/ironlint-cli/tests/cli_session_start.rs
git commit -m "$(cat <<'EOF'
feat(cli): ironlint session start + lazy SessionInit on first record (D1 phase 3)

Adds `ironlint session start` which stamps a SessionInit telemetry record
with ironlint_version + schema_version. Also emits SessionInit lazily from
...
**Files:**
- Create: `crates/ironlint-core/tests/fixtures/log_legacy.jsonl`
- Create: `crates/ironlint-core/tests/telemetry_legacy.rs`

The fixture is a verbatim 5-line capture of what current-main ironlint writes to `.ironlint/log.jsonl` (post-A3, pre-D1). It pins the exact wire shape we're claiming compatibility with so a future "small refactor to telemetry" can't silently drop it.
...
```rust
use ironlint_core::telemetry::{read_all, LogEntry};
...
Run: `cargo test -p ironlint-core --test telemetry_legacy`
...
```bash
git add crates/ironlint-core/tests/fixtures/log_legacy.jsonl crates/ironlint-core/tests/telemetry_legacy.rs
git commit -m "$(cat <<'EOF'
test(telemetry): pin legacy wire shape via fixture (D1 phase 4)

Five-line capture of pre-D1 .ironlint/log.jsonl content; read_all() must
...
**Files:**
- Create: `crates/ironlint-cli/tests/cli_typed_telemetry.rs`

This test runs `ironlint check` against a fixture project that exercises every variant: a pass-through script rule (Check + PerRuleRecord), a semantic rule against a pure-deletion diff (SemanticSkipped), and a `session start` (SessionInit). Asserts every variant appears in the log.

- [ ] **Step 1: Write the test.**

```rust
//! D1: end-to-end assertion that a realistic `ironlint` session writes every
//! typed-telemetry variant to .ironlint/log.jsonl.
...
fn write_trusted_config(dir: &std::path::Path, body: &str) -> std::path::PathBuf {
    ...
    let trusted = ironlint_core::trust::write_trust_block(body).unwrap();
    fs::write(&cfg, trusted).unwrap();
    cfg
}
...
fn full_session_emits_every_typed_variant() {
    ...
    // 1) `ironlint session start` → SessionInit.
    Command::cargo_bin("ironlint")
        .unwrap()
        .args(["session", "start", "--dir", dir.path().to_str().unwrap()])
        .assert()
    ...
    // 2) `ironlint check --file foo.txt` → Check with PerRuleRecord.
    let cfg_body = "schema_version: 2\nrules:\n  always-pass:\n    description: x\n    engine: script\n    scope: [\"*.txt\"]\n    severity: error\n    script: \"true\"\n";
...
"#;
    ...
    Command::cargo_bin("ironlint")
        ...
        .env("IRONLINT_FAKE_KEY", "x") // never read; LLM never dispatched
        .current_dir(dir.path())
        .assert()
        .success();
...
**Files:**
- Modify: `crates/ironlint-core/tests/telemetry.rs` (append `insta` snapshot tests)
- Create: `crates/ironlint-core/tests/snapshots/telemetry__*.snap` (auto-generated)
...
Run: `INSTA_UPDATE=auto cargo test -p ironlint-core --test telemetry snapshot_`
...
Run: `cargo test -p ironlint-core --test telemetry snapshot_`
...
```bash
git add crates/ironlint-core/tests/telemetry.rs crates/ironlint-core/tests/snapshots/
git commit -m "$(cat <<'EOF'
...
IronLint appends one JSON record per line to `.ironlint/log.jsonl` for every check it performs. The file is owner-only (`0o600`) and append-only — IronLint never rewrites or truncates it. Operators rotate it themselves; downstream tools (`ironlint coverage`, `ironlint debt`, dashboards, log greppers) read it line-by-line.
...
**Compatibility:** `ironlint` 0.2.2+ writes the typed shape documented below. IronLints before 0.2.2 wrote a flat shape (`{ "timestamp": ..., "kind": ..., ... }`). The current reader (`ironlint_core::telemetry::read_all`) accepts both; it lifts each legacy line into the closest typed variant and emits a one-time stderr deprecation warning. The legacy reader is removed at the 0.3 verdict freeze.
...
Stamped at the start of every session — either explicitly via `ironlint session start`, or lazily at the first `ironlint session record` when no `session.json` exists yet. Anchors all subsequent records to an ironlint binary version.
...
| `ironlint_version` | string | Value of `CARGO_PKG_VERSION` of the writing binary. |
...
Written once per `ironlint check` call against a single file (or once per `ironlint check --session` aggregate). Carries the verdict status, wall-clock elapsed, and a per-rule outcome list.
...
| Field | Type | Description |
|---|---|---|
| `rule_id` | string | Rule id from `.ironlint.yml`. |
...
| `reason` | string, optional | `"engine_error"` for runtime failures, `"disabled"` for `ironlint-disable:`-suppressed rows, A3 reason (`empty` / `whitespace_only` / `comments_only` / `pure_deletion`) for short-circuited rules. Omitted when there's nothing to say. |

---

## `semantic_verdict`

Written every time the semantic engine reaches the LLM and returns a verdict (pass or violation). One per semantic rule per dispatched evaluation. Used by D2 (`ironlint coverage`) to count semantic-API hits.
...
| `rule` | string | Rule id. |
| `reason` | `"empty"` \| `"whitespace_only"` \| `"comments_only"` \| `"pure_deletion"` | Why the pre-filter decided not to dispatch. See `crates/ironlint-core/src/diff/analysis.rs`. |

---

## Atomicity and concurrency

`telemetry::append` opens with `O_APPEND`, takes an advisory `flock(LOCK_EX)`, writes one buffered line in a single `write_all`, then releases the lock. Concurrent `ironlint` invocations (e.g. parallel rules in a future B1 work-stealing pool) cannot interleave bytes. The kernel's `O_APPEND` atomicity guarantee covers writes below `PIPE_BUF`; the `flock` covers larger lines.
...
- [ ] **Step 2: Cross-link from the `crates/ironlint-core/src/telemetry.rs` module doc.**
...
```bash
git add docs/telemetry.md CHANGELOG.md crates/ironlint-core/src/telemetry.rs
git commit -m "$(cat <<'EOF'
...
**Files:**
- Possibly modify: `crates/ironlint-core/tests/telemetry.rs` if any region is uncovered.
...
Expected: `crates/ironlint-core/src/telemetry.rs` ≥80% region.
...
```rust
// In crates/ironlint-core/tests/runner_semantic_prefilter.rs:
#[test]
...
"#;
    ...
    let engine = IronLintEngine::load(&cfg_path).unwrap();
    let _ = engine
        .check(CheckInput::File { path: target, content: "fn main(){}\n".into() })
        .unwrap();

    let entries = ironlint_core::telemetry::read_all(&dir.path().join(".ironlint/log.jsonl")).unwrap();
    let has_engine_error = entries.iter().any(|e| matches!(
        e, ironlint_core::telemetry::LogEntry::Check { rules, .. }
...
```bash
git add crates/ironlint-core/tests/telemetry.rs crates/ironlint-core/tests/runner_semantic_prefilter.rs
git commit -m "$(cat <<'EOF'
...
Run: `cargo mutants --file crates/ironlint-core/src/telemetry.rs 2>&1 | tail -30`
...
- [ ] **Step 2:** `git mv plans/2026-05-13-ironlint-d1-typed-telemetry.md plans/archive/`.
- [ ] **Step 3:** Commit:

```bash
git add plans/README.md plans/archive/2026-05-13-ironlint-d1-typed-telemetry.md
git commit -m "$(cat <<'EOF'
...
| `session_start_writes_session_init_telemetry` | `cli_session_start.rs` | New `ironlint session start` subcommand |
...
- [ ] **AC1 — `.ironlint/log.jsonl` contains all four record types in a realistic session.** Covered by `full_session_emits_every_typed_variant` (Task 6). Also implicitly by the round-trip + runner tests.
...
- [ ] **`ironlint_version` + `schema_version` stamped at session start.** Task 4 (`SessionInit` carries both).
...
- [ ] Filename is `plans/2026-05-13-ironlint-d1-typed-telemetry.md` per CLAUDE.md / parity-spec §6 — **not** the writing-plans skill default of `docs/superpowers/plans/…`.
