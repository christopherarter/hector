# Hector C2 — `hector explain <file>` and `hector guide <file>`
...
**Goal:** Bring two read-only inspection subcommands to Hector at parity with bully. `hector explain <file>` answers "why didn't my rule fire on this file?" by walking every resolved rule and printing one greppable line per rule indicating whether the file is in scope, plus a leading `SKIPPED` line when the file matches a built-in or user skip pattern. `hector guide <file>` lists the rules that *do* apply to a file with their description and severity, so an author sees the policy surface for a given path without grepping the YAML. Neither command runs a script, calls an LLM, or writes telemetry — they only read config and report scope/skip resolution.

**Architecture:** Both commands lean on a single shared helper, `hector_core::runner::scope_outcomes(&engine, &file) -> ScopeOutcomes`, exported from `hector-core`. The helper walks the **already-extends-resolved** `BTreeMap<String, Rule>` on `HectorEngine` once, records the `SkipMatcher` hit (if any) at the file level, then per rule records `(rule_id, ScopeMatch)` where `ScopeMatch` is either `Match { glob }` (the first scope glob that matched) or `NoMatch { scopes: Vec<String> }` (every scope glob the rule registered). Output formatting lives in the per-command modules (`commands/explain.rs`, `commands/guide.rs`). The library helper deliberately returns structured data — not pre-formatted strings — so the JSON-format path and the human-format path render the same data, and so future callers (e.g. an LSP that wants to highlight in-scope rules) can reuse it without re-parsing text.

**Why one helper in `hector-core`, not duplicated in `hector-cli`:** `explain` and `guide` are both walks of the resolved rule set against a single path; the only difference is what they emit. Putting the walk in core avoids re-implementing scope/skip logic twice in the CLI, keeps `ScopeMatcher`/`SkipMatcher` private to core (where they live today — `pub(crate)` enforces it), and gives library consumers the same view the CLI shows. The alternative (helper in `hector-cli`) would force CLI code to re-construct `ScopeMatcher` per rule and reach into `HectorEngine`'s private fields, which would either bloat `HectorEngine`'s public surface or push the matcher constructors into `pub`. Core is the right home.

**Tech Stack:** clap (`Subcommand` variants with positional `<file>` arg), `assert_cmd` for CLI integration tests, `insta` for snapshot tests on the JSON format and the multi-line text format, `serde` + `serde_json` for the `--format json` path, the existing `hector_core::config::scope::ScopeMatcher` and `hector_core::config::skip::SkipMatcher` for the actual matching.
...
**Exit-code-contract impact:** these commands use **0 (success) and 1 (config error). They never use 2.** Spell this out so a future plan doesn't inadvertently reuse the exit-2 surface for advisory output. The contract that exit-2 means *block* is owned by `hector check` and must stay that way; `explain` and `guide` are inspection tools, not gates, and they have no concept of "block." The integration tests assert exit code is exactly 0 on every happy-path invocation (including when zero rules are in scope — empty output is success).

**Telemetry-schema impact:** **none**. Both commands are read-only — they do not append to `.hector/log.jsonl`. Confirmed by an integration test that runs `hector explain <file>` against a fixture project and asserts `.hector/log.jsonl` is absent or unchanged afterwards.

**Performance impact:** **trivial**. Both walk the rule set once (O(rules × scope_globs_per_rule)). No engine dispatch, no I/O beyond reading the config. A 100-rule `.hector.yml` resolves in well under 10ms.
...
**MODIFIED:**
- `crates/hector-cli/src/cli.rs` — add `Explain { file, format, config }` and `Guide { file, format, config }` variants on `Command`.
- `crates/hector-cli/src/main.rs` — dispatch the two new variants to `commands::explain::run` / `commands::guide::run`.
- `crates/hector-cli/src/commands/mod.rs` — `pub mod explain;` and `pub mod guide;`.
- `crates/hector-core/src/runner.rs` — add `pub fn scope_outcomes(&self, file: &Path) -> ScopeOutcomes` and the public `ScopeOutcomes` / `ScopeMatch` / `SkipHit` types.
- `README.md` — short blurb listing `hector explain` and `hector guide` under the "Commands" section.

**NEW:**
- `crates/hector-cli/src/commands/explain.rs` — formatter for the `explain` subcommand. Owns `ExplainEntry` (serde) and the human-format printer.
- `crates/hector-cli/src/commands/guide.rs` — formatter for the `guide` subcommand. Owns `GuideEntry` (serde) and the human-format printer.
- `crates/hector-cli/tests/cli_e2e_explain.rs` — assert_cmd + insta integration tests for `explain`.
- `crates/hector-cli/tests/cli_e2e_guide.rs` — assert_cmd + insta integration tests for `guide`.
- `crates/hector-core/tests/runner_scope_outcomes.rs` — library-level tests for the shared helper (covers the three branches: skipped file, in-scope rule, out-of-scope rule).

**Out-of-scope to TOUCH:**
- `crates/hector-core/src/verdict.rs` — locked-but-unstable surface. Neither command emits a `Verdict`.
- `crates/hector-core/src/telemetry.rs` — these commands write nothing.
- `crates/hector-core/src/llm/` — neither command constructs an LLM client.

---

## Phase 1 — Shared `scope_outcomes` helper in `hector-core`

### Task 1.1: Failing core test — helper reports the matching scope glob, the skipping pattern, and out-of-scope rules

**Files:**
- Create: `crates/hector-core/tests/runner_scope_outcomes.rs`
...
Run: `cargo test -p hector-core --test runner_scope_outcomes`
Expected: FAIL — `error[E0599]: no method named `scope_outcomes` found for struct `HectorEngine`` and `error[E0432]: unresolved import `hector_core::runner::ScopeMatch``.
...
- [ ] **Step 3: Add the public types near the top of `runner.rs` (above `HectorEngine`)**
...
Insert near the bottom of the existing `runner.rs` `impl HectorEngine` block (or as a free function near `home_dir`):
...
- [ ] **Step 5: Add the public `scope_outcomes` method on `HectorEngine`**

Insert inside `impl HectorEngine` (after `config_rule_ids` is a good neighbor):
...
```bash
git add crates/hector-core/src/runner.rs crates/hector-core/tests/runner_scope_outcomes.rs
git commit -m "$(cat <<'EOF'
feat(core): add scope_outcomes helper for explain/guide (C2 phase 1)

`HectorEngine::scope_outcomes(&file)` is the read-only scope walk
shared by `hector explain` and `hector guide`. Returns a `ScopeOutcomes`
...
same view the CLI shows without going through `HectorEngine`'s private
matcher fields.
...
Run: `cargo test -p hector-cli --test cli_e2e_explain`
...
- Create: `crates/hector-cli/src/commands/explain.rs`
- Modify: `crates/hector-cli/src/main.rs`

- [ ] **Step 11: Register the module**

Edit `crates/hector-cli/src/commands/mod.rs` to add:
...
- [ ] **Step 12: Write `crates/hector-cli/src/commands/explain.rs` in full**
...
```bash
git add crates/hector-cli/src/cli.rs crates/hector-cli/src/main.rs crates/hector-cli/src/commands/mod.rs crates/hector-cli/src/commands/explain.rs crates/hector-cli/src/commands/guide.rs crates/hector-cli/tests/cli_e2e_explain.rs
git commit -m "$(cat <<'EOF'
feat(cli): add hector explain subcommand (C2 phase 2)

`hector explain <file>` prints one greppable line per resolved rule:
...
Run: `cargo test -p hector-cli --test cli_e2e_guide`
...
**Files:**
- Modify: `crates/hector-cli/src/commands/guide.rs` (replace stub)
...
```bash
git add crates/hector-cli/src/commands/guide.rs crates/hector-cli/tests/cli_e2e_guide.rs
git commit -m "$(cat <<'EOF'
feat(cli): add hector guide subcommand (C2 phase 3)

`hector guide <file>` lists rules whose scope matches `<file>` as
...
**Files:**
- Modify: `crates/hector-cli/tests/cli_e2e_explain.rs`
- Modify: `crates/hector-cli/tests/cli_e2e_guide.rs`
...
Run: `cargo test -p hector-cli --test cli_e2e_explain explain_format_json`
Expected: FAIL on first run (no snapshot yet). Run `cargo insta review` and accept the four new snapshots:
- `crates/hector-cli/tests/snapshots/cli_e2e_explain__explain_md_file_json.snap`
- `crates/hector-cli/tests/snapshots/cli_e2e_explain__explain_lockfile_json.snap`
- `crates/hector-cli/tests/snapshots/cli_e2e_guide__guide_md_file_json.snap`
- `crates/hector-cli/tests/snapshots/cli_e2e_guide__guide_lockfile_json.snap`

Re-run: `cargo test -p hector-cli --test cli_e2e_explain && cargo test -p hector-cli --test cli_e2e_guide`
Expected: PASS.
...
```bash
git add crates/hector-cli/tests/cli_e2e_explain.rs crates/hector-cli/tests/cli_e2e_guide.rs crates/hector-cli/tests/snapshots/
git commit -m "$(cat <<'EOF'
test(cli): lock explain/guide JSON shapes via insta (C2 phase 4)

Snapshot the `--format json` output for `hector explain` and
`hector guide` in the happy path and the skipped-file path. Locks
...
git add README.md
git commit -m "$(cat <<'EOF'
docs(readme): document hector explain and hector guide (C2 phase 5)
...
- `crates/hector-cli/src/commands/explain.rs` — `emit_human`, `emit_json`, the `run` dispatch, and the unit-test helpers should already cover both branches of `ScopeMatch` and the `Some/None` skip arm. If still under 80%, add a unit test exercising `emit_human` with a `ScopeOutcomes { skip: None, rules: [Match, NoMatch] }` and another with `skip: Some(_)`.

- `crates/hector-cli/src/commands/guide.rs` — same structure as `explain.rs`. The `if outcomes.skip.is_some()` branch in `emit_json` and the `if matches!(.., Match { .. })` filter in `emit_human` both need at least one true and one false invocation; the existing unit tests cover both. Add a focused test for the `severity_str` helper if the macro-region accounting marks it un-hit (it already has a dedicated test).

- `crates/hector-core/src/runner.rs` — only the **new** functions need to clear the gate, not the whole file (which is pre-existing). The new `scope_outcomes`, `first_matching_scope_glob`, and `first_matching_skip_glob` should be exercised by the three integration tests in `tests/runner_scope_outcomes.rs`. If `first_matching_skip_glob`'s `else if let Some(prefix) = raw.strip_suffix("/**")` branch is uncovered, add a `dist/`-style test fixture to the existing integration test file: a file at `dist/foo.js` should produce a `SkipHit { pattern: "dist/**" }`.
...
```bash
cargo mutants --file crates/hector-core/src/runner.rs --file crates/hector-cli/src/commands/explain.rs --file crates/hector-cli/src/commands/guide.rs
```
...
```bash
git mv plans/2026-05-13-hector-c2-explain-guide.md plans/archive/2026-05-13-hector-c2-explain-guide.md
```
...
- [ ] **"Both subcommands work without a `--config` flag (default `.hector.yml`)."** → Task 2.2 / 3.2 set `default_value = ".hector.yml"` on the clap `--config` arg for both variants. Tests pass `--config` explicitly to use a tempdir; the default applies whenever the flag is omitted.
...
- [x] The shared helper has a single home (`hector_core::runner::scope_outcomes`) with a written justification — duplication-avoidance, reuse by future LSP/`show-resolved-config` callers, and keeping `ScopeMatcher`/`SkipMatcher` private to core.
