# `hector init` Harness Onboarding — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Fold rtk-style, one-command harness onboarding into `hector init` — detect installed coding agents, confirm, and atomically wire Hector's hook into each.

**Architecture:** A new `hector-core::adapter` module owns a 4-entry harness registry, materializes the existing `adapters/<harness>/` artifacts (embedded via `include_str!`) to stable paths, and patches each harness's settings idempotently. `hector init` gains a hook phase (detect → confirm → install/summary) after its existing config-scaffold phase; `hector doctor` gains an adapters section.
...
**Design spec:** `specs/2026-06-27-hector-init-onboarding-design.md`.
...
- `HectorEngine::load` and the trust store stay untouched. Hook integrity is a separate, lighter concern (a content-hash sidecar), not a trust decision.
...
- The materialized hook command references the **hook script path**, not the `hector` binary — the hook finds `hector` on `PATH` itself (matches today's adapters).
...
All in `crates/hector-core/src/adapter/`:
...
pub fn adapters_dir(env: &AdapterEnv) -> PathBuf;            // <config_home>/hector/adapters
...
pub fn sidecar_path(dir: &Path) -> PathBuf;                 // dir/.hector-adapter.json
...
pub struct PluginSpec {
    ...
    pub filename: &'static str,                  // "hector.ts"
...
CLI (in `crates/hector-cli/src/commands/init/`):
...
**Files:**
- Create: `crates/hector-core/src/adapter/mod.rs` (module decl only this task)
- Create: `crates/hector-core/src/adapter/materialize.rs`
- Modify: `crates/hector-core/src/lib.rs` (add `pub mod adapter;` after line 5's `pub mod config;` block)
...
In `crates/hector-core/src/lib.rs`, add after the existing `pub mod` block (keep alphabetical-ish order, place after `pub mod config;`):

```rust
pub mod adapter;
```

Create `crates/hector-core/src/adapter/mod.rs` with just:
...
Create `crates/hector-core/src/adapter/materialize.rs` ending with:
...
Run: `cargo test -p hector-core adapter::materialize`
Expected: FAIL — `materialize` items not defined / module body empty.

- [ ] **Step 4: Implement**

Prepend to `crates/hector-core/src/adapter/materialize.rs` (above the test module):
...
pub fn atomic_write(path: &Path, bytes: &[u8]) -> Result<()> {
    ...
    let tmp = path.with_extension(format!(
        "{}.tmp",
        path.extension().and_then(|e| e.to_str()).unwrap_or("hector")
    ));
...
/// `<dir>/.hector-adapter.json`.
pub fn sidecar_path(dir: &Path) -> PathBuf {
    dir.join(".hector-adapter.json")
}
...
Run: `cargo fmt && cargo clippy -p hector-core --all-targets -- -D warnings`
```bash
git add crates/hector-core/src/lib.rs crates/hector-core/src/adapter/
...
**Files:**
- Create: `crates/hector-core/src/adapter/json_settings.rs`
- Modify: `crates/hector-core/src/adapter/mod.rs` (add `mod json_settings;` + re-export)
...
Create `crates/hector-core/src/adapter/json_settings.rs` ending with:
...
Run: `cargo test -p hector-core adapter::json_settings`
Expected: FAIL — items not defined.

- [ ] **Step 3: Implement**

Prepend to `crates/hector-core/src/adapter/json_settings.rs`:
...
/// hector-owned entry (identified by `marker`). Idempotent: if the only
/// hector entry already equals `desired`, returns `AlreadyPresent`.
pub fn sync_hook_array(settings: &mut Value, key: &str, desired: Value, marker: &str) -> PatchResult {
...
Update `crates/hector-core/src/adapter/mod.rs`:
...
Run: `cargo fmt && cargo clippy -p hector-core --all-targets -- -D warnings`
```bash
git add crates/hector-core/src/adapter/
...
**Files:**
- Create: `crates/hector-core/src/adapter/registry.rs`
- Modify: `crates/hector-core/src/adapter/mod.rs` (define `AdapterEnv`, `Scope`, `Harness`, `HarnessKind`, `CURRENT_ADAPTER_VERSION`, `all_harnesses`, `adapters_dir`, `detect`; re-export specs)
- Modify: `crates/hector-core/src/trust.rs:133` (make `config_home` reusable)
...
In `crates/hector-core/src/trust.rs`, change line 133 from `fn config_home()` to `pub fn config_home()` so the adapter module can resolve `<config_home>` identically to the trust store. Leave `config_home_from` private (already tested via `store_path_joins_under_config_home`).

- [ ] **Step 2: Write the failing tests**

Append to `crates/hector-core/src/adapter/registry.rs` (file starts empty; tests first):
...
Replace `crates/hector-core/src/adapter/mod.rs` with:
...
Prepend to `crates/hector-core/src/adapter/registry.rs`:
...
Run: `cargo test -p hector-core adapter::registry`
Expected: PASS (6 tests). If `embedded_set_covers_on_disk_adapter_files` fails, the `CARGO_MANIFEST_DIR/../../adapters` path is wrong — confirm with `ls crates/hector-core/../../adapters`.

- [ ] **Step 7: Lint + commit**

Run: `cargo fmt && cargo clippy -p hector-core --all-targets -- -D warnings`
```bash
git add crates/hector-core/src/adapter/ crates/hector-core/src/trust.rs
...
**Files:**
- Create: `crates/hector-core/src/adapter/ops.rs`
- Modify: `crates/hector-core/src/adapter/mod.rs` (add `mod ops;` + re-export `install`, `uninstall`, `status`, `InstallOutcome`, `InstallResult`, `HarnessStatus`)
...
Create `crates/hector-core/src/adapter/ops.rs` ending with:
...
Prepend to `crates/hector-core/src/adapter/ops.rs`:
...
Run: `cargo test -p hector-core && cargo clippy -p hector-core --all-targets -- -D warnings`
...
**Files:**
- Modify: `crates/hector-cli/src/cli.rs:71-74` (extend `Init` variant)
- Modify: `crates/hector-cli/src/main.rs:36` (pass new fields)
- Modify: `crates/hector-cli/src/commands/init/mod.rs:17-43` (`Options`, new `run` signature, non-fatal existing config, call hook phase)

**Interfaces:**
- Consumes: `hector_core::adapter::*`.
- Produces: `commands::init::Options`, extended `commands::init::run`.

- [ ] **Step 1: Write the failing test (existing-config is non-fatal)**

Add to the `tests` module in `crates/hector-cli/src/commands/init/mod.rs`:
...
(Requires `tempfile` as a dev-dependency of `hector-cli` — it already is, per `crates/hector-cli/Cargo.toml:24`.)

- [ ] **Step 2: Run test to verify it fails**

Run: `cargo test -p hector-cli init::tests::run_with_existing_config`
Expected: FAIL — `run` takes `(&Path)` not `(&Path, Options)`, and currently errors on existing config.

- [ ] **Step 3: Extend the clap `Init` variant**

Replace `crates/hector-cli/src/cli.rs:71-74` (`Init { dir }`) with:
...
Replace `crates/hector-cli/src/main.rs:36`:
...
In `crates/hector-cli/src/commands/init/mod.rs`, add at the top of the module:
...
(The old `run` printed a `hector check ...` hint; move that hint into the summary printed by `onboard::run_hook_phase` so output stays cohesive. The detect/`build_config` helpers are unchanged.)

- [ ] **Step 6: Stub `onboard` so the crate compiles**

Create `crates/hector-cli/src/commands/init/onboard.rs` with a temporary stub (replaced in Task 6):
...
Run: `cargo test -p hector-cli init`
Expected: PASS (existing init tests + the new non-fatal test). The pre-existing `run` tests that called `run(&dir)` (if any) must be updated to the new signature — search and fix: `rg "init::run\(" crates/hector-cli`.

- [ ] **Step 8: Lint + commit**

Run: `cargo fmt && cargo clippy -p hector-cli --all-targets -- -D warnings`
```bash
git add crates/hector-cli/src/cli.rs crates/hector-cli/src/main.rs crates/hector-cli/src/commands/init/
...
**Files:**
- Modify: `crates/hector-cli/src/commands/init/onboard.rs` (replace stub)
- Create: `crates/hector-cli/tests/cli_init_onboarding.rs` (integration tests)

**Interfaces:**
- Consumes: `hector_core::adapter::{all_harnesses, detect, install, uninstall, AdapterEnv, Scope, InstallResult, Harness}`.
- Produces: `run_hook_phase`.

- [ ] **Step 1: Write the unit test for confirm-parsing**

End `crates/hector-cli/src/commands/init/onboard.rs` with:
...
Create `crates/hector-cli/tests/cli_init_onboarding.rs`:
...
fn uninstall_removes_hook() {
    ...
    hector(&home, &project).args(["init", "--hook-only", "--harness", "reasonix", "--yes"]).assert().success();
    hector(&home, &project).args(["init", "--uninstall", "--harness", "reasonix"]).assert().success();
    assert!(!home.join(".config/hector/adapters/reasonix/hook.sh").exists());
}
...
fn no_tty_without_yes_or_harness_skips_hooks() {
    ...
    hector(&home, &project).args(["init", "--hook-only"]).assert().success();
    assert!(!home.join(".reasonix/settings.json").exists());
}
```
...
Run: `cargo test -p hector-cli --test cli_init_onboarding`
...
Replace the non-test part of `crates/hector-cli/src/commands/init/onboard.rs` with:
...
Run: `cargo test -p hector-cli --test cli_init_onboarding && cargo test -p hector-cli init::`
...
```bash
git add crates/hector-cli/src/commands/init/onboard.rs crates/hector-cli/tests/cli_init_onboarding.rs
git commit -m "feat(init): detect/confirm/install/uninstall onboarding flow"
```

---

### Task 7: `hector doctor` adapters section

**Files:**
- Modify: `crates/hector-cli/src/commands/doctor.rs` (add an adapters section to both `human` and `json` output)

**Interfaces:**
- Consumes: `hector_core::adapter::{all_harnesses, status, AdapterEnv, Scope, HarnessStatus}`.
...
Run: `sed -n '1,60p' crates/hector-cli/src/commands/doctor.rs` to see how it assembles the human checklist and the JSON report (it has a `--format` switch). Match that structure — append an `adapters` array to the JSON report and an "Adapters" block to the human checklist. Do not change existing fields (the JSON schema is documented in `docs/operating/diagnostics.md`; bump that doc's schema note in Task 8).

- [ ] **Step 2: Write the failing test**

Add to the `tests` module in `crates/hector-cli/src/commands/doctor.rs` (use the same temp-env pattern the file already uses; if it shells via assert_cmd, add this to `tests/` instead — follow the file's existing convention):

```rust
#[test]
fn doctor_reports_installed_adapter() {
    let tmp = tempfile::tempdir().unwrap();
    let env = hector_core::adapter::AdapterEnv {
        home: tmp.path().to_path_buf(),
        config_home: tmp.path().join(".config"),
        project_root: tmp.path().join("proj"),
    };
    let h = hector_core::adapter::all_harnesses().into_iter().find(|h| h.name == "reasonix").unwrap();
    hector_core::adapter::install(&h, &env, hector_core::adapter::Scope::Global, false).unwrap();
...
Run: `cargo test -p hector-cli doctor_reports_installed_adapter`
Expected: FAIL — `adapter_report` not defined.

- [ ] **Step 4: Implement**

Add to `crates/hector-cli/src/commands/doctor.rs`:
    ...
    report
        ...
        .map(|s| {
            let state = if !s.installed {
                ...
                "installed, MODIFIED (re-run hector init)"
            } else if s.current == Some(false) {
                "installed, outdated (re-run hector init)"
            } else {
                "installed, ok"
            };
...
```bash
git add crates/hector-cli/src/commands/doctor.rs crates/hector-core/src/adapter/ops.rs
...
**Files:**
- Modify: `adapters/claude-code/README.md`, `adapters/reasonix/README.md`, `adapters/pi/README.md`, `adapters/opencode/README.md` (lead with `hector init`)
...
```bash
hector init --harness reasonix      # or bare `hector init` to detect + confirm
```

This materializes the hook to `~/.config/hector/adapters/reasonix/hook.sh` and
registers a PreToolUse entry in `~/.reasonix/settings.json` (atomic, backed up
to `settings.json.bak`). Verify with `hector doctor`. Remove with
`hector init --uninstall --harness reasonix`.
```

Use the corresponding default path/scope per harness (claude-code: project `.claude/settings.json`, `--global` for user; pi: `.pi/extensions/hector.ts`; opencode: `.opencode/plugins/hector.ts`).
...
```bash
echo "NOTE: this installer is superseded by \`hector init --harness reasonix\`." >&2
echo "      It remains as a fallback for environments without the hector binary." >&2
```
...
- In `CHANGELOG.md`, add an Unreleased entry: `hector init now installs hector's hook into detected coding agents (claude-code, reasonix, pi, opencode) with detect-then-confirm UX, --dry-run, and --uninstall; hector doctor reports adapter status.`
...
```bash
cargo build --release
HOME=$(mktemp -d) XDG_CONFIG_HOME=$(mktemp -d) ./target/release/hector init --hook-only --harness all --yes --dry-run
```
...
git commit -m "docs(init): onboard via hector init; deprecate per-adapter installers"
```
