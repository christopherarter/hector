# IronLint 0.4 — Checks Pipeline Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reposition ironlint as "a local CI for agents" — rename `gates`→`checks`, flip the verdict to nonzero-blocks, add fail-fast `steps`, and add `write`/`pre-commit` lifecycles with `$IRONLINT_FILES`.

**Architecture:** A check is `files` + (`run` xor `steps`) + optional `on`/`name`. The runner matches a touched file, and for each lifecycle in `on:` runs the check's steps fail-fast; a step exiting `1`–`125` blocks. `write` runs per-file with content on stdin; `pre-commit` runs once over the changed set via `$IRONLINT_FILES`. Phases land in compile-dependency order: the vocabulary rename is the keystone, then grammar, engine, runner, CLI, dogfood/docs.

**Tech Stack:** Rust (workspace: `ironlint-core` lib + `ironlint-cli` bin `ironlint`), `serde`/`serde_yaml`, `globset`, `wait_timeout`, `assert_cmd` for CLI e2e.

**Design spec:** `specs/2026-06-28-ironlint-checks-pipeline-design.md`
...
- `engine/gate.rs` — `classify()` flip; `GateEnv` gains `files` + `file: Option`; sets `$IRONLINT_FILES`.
...
**Modified (repo + tests):**
- `.ironlint.yml` — `gates:`→`checks:` (keystone), then `! grep` idiom (dogfood phase).
- `crates/ironlint-cli/tests/*.rs` — fixture YAML + assertions.
- `scripts/ci-dogfood.sh`, README, `ironlint-config` skill — vocabulary + DX.

**Untouched (regression guards):** `tests/fixtures/valid_v2.ironlint.yml`, `tests/fixtures/with_extends/*` (legacy-rejection fixtures), `config/scope.rs`, `config/extends.rs`, `disable.rs` mechanism (comments only).
...
**Files:**
- Modify: `crates/ironlint-core/src/config/types.rs:4-49`
- Modify: `crates/ironlint-core/src/config/parser.rs:50-63` (legacy_marker) + tests
- Modify: `crates/ironlint-core/src/config/mod.rs`
- Modify: `.ironlint.yml` (repo root)
...
- [ ] **Step 4: Run it, confirm it fails**, then confirm Step 2 makes it pass. `cargo test -p ironlint-core config::parser`.

- [ ] **Step 5: Migrate the dogfood `.ironlint.yml`.** Change `gates:` → `checks:`. **Keep the `case $?; exit 2` runs unchanged** (they still block under the unchanged classification; the `! grep` simplification is Phase 6). Update `mod.rs` re-exports (`Gate`→`Check`).
...
**Files:**
- Modify: `crates/ironlint-core/src/engine/gate.rs` (`GateEnv.event` comment: drop `edit` from the doc list only)
- Modify: `crates/ironlint-core/src/runner.rs:14,69,378-417,432-473` + tests (L508-680)

- [ ] **Step 1: Rename runner symbols.** `CheckOptions.gates` → `checks` (the `HashSet<String>` id filter); `IronLintEngine.scope_matchers` keys are check ids (no type change); `for (id, gate) in &self.config.gates` → `&self.config.checks`; `run_one_gate` → `run_one_check`; `gate: &Gate` params → `check: &Check`; `engine.gates()` accessor → `checks()`; `gate_ids()` → `check_ids()`. Update the 9 runner tests' identifiers and any `gates:` YAML in their fixtures.
...
- [ ] **Step 3: `cargo test -p ironlint-core`** — engine + runner now compile; verdict/telemetry still pending (1.3).

### Task 1.3: Schema 5 — verdict + telemetry key rename

**Files:**
- Modify: `crates/ironlint-core/src/verdict.rs:5,34-48` + tests (L91-150)
- Modify: `crates/ironlint-core/src/telemetry.rs:18,22-45` + tests (L111-132)
...
- [ ] **Step 6:** `cargo test -p ironlint-core` green.

### Task 1.4: Rename in the CLI + tests

**Files:**
- Modify: `crates/ironlint-cli/src/cli.rs:18,37,68`
- Modify: `crates/ironlint-cli/src/commands/check.rs:38-101`
- Modify: `crates/ironlint-cli/src/commands/explain.rs:14,33`, `show_resolved_config.rs`
- Modify: `crates/ironlint-cli/tests/cli_e2e_gates.rs`, `cli_validate.rs`, `cli_e2e_explain.rs` (fixtures + assertions)
...
plus the dogfood .ironlint.yml. Behavior identical (exit 2 still blocks); only
...
**Files:**
- Modify: `crates/ironlint-core/src/config/types.rs`
- Modify: `crates/ironlint-core/src/config/mod.rs`
...
- [ ] **Step 2: Run them, confirm they fail** (`Lifecycle`/`Step`/`effective_steps` undefined). `cargo test -p ironlint-core config::types`.
...
- [ ] **Step 5: `cargo build -p ironlint-core` will break** at `check.run` (String) usages in runner — fixed in Task 2.3. Commit after 2.2/2.3 land together.

### Task 2.2: run-xor-steps + per-step validation

**Files:**
- Modify: `crates/ironlint-core/src/config/parser.rs:9-29` + tests
...
- [ ] **Step 2: Confirm they fail.** `cargo test -p ironlint-core config::parser`.
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs:393-417` + tests
...
- [ ] **Step 4: Run tests, confirm pass.** `cargo test -p ironlint-core`.
...
# Phase 3 — Engine: nonzero blocks + `$IRONLINT_FILES`

Flip `classify()` so `1`–`125` blocks; preserve the broken-gate fail-open tier. Add `$IRONLINT_FILES` to the ABI.

### Task 3.1: Classification flip

**Files:**
- Modify: `crates/ironlint-core/src/engine/gate.rs:124-146` + tests (L166-275)
...
- [ ] **Step 2: Confirm the suite fails** on `exit_one_is_block`. `cargo test -p ironlint-core engine::gate`.
...
- [ ] **Step 4: Run, confirm pass.** `cargo test -p ironlint-core`.

### Task 3.2: Add `$IRONLINT_FILES` to the ABI

**Files:**
- Modify: `crates/ironlint-core/src/engine/gate.rs:14-21,61-91` + tests
- Modify: `crates/ironlint-core/src/runner.rs:404-410` (build the new `GateEnv`)

**Interfaces:**
- Produces: `GateEnv { file: Option<&Path>, files: &'a [PathBuf], root: &Path, event: &str }`. `run_gate` sets `$IRONLINT_FILE` only when `file.is_some()`, and always sets `$IRONLINT_FILES` = the `files` joined by `\n` (absolute paths).
...
fn ironlint_files_is_exported_newline_joined() {
    // gate greps $IRONLINT_FILES for two paths; blocks (exit 1) if both present.
    let out = run(
        "grep -q a.rs <<<\"$IRONLINT_FILES\" && grep -q b.rs <<<\"$IRONLINT_FILES\" && exit 1 || exit 0",
        env_with_files(&["/p/a.rs", "/p/b.rs"]),
        None, secs(5));
    assert!(matches!(out, GateOutcome::Block { .. }), "both files must be in $IRONLINT_FILES");
}
```
Add an `env_with_files` test helper paralleling the existing `env()` helper.

- [ ] **Step 2: Confirm it fails** (`$IRONLINT_FILES` unset → grep finds nothing → exit 0 → Pass).

- [ ] **Step 3: Change `GateEnv`** to `file: Option<&Path>` + `files: &[PathBuf]`. In `run_gate` env setup (L61-69): set `IRONLINT_FILE` only `if let Some(f) = env.file`; always `.env("IRONLINT_FILES", env.files.iter().map(|p| p.display().to_string()).collect::<Vec<_>>().join("\n"))`.

- [ ] **Step 4: Update the runner caller** (`run_one_check`, ~L404): build `GateEnv { file: Some(&abs), files: std::slice::from_ref(&abs), root, event }` (per-file: the set is the single file). Update the existing `ironlint_file_env_is_exported` test if its `GateEnv` literal changed.

- [ ] **Step 5: Run tests, confirm pass; clippy; fmt; commit Phase 3.**

```bash
git add -A -- ':!Cargo.lock'
git commit -m "feat(engine): nonzero (1-125) blocks; add \$IRONLINT_FILES to the ABI
...
\$IRONLINT_FILES (newline-joined) in addition to \$IRONLINT_FILE.
...
`write` runs per-file (current path, now also setting `$IRONLINT_FILES` = the one file). `pre-commit` runs each check **once** over the changed set with `$IRONLINT_FILES`, `$IRONLINT_FILE` unset, empty stdin. The `on:` filter skips checks whose `on` excludes the current event; the event is always `write` or `pre-commit`, so the filter always applies.

### Task 4.1: The `on:` filter

**Files:**
- Modify: `crates/ironlint-core/src/runner.rs:378-391` (`skip_reason`) + `CheckOptions` (L14) + tests
...
**Files:**
- Modify: `crates/ironlint-core/src/runner.rs` (add a set-input path) + tests

**Interfaces:**
- Produces: `IronLintEngine::check_set(files: &[PathBuf]) -> Result<Verdict>` — for each check whose scope matches ≥1 file in the set and whose `on` admits the event, run the check's steps **once** with `GateEnv { file: None, files: &matched, .. }`, empty stdin. Folds into the same `Verdict`.
...
fn pre_commit_runs_check_once_over_the_set() {
    ...
    write_config(&dir, "checks:\n  g:\n    files: \"*.rs\"\n    on: [pre-commit]\n    run: \"printf x >> $IRONLINT_ROOT/runs.txt; test $(grep -c x $IRONLINT_ROOT/runs.txt 2>/dev/null || echo 0) -le 1\"\n");
...
\$IRONLINT_FILES and no stdin — the pre-commit / lefthook-parity path.
...
**Files:**
- Modify: `crates/ironlint-cli/src/cli.rs` (`--event` value parser)
- Modify: `crates/ironlint-cli/src/commands/check.rs` (`run_diff`)
- Modify: `crates/ironlint-cli/tests/cli_event_validation.rs`, `cli_e2e_gates.rs`
...
fn diff_pre_commit_runs_check_once_over_set() {
    // two .rs files in the diff, a pre-commit check that blocks if $IRONLINT_FILES has 2 lines
    // ... build a temp repo + unified diff touching a.rs and b.rs ...
    cmd().args(["check","--diff","d.patch","--event","pre-commit","--config",".ironlint.yml"])
        .assert().code(2);
}
```
...
### Task 6.1: Simplify `.ironlint.yml` to the nonzero-blocks idiom
...
checks:
  no-todo-in-src:
    files: crates/*/src/**/*.rs
    run: '! grep -nE "todo!\\(" "$IRONLINT_FILE"'
  no-unimplemented-in-src:
    files: crates/*/src/**/*.rs
    run: '! grep -nE "unimplemented!\\(" "$IRONLINT_FILE"'
  no-dbg-in-src:
    files: crates/*/src/**/*.rs
    run: '! grep -nE "dbg!\\(" "$IRONLINT_FILE"'
```

- [ ] **Step 2: Re-bless trust** (the gates-dir/config hash changed): run `cargo run -q -- trust` (or `./target/release/ironlint trust`) so `~/.config/ironlint/trust.json` matches; confirm `ironlint check` on a dirty file still exits 2.
...
```bash
git add .ironlint.yml
git commit -m "chore(dogfood): adopt nonzero-blocks idiom in ironlint's own checks

Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>
Claude-Session: https://claude.ai/code/session_01KCX6mWmrFwyUeEA2FxhPVU"
```

### Task 6.2: Reposition README + the `ironlint-config` skill

**Files:**
- Modify: `README.md`, the `ironlint-config` skill (`SKILL.md` + `ironlint schema` output if it embeds the grammar), `CHANGELOG.md`

- [ ] **Step 1:** Update the pitch to "a local CI for agents"; replace `gates:`/`gate`/exit-2 examples with `checks:`/`on:`/`steps:`/nonzero-blocks; document the `write`/`pre-commit` lifecycles, `$IRONLINT_FILE`/`$IRONLINT_FILES`/stdin ABI, and the lefthook-parity story (spec §8). Note `ironlint-disable: <check-id>` vocabulary.

- [ ] **Step 2:** If `ironlint schema` (the skill's machine surface) embeds the config grammar, regenerate/update it for `checks`/`run`/`steps`/`on`/`name`.
...
- [ ] `cargo build --release` then sanity-check the three flows by hand: a `write` per-file block, a `pre-commit --diff` run-once block, and a clean pass. Then `cargo clean -p ironlint-cli` to drop the release artifact.
- [ ] Spot-check a surviving-mutant pass on the two highest-risk files: `cargo mutants --file crates/ironlint-core/src/engine/gate.rs` and `--file crates/ironlint-core/src/runner.rs` (local, ad-hoc). Treat survivors in `classify`/`check_set` as coverage gaps.
...
- §6 ABI (`$IRONLINT_FILES`, lifecycle channels) → Task 3.2 + Task 4.2. ✓
- §7 schema 5 / `check`/`step` keys / telemetry → Task 1.3. ✓
- §8 lefthook parity → Task 4.2 + Task 6.2 docs. ✓
- §9 code impact → all phases. ✓
- §12 resolved decisions → fail-fast (2.3), on default [write] (2.1), pre-commit run-once (4.2), `$IRONLINT_FILES` both lifecycles (3.2/4.2), no `uses:` (not added), rename (1.x). ✓
