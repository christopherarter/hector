# IronLint first-run DX audit

**Date:** 2026-05-23
**Auditor:** Chris Arter
**Source:** First hands-on test of the shipped 0.1 binary + Claude Code adapter (H3) against a real project — `samplevu-next` (Next.js / TypeScript / pnpm workspaces / biome). Two annotated transcripts captured in `~/Documents/projects/samplevu-next/2026-05-23-163729-write-this-ironlintyml-file.txt` and `~/Documents/projects/samplevu-next/2026-05-23-163844-make-an-update-to-try-and-blatantly-violate-the.txt`.

## What was tested

- `ironlint init` against a `pnpm` monorepo
- Adding a semantic rule + `llm.provider: claude-code-subagent`
- `ironlint trust` round-trip
...
|----|----------|-------|--------|-------|
| [R1](#r1) | High | `ironlint init` is monorepo-blind and doesn't detect existing linters | M | 5 |
| [R2](#r2) | Low | `model:` required-but-ignored under `claude-code-subagent` | S | 3a |
| [R3](#r3) | Med | Hook self-checks `.ironlint.yml` mid-edit → internal-error noise | XS | 2 |
...
### R1 — `ironlint init` is monorepo-blind and doesn't detect existing linters {#r1}

**Evidence (transcript 1, lines 42–95):** `ironlint init` scaffolded `scope: ["src/**/*.ts", ...]`. The repo's source lives under `apps/**/src/` and `packages/**/src/`, so every scaffolded glob matched zero files. The init also wrote a `no-console-log` grep rule even though biome is configured and biome's `noConsole` lint catches the same thing — when the user later added a `biome-check` rule, the two rules fired in lockstep on the same violation.
...
**Files likely touched:** `crates/ironlint-cli/src/commands/init.rs` (or wherever init lives), associated fixtures + integration test.

**Test approach:** Add fixtures for (pnpm workspace, no linter), (pnpm + biome), (Cargo workspace + clippy), (single-package npm + eslint). Assert the generated `.ironlint.yml` has correct scopes and doesn't duplicate detected linters.
...
**Files likely touched:** `crates/ironlint-core/src/config/types.rs` (LlmConfig parsing), `crates/ironlint-core/src/llm/mod.rs` (provider construction), validate-time check in `runner.rs`.
...
### R3 — Hook self-checks `.ironlint.yml` mid-edit → internal-error noise {#r3}

**Evidence (transcript 1, lines 96–97 and 168):** Editing `.ironlint.yml` itself triggered `PostToolUse:Edit` → hook → `ironlint check --file .ironlint.yml` → exit 1 (file is mid-edit, trust gate fails). Twice in the transcript. The user saw scary "internal error" messages while doing something completely normal.
...
**Fix:** In `adapters/claude-code/hooks/hook.sh` and `adapters/opencode/src/index.ts`, short-circuit when the changed file is the policy file itself (`.ironlint.yml` or `.bully.yml`). Exit 0 with no output. Optional: log a one-liner stderr note (`ironlint: skipping self-check of policy file`) gated behind a debug env var.
...
**Test approach:** Add a scenario to `adapters/claude-code/tests/subagent_mode.sh` (and the opencode equivalent) that pipes a `PostToolUse` event for `.ironlint.yml`. Assert exit 0 and empty stdout/stderr.
...
**Why it matters:** Architectural — we will not be the team that ships a parser for every linter on earth. Bully's passthrough model is correct; the consuming agent (or downstream tooling) can interpret raw diagnostic text. Today's default mismatch makes ironlint's output look broken on the first real linter.

**Fix:**
- Flip `Rule.output` default from `Parsed` → `Passthrough` in `crates/ironlint-core/src/config/types.rs`.
...
- Update `ironlint init` scaffold to never emit `output: parsed`.
...
**Files likely touched:** `crates/ironlint-core/src/config/types.rs`, `crates/ironlint-core/src/engine/script.rs`, `crates/ironlint-core/src/engine/output.rs`, `crates/ironlint-cli/src/commands/init.rs` scaffold, fixtures in `tests/fixtures/`, snapshot tests, `CHANGELOG.md`, `docs/`.
...
**Evidence (transcript 1, lines 207–270):** User asked "can the evaluator use Haiku?" Answer required editing `adapters/claude-code/agents/ironlint-evaluator.md` frontmatter directly — and they had to be warned that the dev-repo copy isn't necessarily the same file Claude Code loads at runtime.
...
- When `provider == claude-code-subagent` and `evaluator_model` is set, propagate it through the deferred-payload envelope so the ironlint skill can dispatch the subagent with a model override.
- Skill update (`adapters/claude-code/skills/ironlint/SKILL.md`) to read the override from the envelope (if exposed) and pass `--model <x>` (or equivalent) on the dispatch. May require adding the field to `DeferredVerdict` envelope; if so, bump `DEFERRED_SCHEMA_VERSION`.
- Doc the field in the adapter README.

**Files likely touched:** `crates/ironlint-core/src/config/types.rs`, `crates/ironlint-core/src/verdict_deferred.rs`, `adapters/claude-code/skills/ironlint/SKILL.md`, `adapters/claude-code/README.md`, `docs/emit-semantic-payload.md`.
...
**Files likely touched:** `crates/ironlint-core/src/runner.rs`, `crates/ironlint-core/src/verdict.rs` (with a `SCHEMA_VERSION` bump if the deterministic verdict shape changes), `crates/ironlint-core/src/verdict_deferred.rs`, `adapters/claude-code/hooks/hook.sh`, `adapters/claude-code/skills/ironlint/SKILL.md`.
...
- Trace the capability-warning leak: confirm whether it fires per `ironlint` invocation or per `bash` invocation, and whether the hook's multiple `ironlint` calls (record + check) trigger it twice.

**Files likely touched:** `adapters/claude-code/hooks/hook.sh`, `crates/ironlint-core/src/engine/capability.rs` warning dedup, regression tests.
...
**Fix (later):**
- `ironlint explain --llm` (or `ironlint doctor`) prints the resolved `llm:` block + a one-liner per provider on what it does.
- README + adapter READMEs include the subagent config snippet inline.
- Eventually: ironlint.dev / docs site, referenced from `--help` / `doctor`. Out of scope for 0.2.
...
2. **R3** — hook skips `.ironlint.yml` (quick win, removes scary errors)
