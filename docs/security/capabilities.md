...
A `script:` rule can declare the network and filesystem capabilities it needs, and Hector constrains the spawned process accordingly. This guards against a *misconfigured* script doing something it shouldn't — it is not a defense against a malicious rule author. That job belongs to [the trust gate](trust.md).
...
The trust gate is the primary defense against malicious rules: Hector won't run a config whose fingerprint it hasn't verified. The fingerprint algorithm, the re-sign workflow, and the threat model it covers live in [The trust gate](trust.md).
