...
Two read-only commands answer "what would Hector do here?" without running a single check. Neither executes a `run` command or writes telemetry. They exit `0` on success and `1` on a config error — never `2`.

## "Which checks apply to this file?"

`hector explain` shows every check in scope for a file and the command each would run:
...
`hector show-resolved-config` prints the merged check set after inheritance is applied, with each check annotated by the file that defined it:
