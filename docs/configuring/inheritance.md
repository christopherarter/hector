...
`extends:` is a list of parent config paths, relative to the file that declares them. Hector resolves each parent depth-first (a parent may extend its own parents) and detects cycles. The merged check set is your local checks plus every check inherited from the closure.
...
So editing a parent — `shared/base.yml` or one of its check scripts — invalidates the root's hash and forces a re-review before the next `check`. A parent change can't silently alter what runs under an already-blessed child. (You only bless a parent directly if you also run `hector check --config shared/base.yml` against it as a root in its own right.) See [The trust store](../security/trust.md).
