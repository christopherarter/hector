# IronLint — pi adapter (tool_call gate)
...
**Scaffold (to build):** `adapters/pi/` — a pi extension wiring `tool_call` / `tool_result` / `session_start` / `agent_end` to the `ironlint` CLI.
...
This is the cleanest integration surface IronLint has targeted yet. pi's `tool_call` event fires *before* the tool runs and blocks by returning `{ block: true, reason }` — no exception-throwing (opencode) or exit-code-2-from-a-shell-hook (reasonix/claude-code) gymnastics. The adapter is a pure translation layer between pi's lifecycle and the `ironlint` binary; it contains no rule logic.
...
| Proposed-content delivery | **`--content -` via stdin** | No disk mutation; AST/semantic/`ironlint-disable` rules gate correctly pre-write. Tradeoff: `engine: script` rules read the pre-edit on-disk file (same documented limitation as reasonix). |
...
| Package name | **`@christopherarter/ironlint-pi`** | Matches `@christopherarter/ironlint-opencode`. |

## 4. Lifecycle mapping

| pi event | Action |
|---|---|
| `tool_call` (`write`/`edit`) | Compute proposed content → `ironlint check --file <path> --content - --config <cfg> --format json` → `return { block: true, reason }` on exit 2. |
| `tool_result` (`write`/`edit`) | `ironlint session record --dir <root> --file <path> --diff <synthetic-diff>` (best-effort). |
| `session_start` | Delete stale `.ironlint/session.json`. |
| `agent_end` | If `.ironlint/session.json` exists, `ironlint check --session`. Advisory — the turn is over, so surface the verdict (cannot retroactively block). |

## 5. The gate (`tool_call`)

```
1. configPath = <root>/.ironlint.yml
2. if !exists(configPath) → return            # re-checked every call: mid-session
                                               #   `ironlint init` starts gating
3. if toolName ∉ {write, edit} → return
4. path = input.path ?? input.file_path
   if !path → return
5. if basename(path) ∈ {.ironlint.yml, .bully.yml} → return   # R3 self-edit short-circuit
...
7. result = run ironlint check --file <path> --content - --config <cfg> --format json
...
8. map result.exitCode:
     ...
     3       → fail-open: console.error(stderr); return
                unless IRONLINT_FAIL_CLOSED_ON_INTERNAL=1 → { block: true, reason }
...
Exact-match-and-unique mirrors pi's own contract. We deliberately do **not** attempt to reproduce pi's fuzzy fallback — divergence there would feed `ironlint` content pi won't actually write, risking false blocks; skipping is safer.

### 5.2 Exit-code contract

Identical to every IronLint adapter (`commands/check.rs`): `0` pass/warn → allow · `2` block · `3` engine-internal error → fail-open by default, fail-closed under `IRONLINT_FAIL_CLOSED_ON_INTERNAL=1` · `1`/other config error → log + allow.
...
5. if basename(path) ∈ {.ironlint.yml, .bully.yml} → return
6. diff = synthesizeDiff(path, toolName, input)
7. run ironlint session record --dir <root> --file <path> --diff <diff>   # errors swallowed
```
...
- **Hunk-header counts (P1-8):** emit `1,N` form whenever a side has >1 line, so `ironlint`'s diff parser numbers added lines correctly.
...
```
1. if !exists(.ironlint/session.json) → return
2. result = run ironlint check --session --config <cfg> --format json
3. map exitCode:
...
Delete a stale `.ironlint/session.json` left by a prior aborted run (`rmSync(..., { force: true })`), best-effort.

## 9. Error handling & invariants

- **Fail-open** on internal/config errors (exit 1/3) by default; `IRONLINT_FAIL_CLOSED_ON_INTERNAL=1` flips exit-3 to a block. A misconfigured `ironlint` must never brick the agent.
- **Best-effort** for `session record` and stale-state cleanup — a flaky write there never affects the agent.
- **R3 self-edit short-circuit:** edits to `.ironlint.yml` / `.bully.yml` skip both the gate and session record (a mid-edit policy file fails the trust gate and would surface a confusing internal error).
- **Late config re-check:** every handler re-checks `.ironlint.yml` existence, so installing the extension globally is safe and mid-session `ironlint init` begins gating without a restart.
...
adapters/pi/
  src/index.ts        # factory + helpers: computeProposedContent,
                      #   synthesizeDiff, exit-code mapper, runIronLint
  package.json        # @christopherarter/ironlint-pi, "pi": { "extensions": ["./src/index.ts"] }
  tsconfig.json
  test/index.test.ts  # node:test suite (drives the factory directly)
  README.md           # install, requirements, exit-code table, known gaps
```

Plus CI wiring in `.github/workflows/ci.yml` to run the node:test suite with the built `ironlint` on PATH (mirroring how the opencode bun-test suite is run; confirm the existing pattern during planning).

## 11. Testing (`node:test`)

Drive the exported factory with synthetic pi-shaped events against the **real** `ironlint` binary (CI prepends `target/release` to `PATH`). Cases mirror the opencode suite:

- no-op when `.ironlint.yml` absent
...
- missing `path` → no-op
- `.ironlint.yml` / `.bully.yml` self-edit short-circuit (R3), incl. bare relative path
...
- exit-3 fail-open by default; fail-closed under `IRONLINT_FAIL_CLOSED_ON_INTERNAL=1`

## 12. Distribution / install

- **Local dev:** symlink or copy `src/index.ts` into `.pi/extensions/ironlint.ts` (project) or `~/.pi/agent/extensions/ironlint.ts` (global), or reference the absolute path in pi `settings.json` `"extensions"`. `pi -e ./adapters/pi/src/index.ts` for an ad-hoc load; `/reload` to hot-reload.
- **npm (once published):** `@christopherarter/ironlint-pi` with the `"pi": { "extensions": ["./src/index.ts"] }` field; pi installs deps on first load.
- Requires the `ironlint` binary on `PATH`. Silent no-op without `.ironlint.yml`, so global install is safe.
- Project setup: `ironlint init && ironlint trust`.
...
- **`engine: script` rules** read the pre-edit on-disk file under `--content -`; AST/semantic/`ironlint-disable` gate correctly pre-write.
...
Confirm at the start of implementation whether pi's `pi.exec(command, args, options?)` supports (a) stdin input and (b) exit-code retrieval. If yes, use it (gains `ctx.signal` abort integration). If not, use `node:child_process` (`execFileSync`/`spawnSync` with `{ input, encoding }`) — deterministic stdin + exit code, at the cost of no abort wiring. Resolve by reading `pi.exec`'s signature in `earendil-works/pi` before writing `runIronLint`.
