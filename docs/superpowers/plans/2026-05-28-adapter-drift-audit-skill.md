...
**Goal:** Build a repo-local, maintainer-facing skill that loads a coding harness's contract intel into context and audits the corresponding IronLint adapter for drift (read-only report).
...
Run:
```bash
cd /Users/chrisarter/Documents/projects/ironlint && git status --short .agents/skills/adapter-drift-audit && git log --oneline -3
```
