# ironlint-core Resilience & Correctness Hardening Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Fix the highest-value correctness/resilience defects in `crates/ironlint-core` surfaced by the 2026-05-28 review — a `clone(2)` deadlock hazard, silent script-output truncation, stale-content evaluation, brittle LLM parsing, missing retry, and duplicated glob logic — each as an independent, test-first change.
...
**Tech Stack:** Rust (edition from workspace), `anyhow` errors, `nix` (Linux clone/exec syscalls), `rayon` (parallel dispatch), `globset`, `reqwest::blocking`, `wiremock` + `tokio` (HTTP tests), `tempfile`, `insta`. Cargo workspace; crate name `ironlint-core`, binary `ironlint`.
...
cargo test -p ironlint-core                       # full core suite green
...
Run a single new test file with `cargo test -p ironlint-core --test <file_stem>`; filter by test-fn name with `cargo test -p ironlint-core <name>`.
...
- `crates/ironlint-core/src/engine/capability.rs` — **Tasks 1 & 2.** Script-subprocess runner. Task 1 makes stdout/stderr draining concurrent with the wait (fixes the pipe-buffer deadlock). Task 2 makes the cloned child async-signal-safe (pre-builds CStrings/envp in the parent, execs via `execve`+`chdir`). Same file — Task 2 re-reads it after Task 1 lands.
- `crates/ironlint-core/tests/capability.rs` — **Tasks 1 & 2.** Integration tests for the runner (existing file; append new tests).
- `crates/ironlint-core/src/engine/context.rs` — **Task 3.** `expand_context` gains an authoritative-content parameter so `context: file`/`repo` rules honor proposed `--content` instead of re-reading disk.
- `crates/ironlint-core/src/engine/semantic.rs` — **Task 3.** Passes `ctx.content` into `expand_context`.
- `crates/ironlint-core/src/runner.rs` — **Tasks 3 & 6.** Task 3 threads authoritative content through the deferred/preview call sites of `expand_context`. Task 6 deletes the two hand-rolled glob-identification helpers and calls the matchers' new `matched_pattern`.
- `crates/ironlint-core/tests/context_expansion.rs` — **Task 3.** Existing file; append content-precedence tests.
- `crates/ironlint-core/src/llm/mod.rs` — **Tasks 4 & 5.** Task 4 replaces the fragile first-`[`/last-`]` slice in `parse_verdicts` with a balanced-array scan. Task 5 adds a generic `retry_with_backoff` helper + `is_retryable_status`/`backoff_delay`.
- `crates/ironlint-core/src/llm/anthropic.rs` — **Task 5.** Wraps the request send in `retry_with_backoff`.
- `crates/ironlint-core/src/llm/openai_compat.rs` — **Task 5.** Same.
- `crates/ironlint-core/tests/anthropic.rs` — **Task 5.** Existing file; append a 429→200 recovery test.
- `crates/ironlint-core/src/config/scope.rs` — **Task 6.** `ScopeMatcher` tracks raw patterns and exposes `matched_pattern`.
- `crates/ironlint-core/src/config/skip.rs` — **Task 6.** `SkipMatcher` does the same.
...
**Files:**
- Modify: `crates/ironlint-core/src/engine/capability.rs` (`spawn_with_timeout` ~lines 422-470; `wait_for_child` ~lines 296-338; remove `read_pipes_bounded` ~lines 343-357; add `spawn_reader`/`join_reader` helpers)
- Test: `crates/ironlint-core/tests/capability.rs` (append)
...
Run: `cargo test -p ironlint-core --test capability captures_large_output_without_deadlocking -- --nocapture`
Expected: FAIL — either `exit_code` is `124` (timeout) with empty stdout, or it takes ~5s.

- [ ] **Step 3: Add the reader helpers**

In `crates/ironlint-core/src/engine/capability.rs`, add these two helpers (place them just above `spawn_with_timeout`, with no `#[cfg]` gate — both targets use them):
...
**Files:**
- Modify: `crates/ironlint-core/src/engine/capability.rs` (`spawn_clone` ~lines 148-216; rename/rewrite `child_main` → `child_exec` ~lines 226-282)
- Test: `crates/ironlint-core/tests/capability.rs` (append)

- [ ] **Step 1: Write the failing test**

Append to `crates/ironlint-core/tests/capability.rs` (the import line at the top of the file must include `run_with_capabilities_env` — change `use ironlint_core::engine::capability::run_with_capabilities;` to `use ironlint_core::engine::capability::{run_with_capabilities, run_with_capabilities_env};`):
...
fn clone_child_receives_injected_env_and_inherits_path() {
    ...
    let out = run_with_capabilities_env(
        "printf '%s\\n' \"$IRONLINT_TEST_VAR\"; command -v sh >/dev/null && echo PATH_OK",
        std::path::Path::new("/tmp"),
        &caps,
        &[("IRONLINT_TEST_VAR", "injected-value")],
    )
    .expect("run");
    assert!(
...
Run: `cargo test -p ironlint-core --test capability clone_child_receives_injected_env_and_inherits_path`
...
> If you are working on macOS, you cannot exercise the clone path locally. Implement the change, confirm the non-Linux build still compiles (`cargo build -p ironlint-core`), and rely on Linux CI for this test. State this explicitly when requesting review.
...
Run: `cargo build -p ironlint-core` then `cargo test -p ironlint-core --test capability`
...
**Files:**
- Modify: `crates/ironlint-core/src/engine/context.rs` (`expand_context` signature + `File`/`Repo` arms)
- Modify: `crates/ironlint-core/src/engine/semantic.rs` (the `expand_context` call ~line 13)
- Modify: `crates/ironlint-core/src/runner.rs` (`render_semantic_prompts` call ~line 1496; `expand_deferred_contexts` ~lines 1435-1471 + its caller `build_deferred_evaluator_input` ~line 1221 + that caller in `check_inner` ~line 1341)
- Test: `crates/ironlint-core/tests/context_expansion.rs` (append)

- [ ] **Step 1: Write the failing test**

Append to `crates/ironlint-core/tests/context_expansion.rs`:
...
Run: `cargo test -p ironlint-core --test context_expansion file_scope_prefers_authoritative_content`
...
Replace the `expand_context` function in `crates/ironlint-core/src/engine/context.rs` with:
...
Run: `cargo test -p ironlint-core --test context_expansion`
...
In `crates/ironlint-core/src/engine/semantic.rs`, change the `expand_context` call (line ~13) from:
...
In `crates/ironlint-core/src/runner.rs`:
...
Run: `cargo test -p ironlint-core`
...
cargo test -p ironlint-core
rm -rf target/llvm-cov-target target/llvm-cov
git add crates/ironlint-core/src/engine/context.rs crates/ironlint-core/src/engine/semantic.rs crates/ironlint-core/src/runner.rs crates/ironlint-core/tests/context_expansion.rs
...
**Files:**
- Modify: `crates/ironlint-core/src/llm/mod.rs` (`parse_verdicts` ~lines 228-258; add `balanced_array_span` + `extract_wire_verdicts` helpers; add tests to the existing `#[cfg(test)] mod redact_tests` or a new test module)

- [ ] **Step 1: Write the failing test**

In `crates/ironlint-core/src/llm/mod.rs`, add a test module at the end of the file (after `redact_tests`):
...
Run: `cargo test -p ironlint-core extracts_array_amid_prose_with_incidental_brackets`
...
In `crates/ironlint-core/src/llm/mod.rs`, replace the `parse_verdicts` function with:
...
Run: `cargo test -p ironlint-core --test anthropic && cargo test -p ironlint-core parse_verdict_tests`
...
cargo test -p ironlint-core
rm -rf target/llvm-cov-target target/llvm-cov
git add crates/ironlint-core/src/llm/mod.rs
...
**Files:**
- Modify: `crates/ironlint-core/src/llm/mod.rs` (add `retry_with_backoff`, `is_retryable_status`, `backoff_delay`, `MAX_LLM_RETRIES`; add unit tests)
- Modify: `crates/ironlint-core/src/llm/anthropic.rs` (wrap the send ~lines 84-91)
- Modify: `crates/ironlint-core/src/llm/openai_compat.rs` (wrap the send ~lines 84-88)
- Test: `crates/ironlint-core/tests/anthropic.rs` (append a 429→200 recovery test)

- [ ] **Step 1: Write the failing unit tests for the retry helper**

In `crates/ironlint-core/src/llm/mod.rs`, add a test module at the end of the file:
...
Run: `cargo test -p ironlint-core retry_tests`
...
In `crates/ironlint-core/src/llm/mod.rs`, add (place near the top, after the imports / `RuleVerdict` definitions):
...
In `crates/ironlint-core/src/llm/anthropic.rs`, replace the request-send block:
...
In `crates/ironlint-core/src/llm/openai_compat.rs`, replace:
...
Append to `crates/ironlint-core/tests/anthropic.rs`:
...
Run: `cargo test -p ironlint-core --test anthropic && cargo test -p ironlint-core --test openai_compat`
...
cargo test -p ironlint-core
rm -rf target/llvm-cov-target target/llvm-cov
git add crates/ironlint-core/src/llm/mod.rs crates/ironlint-core/src/llm/anthropic.rs crates/ironlint-core/src/llm/openai_compat.rs crates/ironlint-core/tests/anthropic.rs
...
**Files:**
- Modify: `crates/ironlint-core/src/config/scope.rs` (`ScopeMatcher`: track raw patterns + `matched_pattern`; add tests)
- Modify: `crates/ironlint-core/src/config/skip.rs` (`SkipMatcher`: track raw patterns + `matched_pattern`; add tests)
- Modify: `crates/ironlint-core/src/runner.rs` (`scope_outcomes` ~lines 536-568 uses the cached matchers; delete `first_matching_skip_glob` ~lines 371-403 and `first_matching_scope_glob` ~lines 408-426)

- [ ] **Step 1: Write the failing tests for `matched_pattern`**

In `crates/ironlint-core/src/config/scope.rs`, add a test module at the end:
...
And in `crates/ironlint-core/src/config/skip.rs`, add to the existing `mod tests`:
...
Run: `cargo test -p ironlint-core matched_pattern`
...
Replace `crates/ironlint-core/src/config/scope.rs`'s `ScopeMatcher` struct and `impl` with:
...
In `crates/ironlint-core/src/config/skip.rs`, change the struct to carry patterns and have `add_glob` record them. Replace the `SkipMatcher` struct, its `impl`, and `add_glob` with:
...
impl SkipMatcher {
    ...
    /// provides (project `skip:` list, `~/.ironlint-ignore` entries, etc.).
...
Run: `cargo test -p ironlint-core --test runner_scope_outcomes && cargo test -p ironlint-core --test runner_skip`
...
cargo test -p ironlint-core
rm -rf target/llvm-cov-target target/llvm-cov
git add crates/ironlint-core/src/config/scope.rs crates/ironlint-core/src/config/skip.rs crates/ironlint-core/src/runner.rs
...
Plan complete and saved to `docs/superpowers/plans/2026-05-28-ironlint-core-resilience-hardening.md`. Two execution options:
