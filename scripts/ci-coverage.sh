...
# Enforce a minimum line-coverage threshold on every Rust source file under
# `crates/*/src/`. Used by CI; runnable locally with the same dependencies.
#
...
#
# Configuration:
#   COVERAGE_THRESHOLD — minimum line coverage percent per file (default: 90)
#
# Exit codes:
...
done < <(jq -r '.data[0].files[] | [(.filename | sub("^.*?/crates/"; "crates/")), .summary.lines.percent] | @tsv' "${REPORT}")

total_lines_pct=$(jq -r '.data[0].totals.lines.percent' "${REPORT}")
total_funcs_pct=$(jq -r '.data[0].totals.functions.percent' "${REPORT}")

echo
echo "ci-coverage: workspace totals — lines ${total_lines_pct}%, functions ${total_funcs_pct}%"

if (( ${#below[@]} > 0 )); then
  echo
  echo "ci-coverage: ${#below[@]} file(s) below ${THRESHOLD}% line coverage:" >&2
  printf '%s\n' "${below[@]}" >&2
  echo >&2
...
echo "ci-coverage: all files ≥ ${THRESHOLD}% line coverage."
