...
/// `IRONLINT_TIMEOUT` env var overrides this at run time. Dispatch is
...
/// or `steps` is set. The path under check arrives as `$IRONLINT_FILE`; proposed
...
pub struct Check {
    ...
    /// Wiring `name` into block messages and `ironlint explain` rows is a
    /// tracked follow-up.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
}
