//! `ironlint-disable: <gate-id>` line directives.
//!
...
/// True if any `ironlint-disable: <gate-id>` directive in `content` names
...
/// Collect comma- or whitespace-separated gate IDs after `ironlint-disable:`.
...
mod tests {
    ...
    fn preserves_namespaced_gate_ids() {
        assert!(is_disabled(
            "x // ironlint-disable: python/no-print\n",
            "python/no-print"
        ));
        assert!(!is_disabled(
            "x // ironlint-disable: python/no-print\n",
    ...
    fn stops_at_block_comment_close_and_reason() {
        assert!(is_disabled("x /* ironlint-disable: g1 */\n", "g1"));
        assert!(is_disabled(
            "x // ironlint-disable: g1 reason: legacy\n",
            "g1"
        ));
        assert!(!is_disabled(
            "x // ironlint-disable: g1 reason: legacy\n",
    ...
    fn comma_and_whitespace_separated() {
        assert!(is_disabled("// ironlint-disable: a, b c\n", "a"));
        assert!(is_disabled("// ironlint-disable: a, b c\n", "b"));
        assert!(is_disabled("// ironlint-disable: a, b c\n", "c"));
    }
}
