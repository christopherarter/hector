...
mod tests {
    ...
    fn claude_plan() -> HarnessPlan {
        HarnessPlan {
            ...
            steps: vec![
                PlanStep::Hook {
                    path: PathBuf::from("/home/u/.config/hector/adapters/claude-code/hook.sh"),
                },
                PlanStep::Patch {
                ...
                PlanStep::Skill {
                    path: PathBuf::from("/home/u/proj/.claude/skills/hector-config/SKILL.md"),
                },
            ],
        }
    }
    ...
    fn renders_header_tag_and_tree_without_color() {
        ...
        // path shortening
        assert!(
            out.contains("~/.config/hector/adapters/claude-code/hook.sh"),
            "home-relative:\n{out}"
        );
        assert!(
    ...
    fn absolute_path_fallback_when_outside_home_and_project() {
        let plan = HarnessPlan {
            ...
            source: Source::Requested,
            steps: vec![PlanStep::Plugin {
                path: PathBuf::from("/opt/elsewhere/hector.ts"),
            }],
        };
        let out = render_plan(&[plan], false, &env(), false);
        assert!(
            out.contains("/opt/elsewhere/hector.ts"),
            "absolute fallback:\n{out}"
        );
    }
}
