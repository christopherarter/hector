//! `hector update` — self-update to the latest GitHub release.
//!
...
//! `hector-installer.{sh,ps1}` the user originally installed with, then
...
/// The dist *app name* (the package name `hector-cli`, not the `hector`
/// binary) — this is what dist prefixes installer assets with and what the
/// install receipt is keyed to (`~/.config/hector-cli/`), so it's the name
/// axoupdater must look up. Verified against the shipped `dist-manifest.json`
/// and `hector-cli-installer.sh`.
const APP_NAME: &str = "hector-cli";

/// The installer the no-receipt deferral points at — the same script the curl
/// one-liner runs. Matches the released asset name (`hector-cli-installer.sh`).
const INSTALLER_URL: &str =
    "https://github.com/christopherarter/hector/releases/latest/download/hector-cli-installer.sh";

/// Repo root, for the `cargo install --git` line in the no-receipt deferral.
/// `hector-cli` isn't published to crates.io, so a bare `cargo install
/// hector-cli` would fail — the git form is the correct one.
const REPO_URL: &str = "https://github.com/christopherarter/hector";
...
/// `hector update` entry point. Returns the process exit code: `0` on a
...
fn render(outcome: &Outcome) -> (String, i32) {
    match outcome {
        Outcome::Updated {
            old: Some(old),
            new,
        } => (format!("updated hector v{old} → v{new}"), 0),
        Outcome::Updated { old: None, new } => (format!("updated hector to v{new}"), 0),
        Outcome::AlreadyCurrent => (
            format!(
                "hector is already up to date (v{}).",
...
fn not_installer_managed_message() -> String {
    format!(
        "error: this hector wasn't installed by the hector installer, so it can't self-update.\n  \
         • reinstall (recommended): curl -LsSf {INSTALLER_URL} | sh\n  \
         • or, with cargo:          cargo install --git {REPO_URL} hector-cli --force"
    )
}
...
mod tests {
    ...
    fn render_not_installer_managed_points_to_real_update_paths_and_exits_one() {
        ...
        assert!(msg.contains("hector-cli-installer.sh"), "{msg}");
        // The cargo path must be the git form — hector-cli isn't on crates.io.
        assert!(msg.contains("cargo install --git"), "{msg}");
        assert!(msg.contains("hector-cli --force"), "{msg}");
    ...
    fn run_result_some_is_updated_with_both_versions() {
        let update = UpdateResult {
            ...
            install_prefix: camino::Utf8PathBuf::from("/tmp/hector"),
        };
        assert_eq!(
