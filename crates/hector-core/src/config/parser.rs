...
pub fn parse_str(input: &str) -> Result<Config> {
    ...
    for (id, gate) in &cfg.gates {
        if !run_has_executable_content(&gate.run) {
            return Err(anyhow!(
                "gate `{id}` has a `run` with no executable command (only blank or `#` comment \
...
///
/// A `run` made entirely of blank/comment lines (or empty) is a gate that can
...
mod tests {
    ...
    fn rejects_unknown_gate_field() {
        // `exclude:` was a real field in the pre-0.3 engine model; in 0.3 a gate
        // is exactly `{ files, run }`. A stale or typo'd field must be a hard
        // error, never silently dropped — dropping it makes the gate behave
    ...
    fn rejects_run_that_is_only_a_comment() {
        ...
        // whole multi-line script into a single comment line, yielding a gate
        ...
        assert!(
            err.contains("`g`"),
            "error must name the offending gate: {err}"
        );
    }

    #[test]
    fn rejects_empty_run() {
        // An empty (or whitespace-only) `run` is a gate that can never act —
    ...
    fn accepts_multiline_block_scalar_run() {
        ...
        // gate. Newlines are preserved verbatim into the `run` string (and
