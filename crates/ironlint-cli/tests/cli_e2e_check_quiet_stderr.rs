//! Routine `ironlint check` invocations must stay quiet on stderr for
...
fn check_stays_quiet_on_stderr_for_passing_gate() {
    ...
    let cfg = project.join(".ironlint.yml");
    fs::write(
        &cfg,
        "checks:\n  ok:\n    files: [\"*.txt\"]\n    run: \"exit 0\"\n",
    )
    ...
    assert!(
        stderr.is_empty(),
        "routine `ironlint check` against a passing gate must keep stderr empty; got: {stderr:?}"
    );
}
