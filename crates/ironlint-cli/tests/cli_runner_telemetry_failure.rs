...
fn telemetry_failure_warns_to_stderr() {
    ...
    // Pre-place a regular file at `<cfg_dir>/.ironlint` so that
    // `create_dir_all(".ironlint")` inside telemetry::append fails (parent
    // exists but is not a directory).
    fs::write(dir.path().join(".ironlint"), "not a directory").unwrap();
