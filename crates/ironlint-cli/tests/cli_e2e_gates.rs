...
/// files, not loop per-file. Proof: the check blocks iff `$IRONLINT_FILES`
...
fn diff_pre_commit_runs_check_once_over_set() {
    let dir = tempfile::tempdir().unwrap();
    // Pre-commit check: exits 2 when $IRONLINT_FILES has ≥2 entries (newline-
    ...
    cfg(
        dir.path(),
        "checks:\n  pc:\n    files: \"**/*.rs\"\n    on: [pre-commit]\n    run: \"printf '%s' \\\"$IRONLINT_FILES\\\" | awk 'END{exit (NR >= 2 ? 2 : 0)}'\"\n",
    );
    let xdg = common::blessed_store(&dir.path().join(".ironlint.yml"));
