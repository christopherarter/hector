...
description: Reviews ironlint check health from the telemetry log. Use when the user says "review my ironlint checks", "check health", "which ironlint checks are noisy", "find dead ironlint checks", "ironlint review", or asks for an audit of .ironlint.yml.
metadata:
  author: dynamik-dev
  version: 0.2.0
...
re-confirm what a check does on a file, run `ironlint check --file <path> --check
