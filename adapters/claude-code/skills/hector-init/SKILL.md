---
name: hector-init
description: Bootstraps a project's .hector.yml by detecting the tech stack from manifest files, wrapping existing linters as checks, and generating a baseline config. Use when user says "init hector", "set up hector", "bootstrap hector config", "create hector checks", "hector init", or asks to create or generate a hector configuration.
metadata:
  author: dynamik-dev
  version: 0.2.0
...
This creates `.hector.yml` with one or two starter checks for the detected stack
...
or blocks (with the check's message). See the `hector` skill for how to read a
block verdict.
...
- Telemetry lands at `.hector/log.jsonl`, one record per check with a per-check
